...
| 21 | **Trình lên TỰ ĐỘNG cho Phó GĐ phụ trách (fallback GĐ) hay TP/PP tự chọn thời điểm?** Hiện làm: TP/PP bấm nút «Trình Phó giám đốc», người nhận = Phó GĐ phụ trách phòng của công việc qua `department_managers`; phòng không có PGD phụ trách thì không ai được báo (admin vẫn duyệt được) | Không chặn | ⏳ **chờ trả lời** — mặc định đang dùng: **TỰ ĐỘNG** theo `department_managers` như luồng duyệt cây |
| 22 | **Giới hạn file kết quả nhiệm vụ?** (loại + dung lượng) | Không chặn | ⏳ **chờ trả lời** — mặc định: `.doc/.docx/.pdf` ≤ **20 MB**, mỗi lần nộp là một bản mới; đổi giới hạn là 2 hằng số |
| 23 | **RAM/HTTPS của VPS cho editor trực tuyến?** Nextcloud AIO cần domain + HTTPS (Let's Encrypt tự động) và ~2–4 GB RAM; ONLYOFFICE Docs standalone nhẹ hơn, KHÔNG cần Nextcloud — bảng so sánh ở §7 `docs/KE-HOACH-KET-QUA-FILE.md` | Phase editor trực tuyến (chưa làm — app chạy được ngay không cần editor: PDF iframe, DOCX tải về) | ⏳ **chờ trả lời** khi có VPS |
| 24 | **`.doc` cũ có cần xem trực tuyến không?** | Không chặn | ⏳ **chờ trả lời** — mặc định: chỉ tải về; `.pdf` xem bằng iframe trình duyệt; `.docx` mở editor là việc sau |
