...
| 8b | **Sửa lỗi vặt giao diện** (người dùng yêu cầu làm TRƯỚC Phase 9) | 🟡 | Mở 2026-09-06 — người dùng tạm gác e2e OnlyOffice/Zalo để ưu tiên sửa lỗi vặt: sửa trên PC, test xanh, rồi push nhánh `vps/sua-loi-vat` (cắt từ 81d9fb9 — bản VPS đang chạy) và deploy VPS theo runbook. **Lỗi 1 đã ghi**: menu «+ Tạo mới» trang «Quản lý tài khoản» KÉO DÀI khung đầu trang thay vì phủ xuống (đã khoanh vùng `web/index.html` ~386–417; menu «Xuất Excel» ~361–384 cùng khuôn phải đúng cả hai; nguyên nhân để session sửa tìm bằng DevTools). **Danh sách MỞ — người dùng bổ sung tiếp**. Prompt dán ở **mục 3b** `docs/BAT-DAU-SESSION.md`; dòng «Phase kế tiếp» mục 3 đã trỏ 8b trước Phase 9 |
