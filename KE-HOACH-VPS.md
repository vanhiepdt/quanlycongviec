...
- **Vai phân quyền `Quản lý công việc` đã bỏ bằng migration021 trên VPS ngày 2026-09-08.**
  Năm vai còn lại: admin, Phó Giám đốc, Trưởng phòng,
  Phó phòng, Nhân viên (nhãn Cán bộ). Không nhầm vai cũ với tên trang «Quản lý công việc»
  hoặc người được phân công quản lý một công việc; hai khái niệm đó vẫn giữ nguyên.
...
| Upload file | `multer` 2.x | Thêm 2026-09-01 (014, kết quả nhiệm vụ là file): `memoryStorage` + `limits.fileSize` 20 MB, whitelist đuôi/mimeType; cài bằng `npm i multer`. Lỗi `MulterError` phải đổi thành 400 bằng middleware đứng SAU `upload.single` — đặt trong catch của handler là không bao giờ thấy (multer tự `next(err)` trước khi vào handler) |
...
**REST không có tên RPC** (bổ sung 2026-09-06, §13.4 mục 16 chốt phương án b). Giao diện cũ chưa từng
vẽ danh sách thông báo nên không có tên RPC nào để nối; các đường dưới đây gọi thẳng REST bằng
`restGetIm`/`restGhi` như phần ủy quyền đang làm, **cầu RPC vẫn 37/37**:

| REST | Dùng cho |
|---|---|
| `GET /api/v1/notifications?limit=&onlyUnread=` | hộp chuông — CHỈ thông báo của chính người gọi |
| `GET /api/v1/notifications/unread-count` | con số trên badge, vòng hỏi lại 60 giây |
| `PATCH /api/v1/notifications/read` | đánh dấu đã đọc (`ids` rỗng = tất cả của mình) |
| `GET /api/v1/task-files/lenh-sua` | lệnh sửa của chính người gọi, đúng phạm vi nhiệm vụ |
| `POST /api/v1/task-files/:id/gui-ban-moi` | chủ lệnh có quyền sửa gửi bản mới nhất đã lưu qua `apTuDong` |
| `POST /api/v1/task-files/:id/huy-lenh-sua` | chủ lệnh hủy lệnh, giữ file, trả cửa trước và báo người ra lệnh |
| `PATCH /api/v1/task-files/:id/luu-tam` | chỉ ghi chú tạm của chủ lệnh, không đổi trạng thái |
| `POST /api/zalo-bot/webhook` | Zalo Bot gọi ngược vào (máy-đối-máy, mount TRƯỚC CSRF, chặn bằng `X-Bot-Api-Secret-Token`) |

Kế hoạch chi tiết: `docs/KE-HOACH-THONG-BAO.md`.
...
**Bản phát hành 2026-09-08, migration021:** bỏ vai hệ thống
`Quản lý công việc`; quyền của năm vai còn lại không đổi. Tài khoản mang vai cũ được chuyển
theo `dept_role`: Trưởng phòng/Phó phòng giữ đúng chức vụ, còn lại về Nhân viên; tài khoản
Nhân viên có `dept_role` TP/PP cũng được đồng bộ. Không đổi admin/Phó Giám đốc, mật khẩu,
liên kết Zalo, phòng hay phân công. Form/API đồng bộ hai trường cho nhóm TP/PP/Nhân viên,
không cho tạo lại vai cũ; CHECK CSDL và `can()` đều chặn. Xóa ghi đè quyền của vai cũ,
không sao chép sang vai khác. `apps.allowed_roles` chỉ có vai cũ chuyển thành `['admin']`
để tiếp tục đóng với người dùng thường (admin vốn luôn thấy); không biến thành mảng rỗng
(= mở cho tất cả), kể cả khi vai cũ bị lặp. Down chỉ nới CHECK,
không tự hoàn tác vai đã chuẩn hóa hoặc phục hồi ghi đè đã xóa.
...
`Trưởng phòng` / `Phó phòng` của phòng chứa nhiệm vụ đó**; `Nhân viên`
...
| — | **Ngoài kế hoạch: TÊN RIÊNG THEO TỪNG THÁNG cho đầu việc dài hơn một tháng (cả 3 cấp)** | ✅ | Xong 2026-08-28 trên nhánh `vps/quan-ly-nhiem-vu-pgd` — migration **008** `work_month_names` (hai FK rời `work_id`/`item_id` + `CHECK` đúng một cái, `month` dạng `YYYY-MM`, hai **unique bộ phận** `ux_wmn_work`/`ux_wmn_item`), REST `PUT`/`DELETE /works/:id/month-names/:month` và `/work-items/:id/...` (chặn **tháng đầu** = `MONTH_IS_FIRST`, ngoài khoảng = `MONTH_OUT_OF_RANGE`, cả hai 400), bản đồ `monthNames` gắn vào **3 đường đọc** (bootstrap, gantt, cầu RPC) — `COL.P_NAME`/`COL.T_NAME` **vẫn là tên gốc**; trình duyệt chọn tên theo tháng đang xem ở tab «Quản lý công việc», tab «Quản lý Nhiệm vụ» và **Sơ đồ Gantt**, tháng đã đổi thì **di chuột hiện tên cũ**. **1.325 test / 77 file** xanh; pin XSS 89/632 → **90/662**; cầu RPC vẫn **37/37**; banner `app.js 20260828-86`. Chi tiết §13.3 dòng 2026-08-28 cuối bảng + `docs/KE-HOACH-TEN-THEO-THANG.md`. Còn treo: test tay banner `-86` và `-85`, §13.4 **mục 19** và **mục 16** |
| — | **Ngoài kế hoạch: KẾT QUẢ NHIỆM VỤ LÀ FILE (Vòng 14 + 续1..续11)** | ✅ | Xong 2026-09-04 trên nhánh `vps/ket-qua-thiet-ke-lai` (tách từ `vps/ket-qua-file`) — migration **014** (4 bảng `task_files`/`task_file_versions`/`task_file_comments`/`task_file_flow`, chiều độc lập với `approval_status`) · **015** (sửa trực tuyến) · **016** (`ten_ket_qua`/`dinh_dang` + `noi_dung` + CHECK XOR file↔chữ). Luồng 5 trạng thái `cho-xem`→`can-sua`⇄→`cho-lanh-dao`→`da-duyet` (+`hoan-thanh`), 10 verdict, quyền đọc **Bảng phân quyền động** (`file:create`/`file:approve`, ✓ = duyệt tự động). 12 đuôi / 50 MB, `.svg` chặn có ý, ONLYOFFICE `word|cell|slide|pdf`. Hai bảng **8 cột** theo `docs/moi.xlsx`: khối «Kết quả» trong modal (dòng cha 1. 2. 3. + dòng bản 1.1/1.2 thu gọn sau ▸) và «Phê duyệt kết quả» phẳng ba cấp thành ba cột, mọi hành động trong **một menu ⋯** (dời ra `<body>` để không bị `backdrop-filter` cắt). Đợt 2: **khai kết quả TRƯỚC khi có file** («Chưa có» = nhóm 0 bản), **«Báo cáo» = bản không có file**, và **form TẠO nhiệm vụ hiện bảng 8 cột** + dòng khai tạm. **1588 test / 87 file** xanh; pin XSS **102/892**; banner + buster `20260905-1` (bố cục form 1/3–2/3, bỏ ô link nhiều dòng, TP/PP chọn được Cán bộ trực tiếp). Chi tiết §13.3 mười một dòng 2026-09-01→09-04; bẫy ở §13.5; hướng dẫn test tay `docs/HUONG-DAN-TEST-GIAO-DIEN.md` mục **9b**–**9b.10** |
| — | **Vá giao diện sau Vòng 14: menu, dòng khai kết quả, tạo và gửi duyệt, chi tiết hàng chờ** | 🟡 | Đang làm 2026-09-06. **Năm việc người dùng báo đã cài xong**: (1) «Xem chi tiết» ở hàng chờ duyệt vẽ dữ liệu THẬT của công việc vừa tạo (thêm khối «Thông tin công việc» + vẽ ngay từ ảnh bootstrap rồi nạp lại nền); (2) tạo xong công việc cha thì mở lại đúng modal chi tiết cha để tạo tiếp công việc con/nhiệm vụ; (3) chân form TẠO dính đáy có «Hủy» + **đúng hai** nút ý định «Lưu tạm»/«Gửi đi duyệt» (bỏ nút submit trần ở thanh tiêu đề — bấm vào là tạo bằng mặc định máy chủ mà người dùng không biết), «Chờ duyệt» hiện ngay không tải lại trang, và **bỏ hẳn** việc tự gán «Chờ duyệt» cho nhiệm vụ chỉ vì cha đang chờ; (4) dòng khai kết quả thẳng hàng — hàng nội dung Báo cáo tách khỏi 8 cột chính thành `<tr colspan=8>` riêng, xoá dòng thì xoá cả cặp và đánh số lại; (5) menu ⋯ nằm TRÊN modal (dời ra `<body>`, `position:fixed`, z-index 200 > 70). **TOÀN BỘ suite xanh: 1596 test / 87 file** (1588 + 8 test mới: TCKQ-37b, TCKQ-32b, 3 test chân form, 1 test «gửi công việc con không hạ nhiệm vụ đã duyệt», 2 test chi tiết authoritative), `npm run lint` + `format:check` sạch, `node --check` sạch. Pin XSS **giữ 102/893** — trừ một lỗ (`resultLinks`) thêm một lỗ (`buildLuuNhapNutHtml`), đối chiếu từng lỗ bằng `tools/dem-xss.mjs`. Banner + buster `app.js 20260905-2`, `app.css`/`project-details.js` `20260905-1`. Còn lại: **chưa test tay** trên trình duyệt thật (mục 9b.11). |
| — | **Ngoài kế hoạch: THÔNG BÁO — chuông trong ứng dụng (việc A) + đẩy Zalo (việc B)** | 🟡 | Bắt đầu 2026-09-06 theo yêu cầu người dùng «khi có thông báo phê duyệt gì thì nổi lên thông báo. Thêm thông báo đến zalo bot creator». **VIỆC A XONG**: §13.4 mục 16 chốt phương án (b) — mở `GET /notifications`, `GET /notifications/unread-count`, `PATCH /notifications/read` (đọc/ghi CHỈ của chính người gọi, `userId` từ query bị bỏ qua kể cả với admin); `bootstrap` trả thêm `unreadCount` nên badge đúng ngay lần vẽ đầu; chuông 🔔 cạnh nút chat, bấm dòng thì mở đúng công việc/nhiệm vụ theo `ref_type`/`ref_id`, «đánh dấu đã đọc hết», vòng hỏi lại **60 giây** và **chỉ khi tab đang hiện** (chat 10 giây đang bỏ sót điều này). Ba đường REST **không có tên RPC** ⇒ cầu vẫn **37/37**. **1625 test / 88 file xanh** (+29: TC-TBUI-01..06 là 18 test jsdom, 11 test REST/bootstrap), lint + `format:check` sạch. Pin XSS **102/893 → 104/900** (2 sink mới, 7 lỗ mới đều DA-THOAT trong `buildMotThongBao`). Banner + buster `app.js 20260906-1`. **VIỆC B (Zalo) — MÃ XONG ở Phase 8 việc 1** (2026-09-06): module `zalo/`, `services/zalo.js`, 3 script `zalo:kiem`/`zalo:link`/`zalo:webhook`, webhook `/api/zalo-bot/webhook` xác thực chữ ký bằng `safeEqual` có sẵn, `ZALO_BOT_NHAN` ba chế độ `tat`/`polling`/`webhook`, 4 câu §13.4 mục 25 được **chấp nhận qua prompt Phase 8** (webhook trên `ttdt.site`, mỗi cán bộ tự nhắn `LIENKET <mã>`). **43 test TC-ZL-01..14 xanh, suite 1668/1668**, pin XSS **104/900 → 106/903**. Người dùng XOAY token mới, tự dán `deploy/.env` (2026-09-06) ⇒ **HẠ TẦNG CHẠY THẬT**: `zalo:kiem` hợp lệ, webhook đặt + Zalo tự kiểm 200, sai chữ ký 403, lịch đẩy `*/2 *` bật; secret webhook XOAY lần hai vì bản đầu lọt `docker logs` (vá redact `2aeebd3`). **Còn lại → bước 0 Phase 9**: liên kết + đẩy duyệt với tài khoản THẬT (production chưa có tài khoản). Kế hoạch đầy đủ: `docs/KE-HOACH-THONG-BAO.md`. Chưa test tay mục **9b.12**. |
| 8 | Hạ tầng VPS, bảo mật, sao lưu | 🟡 | Nhánh `vps/phase-8-hatang`. **XONG việc 1–4 + 7** (2026-09-06): 1 mã Zalo (43 test) · 2 Dockerfile + compose (bcrypt alpine, db không publish port, OO bắt buộc JWT) · 3 deploy thật `/opt/qlcv` + swap 4 GB + 17 migration · 4 nginx host `https://ttdt.site` (HSTS đúng một lần, tài sản cache 30 ngày) · 7 hardening (UFW 22/80/443, fail2ban, SSH chỉ khoá, `.env` 600) + sao lưu cron 02:00 giữ 14 bản + **drill khôi phục thật ~2 giây** + `deploy/runbook.md`. **XONG tiếp việc 5 + 6** (cùng ngày): 5 — DNS A do người dùng trỏ, certbot cert RIÊNG, bật `office.ttdt.site.conf` + `websocket-map.conf`, kiểm thật HTTP→HTTPS 301 + HSTS + `/healthcheck=true` + `api.js` 200; 6 — token MỚI hợp lệ (`zalo:kiem`), webhook `https://ttdt.site/api/zalo-bot/webhook` đặt + Zalo tự kiểm 200, sai chữ ký 403, lịch đẩy bật. Vá giữa chừng: redact logger (`2aeebd3`, +2 test, **1670/1670**) + XOAY secret webhook vì bản đầu lọt `docker logs`. **Còn lại → bước 0 Phase 9**: e2e `.docx` + `LIENKET` + đẩy duyệt với NGƯỜI THẬT — production chưa có tài khoản nào (đúng thiết kế: 9.1/9.1b). Chi tiết §13.3 ba dòng 2026-09-06 |
| 8b | **Sửa lỗi vặt** (danh sách còn mở, trước Phase 9) | 🟡 | **2026-09-08: người dùng OK, ĐÃ PHÁT HÀNH bỏ vai phân quyền Quản lý công việc.** Commit `fcc8aed` máy chủ/migration021/restart + `f23fd34` giao diện, VPS ff-only đúng `f23fd34`; restart script exit0 (backup/build/migrate/tạo lại db/OO/app). NV004→Phó phòng, NV005→Trưởng phòng; role cũ/override/app-role cũ=0. Dữ liệu nghiệp vụ và users trừ role/updated_at fingerprint trùng; Zalo liên kết/mã=1/1, sessions4 dòng trước-sau (thời hạn phiên đổi). app/db healthy, OO=true, public health200; buster app20260908-1. Full PC1751/1751,99file,XSS106/903. Không reset/seed hay đổi .env/storage; `deploy/.env.save` untracked giữ nguyên. Script restart.sh đã trên VPS mode755; `--check` chỉ đọc. Danh sách lỗi vẫn mở. |
...
| 2026-08-28 | **TÊN RIÊNG THEO TỪNG THÁNG — đầu việc dài hơn một tháng đổi được tên cho từng tháng SAU, xem tháng nào hiện tên tháng đó (kể cả Gantt), di chuột hiện tên cũ** (3 commit: `1e48a23` bảng+repo, `abf93df` API, `dbdd218` trình duyệt). Yêu cầu nguyên văn: «Các công việc, nhiệm vụ mà thời gian nhiều hơn 1 tháng thì có chức năng sửa tên của công việc trong các tháng tiếp theo, khi xem thông tin công việc theo tháng nào thì sẽ hiển thị tên đã sửa của công việc, nhiệm vụ theo tháng đấy kể cả trên sơ đồ Gantt, nếu ko sửa thì mặc định vẫn tên vậy (áp dụng cho công việc cha và công việc con và cả nhiệm vụ) nhưng tháng sau khi được đổi tên thì khi di chuột vào công việc đấy sẽ hiển thị tên cũ». **Chọn bảng RỜI `work_month_names` chứ không thêm cột JSON** vào `works`/`work_items`: (a) mỗi tên tháng là một dòng có `updated_at` riêng nên nhật ký ghi được từng lần đổi; (b) `CHECK` dạng `YYYY-MM` + chặn tên trắng nằm ở CSDL, không phụ thuộc mã; (c) hai cấp dùng **một** bảng nhờ hai FK rời `work_id`/`item_id` + `CHECK ((work_id IS NULL) <> (item_id IS NULL))` — kéo theo bẫy: `ON CONFLICT` **phải nêu đúng tên unique bộ phận** (`ux_wmn_work`/`ux_wmn_item`), viết `ON CONFLICT (work_id, month)` là lỗi «no unique or exclusion constraint matching». Luật nghiệp vụ: **tháng ĐẦU không đặt tên riêng** (`MONTH_IS_FIRST`) vì tháng đầu chính là tên gốc, tháng ngoài khoảng bắt đầu→kết thúc bị chặn (`MONTH_OUT_OF_RANGE`), cả hai **400**; PUT với tên trắng = **bỏ** tên riêng (đi chung đường với DELETE) nên giao diện không cần hai lối riêng. Bản đồ `monthNames` gắn vào **cả 3 đường đọc** (`bootstrap/service.js`, `gantt/service.js`, `rpc/legacyFields.js`) nhưng `COL.P_NAME`/`COL.T_NAME` **giữ TÊN GỐC** — chỗ quyết định là: **máy chủ không chọn được tên**, vì hai tab nạp `allProjects`/`allTasks` **một lần** rồi lọc tháng tại trình duyệt (không gọi lại), và **một** lượt `/api/v1/gantt` có thể trải nhiều tháng ⇒ máy chủ gửi bản đồ, trình duyệt chọn. Trình duyệt: 9 tiện ích thuần (`thangCuaNgay`, `cacThangCuaDauViec` chặn **240** tháng để ngày gõ sai không kéo vòng lặp vô hạn, `thangSuaDuocCuaDauViec` = bỏ tháng đầu, `banDoTenThangCuaDong` nhận **cả** `monthNames` lẫn `month_names`, `tenTheoThangCuaDong` — tên trắng/thiếu/«Tất cả tháng» đều lùi về tên gốc, `tenGocNeuDaDoiCuaDong`, `thangLocNhiemVu`, `thangLocGantt`, `nhanThangVN`) + **tab thứ 3 «Tên theo tháng»** trong modal chỉnh sửa, chỉ hiện khi đầu việc dài hơn một tháng (tab rỗng trông như tính năng hỏng). Ba bẫy giao diện đã vá ngay: ô nhập **không có `name`** nên không lọt vào `FormData` của `task-form` (panel nằm TRONG form), `onkeydown` biến **Enter** thành «lưu tháng này» thay vì bấm Cập nhật cả nhiệm vụ, và mọi `data-name`/`data-project-name` + ô «Tên» của form **vẫn là tên gốc** để hộp thoại Xoá/Nhân bản/Cập nhật (không có tháng nào trong tầm nhìn) không bị tên tháng leo vào. Vì không có lượt tải lại nào, `ghiTenThang` **vá `monthNames` của dòng trong bộ nhớ** rồi vẽ lại cả hai tab (+ Gantt nếu đang mở). R6 «hiện tên cũ» làm bằng `title="Tên gốc: …"` ở 2 tab và dòng «Tên gốc» trong thẻ tooltip Gantt — bản nháp đầu dùng span `sr-only` **không** đạt vì không hiện khi di chuột, đã đổi ngay. **1.287 → 1.325 test / 74 → 77 file** xanh (TC-TENTHANG-01..12 đơn vị · 13..24 tích hợp API · **25..38** jsdom); lint + `format:check` sạch; `node --check app.js` OK; pin XSS **89/632 → 90/662** ghi trong **cùng commit** với `docs/XSS-4.6.md`; banner + buster `app.js 20260828-86` (CSS không đổi); cầu RPC vẫn **37/37**. Cố ý **không** đổi: Tổng quan, thống kê, biểu đồ, xuất Excel, tìm kiếm — ở đó không có một tháng nào trong tầm nhìn nên tên gốc mới đúng | `server/src/db/migrations/008_work_month_names.sql` (mới), `server/src/utils/monthNames.js` (mới), `server/src/modules/workMonthNames/{repo,service}.js` (mới), `server/src/modules/{works,workItems}/{service,routes}.js`, `server/src/modules/{bootstrap,gantt}/service.js`, `server/src/modules/rpc/legacyFields.js`, `server/src/utils/errors.js`, `server/tests/{helpers/db.js,helpers/http.js}`, `server/tests/unit/{ten-thang,ten-thang-ui,xss-guard}.test.js`, `server/tests/integration/ten-thang-api.test.js`, `server/eslint.config.js`, `web/assets/js/app.js`, `web/index.html`, `docs/KE-HOACH-TEN-THEO-THANG.md` (mới), `docs/XSS-4.6.md`, `docs/BAT-DAU-SESSION.md`, `KE-HOACH-VPS.md` §13.2 + §13.3 | **Test tay** với banner `20260828-86`: mở modal sửa một công việc dài hơn một tháng → tab «Tên theo tháng» liệt kê tháng 2..N (không có tháng đầu), lưu một tên rồi xem tab «Quản lý công việc», tab «Quản lý Nhiệm vụ» và Gantt của **đúng tháng đó** phải hiện tên mới, di chuột hiện tên cũ; đầu việc gói trong một tháng thì **không** có tab ở cả 3 cấp. Vẫn treo: test tay nhật ký (banner `-85`) và tab nhiệm vụ PGĐ (banner `-84`), §13.4 **mục 19** và **mục 16**, soát mắt §10 + §12 của `docs/KE-HOACH-UY-QUYEN.md`. Sau đó **Phase 8** — hạ tầng VPS thật |

| 2026-08-29 | **Phó Giám đốc lại không thấy công việc/nhiệm vụ của phòng mình phụ trách — hoá ra là RACE CONDITION ở trình duyệt, không phải lỗi phân quyền** (2 commit: `5a17fab` bản vá, `29c57a7` dứt điểm đỏ giả nhật ký kiểm toán). Yêu cầu nguyên văn: «lại lỗi Phó Giám đốc không thấy nhiệm vụ và công việc của phòng mình phụ trách», nói rõ thêm giữa lượt: «bị lõi,, user PGD một thì ko thấy công việc nhiệm vụ user PGD hai thì lại thấy». Câu «PGĐ một không thấy, PGĐ hai thấy» kéo hướng điều tra về phía máy chủ, nên **loại trừ máy chủ trước** bằng script gọi API thật (đăng nhập → `getDepartmentContext` → bootstrap → RPC `getTasks`): `pgd1@test.local` nhận **5 công việc / 24 dòng cấp 2–3** của `PH01`+`PH02`, `pgd2@test.local` nhận **4 / 6** của `PH03`+`PH04` — **cả hai đều đúng**; tên phòng so từng byte NFC khớp, không có dòng `delegations` nào dính vào. Gốc lỗi nằm ở **thứ tự**: `handleSuccessfulLogin` gọi `renderProjects()`/`renderTasks()` **đồng bộ ngay**, còn `visibleDepartments` chỉ về **sau** bằng `getDepartmentContext()` bất đồng bộ, và trước đây **không có gì vẽ lại 2 tab** khi nó về ⇒ lần vẽ đầu chạy với `visibleDepartments = []` nên `getUserAllowedProjects()` (nhánh `laQuanTriTrongPhamVi()`) và `dsNhiemVuToiDuocThay()` trả rỗng. Vì thế nó **chập chờn** theo thứ tự thao tác — đổi tab hay bấm lại là thấy — và trông như "tài khoản này lỗi, tài khoản kia không". Sửa: `loadDepartmentContext` nhớ vai **trước** khi ghi biến, hễ `isDeputyDirector` bật **hoặc vừa tắt** thì vẽ lại `renderProjects` · `renderTasks` · `renderProjectStats` · `renderTaskStats` · `renderStats` · `renderPriorityTasksMini` (+ `renderGanttChart` nếu đang mở), bọc `try/catch` — chính chỗ này bắt được **regression của tôi**: test `project-form-phan-cong` mở form khi danh sách phòng chưa nạp xong, DOM chỉ có modal nên `renderProjectStats` ném `innerHTML` của `null` và **chặn mất `callback(response)`**, làm ô chọn phòng rỗng. **Không đổi một dòng máy chủ nào** — `inScope()` và `departmentContext()` đã đúng; đúng tinh thần "máy chủ là rào chặn cuối, client không tự nới quyền". Nhân lượt chạy cả bộ, dứt điểm luôn **chùm đỏ giả của nhật ký kiểm toán**: `middleware/audit.js` cố ý ghi ở `res.on('finish')` (để nhật ký không làm chậm người dùng) nên lượt ghi **sống lâu hơn** request — test đọc `activity_logs` liền sau supertest thì chưa thấy dòng, còn lượt ghi rơi muộn lại **lọt qua `TRUNCATE` của test SAU** và hiện ra như dòng lạ; đó là lý do TC-UQ-13b/13c/17, `proposal.update`, `chat.send`, TC-RPC-21 đỏ **đổi chỗ mỗi lượt chạy** và từng bị ghi nhận là "đỏ giả có sẵn". Chữa bằng `flushAudit()` xuất từ `audit.js` (tập promise đang bay, chạy thật thì tự rỗng nên không tốn gì): `resetTables()` chờ nó **trước** `TRUNCATE`, 7 helper đọc nhật ký gọi nó trước `SELECT` — thay 4 bản `waitForLogs` đếm-rồi-ngủ và 3 chỗ đọc thẳng. **1325/1325 test xanh, 77/77 file, hai lượt `npm test` liền không còn ca nào đỏ** (trước đó mỗi lượt đỏ 1–4 ca khác nhau); lint + `format:check` sạch; `node --check` OK; pin XSS **giữ nguyên 90/662** (chỉ vẽ lại, không sinh HTML mới); banner + buster `app.js 20260828-87` (CSS không đổi). Xác nhận thêm trên UAT: `008_work_month_names` **đã** chạy (`pgmigrations` + 2 dòng `work_month_names`) — dòng "đang nợ" ở `BAT-DAU-SESSION.md` là **sai**, đã sửa | `web/assets/js/app.js`, `web/index.html`, `server/src/middleware/audit.js`, `server/tests/helpers/db.js`, `server/tests/integration/{delegations-api,proposals-api,chat-api,rpc-bridge,auth-password,reminders-api,work-origin-history}.test.js`, `docs/BAT-DAU-SESSION.md`, `KE-HOACH-VPS.md` §13.3 + §13.5 (2 bẫy mới) | **Test tay** với banner `20260828-87`: đăng nhập *PGĐ Một* → **ngay lần đầu**, chưa đổi tab, hai tab «Quản lý công việc» và «Quản lý Nhiệm vụ» phải có dữ liệu của `PH01`+`PH02`; *PGĐ Hai* tương tự với `PH03`+`PH04`; đăng xuất rồi vào bằng vai khác → **không** được nới thêm gì. Vẫn treo: test tay tên theo tháng (banner `-86`), nhật ký (`-85`), tab nhiệm vụ PGĐ (`-84`), §13.4 **mục 16** và **mục 19**, soát mắt §10 + §12 của `docs/KE-HOACH-UY-QUYEN.md`. Sau đó **Phase 8** — hạ tầng VPS thật |

| 2026-08-28 | **Màn hình duyệt «Chờ duyệt» + hiện người duyệt (client cho luồng duyệt Phase 5).** Trước đó: server đủ `GET /approvals/pending` + approve/reject nhưng client không gọi đâu cả ⇒ việc chờ duyệt treo mãi, không ai biết ai duyệt/duyệt ở đâu. Đã làm: panel «Chờ duyệt» trong Quản lý công việc (chỉ admin/Phó GĐ thấy, nạp `GET /approvals/pending` theo phạm vi), dòng việc có nút Duyệt / Từ chối (lý do ≥ 10 ký tự, client+server cùng chặn), `restPost` + CSRF cookie `_csrf`/`/api/csrf`; nhãn «Chờ duyệt» và modal chi tiết hiện **tên người duyệt** (`COL.P_APPROVER`, thiếu thì ghi «Phó Giám đốc phụ trách phòng») và lý do từ chối. Test mới `approvals-ui.test.js` 5 test (builder/escape/ẩn-hiện theo vai/POST đúng đường). Pin XSS 90/662 → **92/668**. Bump `project-details.js?v=20260828-1` | `web/index.html`, `web/assets/js/app.js`, `web/assets/js/project-details.js`, `server/tests/unit/approvals-ui.test.js` (mới), `server/eslint.config.js`, `server/tests/unit/xss-guard.test.js`, `docs/XSS-4.6.md`, `docs/NHAT-KY-MAN-HINH-DUYET.md` (mới) | Phase 8 — hạ tầng; deploy sync `web/` lên VPS |

| 2026-08-29 | **Vòng 5–6 Gantt/Nhiệm vụ/Phân quyền (5 phản hồi).** (1) Thanh Gantt mất màu — `.gantt-bar-subwork` chưa từng có luật màu ⇒ khối màu ghim đè cuối app.css (project xanh/subwork cam/task xanh lá/overdue đỏ). (2) Bỏ mã (CV00x) ở tab Nhiệm vụ + Công việc — `boLocMaCV()` bọc kết quả render (regex từ charCode, không đụng chuỗi template). (3) PGD sửa công việc cha/CV con phòng mình — `canUserEditResource` viết lại (server rbac đã có update trong phạm vi). (4) Nút ✎ sửa CV con ở 2 tab cho admin/PGD/TP/PP; **TP/PP sửa cấp 2 đã duyệt ⇒ server tự hạ về «Chờ duyệt»** (`workItems/service.update` trả `choDuyetLai`) chờ Phó GĐ duyệt lại. (5) Ủy quyền: `phamViBadge` (approvals) + `boLocPhong` (Gantt/nhiệm vụ) gộp phòng của bên ủy quyền từ `user.delegations` ⇒ Phó GĐ được ủy quyền thấy nút duyệt + việc phòng bên ủy quyền. Pin giữ **92/668** (dem-xss đo lại). Bump `app.js?v=20260828-88`, `app.css?v=20260828-7`, banner `-88` | `web/assets/js/app.js`, `web/assets/css/app.css`, `web/index.html`, `server/src/modules/workItems/service.js`, `server/src/modules/approvals/service.js`, `server/src/modules/stats/service.js`, `server/eslint.config.js` | Phase 8 — hạ tầng; deploy sync `web/` + `server/src/` lên VPS, restart Node |

| 2026-08-29 | **«Hoạt động gần đây» ở Tổng quan đọc được (4 lỗi: action thô · `{}` · rác `rpc.*`/`bootstrap.get` · mã CV003).** Gốc: cầu RPC đặt mốc `res.locals.audit` cho **mọi** lời gọi kể cả lời ĐỌC, và route GET (`bootstrap.get`, `stats.*`, `gantt.tree`) **giẫm đè** mốc của cầu qua subrequest (`locals` dùng chung) ⇒ mỗi lần mở trang sinh cả chục dòng rác. Sửa: cầu nhận lời ĐỌC bằng mốc còn nguyên ⇒ `skipAudit`; bỏ 5 dòng audit chết ở route GET; lọc dòng rác cũ khỏi 2 đường đọc (ẩn, không xoá); `moTaNhatKy`/`moTaChiTietHoatDong` mới (hết `{}`, tên đầu việc + Tháng n/YYYY **bỏ mã**, «Cập nhật N trường»); `renderActivity` → builder `createHoatDongItemHtml` theo bản đồ NHAT_KY_HANH_DONG (+21 hành động: auth/users/departments/delegations/proposal/app/notification/chat). Test: jsdom `hoat-dong-ui.test.js` TC-HD-01..06 + TC-RPC-24 — **1337 test / 79 file** xanh; pin **92/670**; banner `app.js 20260829-1`. Chi tiết `docs/NHAT-KY-HOAT-DONG-GAN-DAY.md` | `server/src/rpc/index.js`, `server/src/rpc/legacyFields.js`, `server/src/modules/{bootstrap,stats,gantt}/routes.js`, `server/src/modules/{activityLogs,stats}/repo.js`, `server/src/modules/{works,workItems}/routes.js`, `web/assets/js/app.js`, `web/index.html`, `server/tests/unit/hoat-dong-ui.test.js` (mới), `server/tests/integration/rpc-bridge.test.js`, `server/eslint.config.js`, `server/tests/unit/xss-guard.test.js`, `docs/XSS-4.6.md` | Phase 8 — deploy sync `web/` + `server/src/` lên VPS, restart Node |

| 2026-08-29 | **Vòng 7: bỏ nốt mã khỏi tên · «Tạo mới» hết nhảy đầu trang · TP/PP thêm được công việc.** (1) Mã `(CV002)`/`(CV002-009)`/`CV002-010` còn ở 5 chỗ (vòng 5 bị `git checkout` hoàn tác một phần) + 2 chỗ project-details.js — gỡ hết khỏi HTML hiển thị, GIỮ mã trong `data-*`/onclick; (2) dropdown «Tạo mới» là `<a href="#">` ⇒ bấm là cuộn lên đầu trang — bọc `preventDefault` cả 7 nút; (3) TP/PP/PGD thêm công việc: máy chủ đã cho `work.create`, client mở nút qua helper mới `laLanhDaoPhong()` + gate `laQuanTriTrongPhamVi() || isManager() || laLanhDaoPhong()` (TP/PP tạo xong về «Chờ duyệt» như luật Phase 5). Test: TC-TASKUI-19, TC-CV-BL-2, TC-TP-UI + sửa assertion cũ ten-thang-ui/project-details — **1343 test / 79 file** xanh; pin **92/666**; banner `app.js 20260829-2`. Chi tiết `docs/NHAT-KY-GANTT-THEO-THANG.md` mục Vòng 7 | `web/assets/js/app.js`, `web/assets/js/project-details.js`, `web/index.html`, `server/tests/unit/{tasks-nhiem-vu-ui,bo-loc-cong-viec,pho-giam-doc-ui,ten-thang-ui,project-details-phan-cong,xss-guard}.test.js`, `docs/XSS-4.6.md` | Phase 8 — deploy sync `web/` lên VPS |

| 2026-08-29 | **Vòng 8: BẢNG PHÂN QUYỀN HỆ THỐNG vào trang «Quản lý tài khoản».** Làm lại bảng cũ tĩnh 3 cột (ADMIN/QUẢN LÝ/CÁN BỘ, đã GỠ khỏi section Cán bộ) thành bảng động **15 chức năng × 6 vai** vẽ từ hằng `BANG_PHAN_QUYEN` (khớp `PERMISSIONS`/`inScope` + `trangThaiDuyetKhiTao`), ký hiệu ✓/⏳/**làm được nhưng chờ Phó GĐ duyệt**/✕/↻(ủy quyền)/👁(chỉ xem) + ghi chú phạm vi từng vai; khung mới chèn giữa «Thông tin tài khoản» và «Đổi mật khẩu» (`#account-permission-table`, `renderTrangTaiKhoan` gọi `veBangPhanQuyen()`). Bẫy: helper bọc escape khiến TC-SEC-10 báo 6 lỗ KHÔNG thoát → viết lại builder escape trực tiếp. Test TC-TKPQ-01..06 — **1349 test / 79 file** xanh; pin **93/675**; banner `app.js 20260829-3`. Chi tiết `docs/NHAT-KY-GANTT-THEO-THANG.md` mục Vòng 8 | `web/assets/js/app.js`, `web/index.html`, `server/tests/unit/{tai-khoan-ui,xss-guard}.test.js`, `docs/XSS-4.6.md` | Phase 8 — deploy sync `web/` lên VPS |

| 2026-08-29 | **Vòng 9: BẢNG PHÂN QUYỀN ĐỘNG — admin sửa bằng dropdown · bỏ đối tượng Quản lý công việc.** Migration **009_permission_overrides.sql** (`(vai, entity_type, action) → gia_tri`, CHECK giới hạn work/subwork/task, «chờ duyệt» chỉ cho create); REST `GET/PUT /api/v1/permissions` (chỉ admin, zod, audit `permissions.update`); `attachSession` nạp `user.ghiDe` ⇒ `can()` vẫn thuần — lớp 4: `tu-choi` tắt (đè cả ma trận), `cho-phep`/`cho-duyet` mở khi ma trận từ chối (inScope vẫn xét), admin không chịu ghi đè; `trangThaiDuyetKhiTao` tôn trọng ghi đè create (cho-phep ⇒ «Đã duyệt» ngay). Client: bảng hiển thị **bỏ cột Quản lý công việc**, chú thích ký hiệu xuống dưới cùng; khung «Sửa bảng phân quyền» chỉ admin — dropdown từng ô (3 nhóm × hành động × 3 vai) + nút Lưu (PUT, hiệu lực ngay). Test: unit `phan-quyen-ghi-de` TC-PQ-10..13, integration `permissions-api` TC-PQ-01..09, jsdom TC-TKPQ-01..08 cập nhật — **1368 test / 81 file** xanh; pin **98/686**; banner `app.js 20260829-4`. Kế hoạch + còn nợ: `docs/KE-HOACH-PHAN-QUYEN-DONG.md` | `server/src/db/migrations/009_permission_overrides.sql` (mới), `server/src/modules/permissions/*` (mới), `server/src/app.js`, `server/src/middleware/{session,rbac}.js`, `server/src/modules/approvals/rules.js`, `web/assets/js/app.js`, `web/index.html`, `server/tests/{helpers/db.js,unit/phan-quyen-ghi-de.test.js (mới),integration/permissions-api.test.js (mới),unit/{tai-khoan-ui,xss-guard}.test.js}`, `docs/XSS-4.6.md` | Phase 8 — deploy **phải chạy `npm run migrate:up`** + sync `web/` + `server/src/`, restart Node |

| 2026-08-29 | **Vòng 10: dropdown ngay trên BẢNG 15 chức năng + điều kiện phạm vi cho PGD/TP/PP.** Migration **010** (`permission_overrides` thêm `pham_vi` 'phong'⟷'tat-ca'); `can()` bỏ qua `inScope()` khi ô được nới «Tất cả các phòng» (chỉ Phó GĐ/TP/PP được nới — service chặn 400); `GET /permissions` mở cho mọi vai (bảng không phải dữ liệu mật), PUT vẫn admin; client: **12 hàng nghiệp vụ × 4 vai là dropdown ngay trên bảng** (Mặc định/✓/⏳/✕) + dropdown phạm vi riêng cho PGD/TP/PP, nút Lưu dưới bảng; người thường thấy badge trạng thái hiệu lực («Ghi đè», «TẤT CẢ các phòng»); trình sửa kỹ thuật riêng của Vòng 9 gỡ. Test: TC-PQ-10 (nới phạm vi thật + cản vai khác), TC-TKPQ-01..08 viết lại — **1371 test / 81 file** xanh; pin **95/694**; banner `app.js 20260829-5`. Chi tiết `docs/KE-HOACH-PHAN-QUYEN-DONG.md` §4 | `server/src/db/migrations/010_permission_overrides_pham_vi.sql` (mới), `server/src/modules/permissions/*`, `server/src/middleware/{rbac,session}.js`, `server/src/modules/approvals/rules.js`, `web/assets/js/app.js`, `web/index.html`, `server/tests/{integration/permissions-api,unit/tai-khoan-ui,xss-guard}.test.js`, `docs/XSS-4.6.md` | Phase 8 — deploy **migrate:up (010)** + sync `web/` + `server/src/`, restart Node |

| 2026-08-29 | **Vòng 11→12c: 4 chỉnh bảng theo ảnh (chú thích xuống cùng, option «Đang dùng», TP/PP thêm ⏳ cho Sửa/Xoá — migration **011**, dropdown + phạm vi cùng 1 hàng, Cán bộ badge «Phòng của mình») → nút Lưu biến mất (Vòng 10 xoá nhầm, khôi phục) → dropdown lặp option → Lưu im lặng do cột Cán bộ gửi `vai='Cán bộ'` (nhãn) thay vì `'Nhân viên'` ⇒ 400 — đã đổi `VAU_BANG.vai`. Vòng 12c: TP/PP bị khoá nhầm 3 chỗ — icon sửa CV con (`coQuyenSuaCongViecCon` chỉ nhìn phân công), tab nhiệm vụ trống (`dsNhiemVuToiDuocThay` thiếu nhánh TP/PP theo phòng cha), bảng phân quyền đứng im (`oPhanQuyenHieuLuc` đoán gốc bằng cột tĩnh — giờ đọc ma trận server từ `GET /permissions`). **1375 test / 81 file** xanh; pin **96/698**; banner `app.js 20260829-10`. Chi tiết `docs/KE-HOACH-PHAN-QUYEN-DONG.md` §5–§8 | `server/src/db/migrations/011_permission_cho_duyet_update_delete.sql`, `server/src/modules/{permissions/*,works/service.js,workItems/service.js}`, `server/src/modules/approvals/rules.js`, `web/assets/js/{app.js,project-details.js}`, `web/index.html`, `server/tests/unit/{tai-khoan-ui,project-details-phan-cong,xss-guard}.test.js` | Phase 8 — deploy migrate:up (011) + sync `web/` + `server/src/`, restart Node |

| 2026-08-29 | **Vòng 12d: «Quản lý nhiệm vụ của TP không thấy» — race còn sót.** Tab không bị ẩn, nó TRỐNG: `loadDepartmentContext` chỉ vẽ lại khi `isDeputyDirector` đổi (vá 2026-08-28 chỉ nửa bài), TP/PP vẽ lúc `myDepartment` rỗng rồi không bao giờ vẽ lại. Sửa: điều kiện vẽ lại thêm `isDepartmentHeadUser \|\| truocLaHead`. Test: TC-TASKUI-17 viết lại (TP thấy nhiệm vụ phòng mình qua `myDepartment`, không dùng kênh PGĐ), thêm TC-TASKUI-19 (rỗng ⇒ trống, bối cảnh về ⇒ thấy). **1376 test / 81 file** xanh; banner `app.js 20260829-11`. Chi tiết `docs/KE-HOACH-PHAN-QUYEN-DONG.md` §9 | `web/assets/js/app.js`, `web/index.html`, `server/tests/unit/tasks-nhiem-vu-ui.test.js` | chỉ web — sync `web/`, Ctrl+Shift+R |

| 2026-08-31 | **Vòng 12e: 3 lỗi người dùng + SOÁT TOÀN BỘ hiển thị theo vai (ma trận chân lý 5 vai).** Đo trước khi sửa: máy chủ KHÔNG sai — `getBundle()` cho Phó phòng PH01 trả đủ 4 công việc + 11 nhiệm vụ. (A) `getUserAllowedProjects()` **thiếu nhánh TP/PP** ⇒ chạy app.js thật trong jsdom trả `["CV008"]` thay vì 4; CV008 chạy 01→31/12 nên hoá ra «chỉ thấy tháng 12» — trùng hợp dữ liệu, `workMatchesMonth` vẫn đúng. Sửa: nhánh TP/PP theo phòng từ `tenPhongTaiKhoan()` (KHÔNG dùng `visibleDepartments`). (A2) «nhiệm vụ không thấy tháng nào»: `dsNhiemVuToiDuocThay` đã đúng (T6/T7/T8/T9 đều có, T12 trống vì dữ liệu) — ô Tháng tab Nhiệm vụ THIẾU «Tất cả tháng»; đã thêm (value 0), mặc định vẫn tháng hiện tại. (A3) phát hiện thêm: `renderTaskStats` có bộ lọc riêng hẹp hơn ⇒ 4 thẻ đếm hiện 0 cho PGĐ/TP/PP, nay dùng lại `dsNhiemVuToiDuocThay()`. (B) «không sửa được chức năng nào của Cán bộ» KHÔNG phải thiếu dropdown: `buildBangPhanQuyenHtml` tra ghi đè/ma trận bằng **NHÃN** 'Cán bộ' còn server khoá bằng **VAI CSDL** 'Nhân viên' ⇒ mọi ô hiện «✕ Tắt» và option đầu `value=""` nên bấm Lưu là **XOÁ SẠCH ghi đè** (im lặng — `'mac-dinh'` là lệnh hợp lệ). Đã đổi toàn bộ sang `vaiCot.vai`. (B2) theo yêu cầu giữa session: Cán bộ có ⏳ ở **Tạo + Sửa** (server `permissions/service.js` nới, không cần migration — CHECK 011 đã cho); **không** mở Xoá vì 'cho-duyet' ở delete là CHẶN xoá mà chưa có luồng duyệt-yêu-cầu-xoá. (C) ô lệch: server cho TP/PP `delete` cả 3 cấp nhưng `canUserDeleteResource` không có nhánh nào và nút ✎/⧉/🗑 của thẻ CV + Gantt cũng không ⇒ đã thêm `laLanhDaoPhong()`. Ma trận chân lý 5 vai + bảng đối chiếu hai bộ lọc tháng ghi ở §10 `docs/KE-HOACH-PHAN-QUYEN-DONG.md`. Test mới TC-TKPQ-11..15, TC-TP-CV-01..05, TC-PQ-13 — **1387 test / 81 file** xanh; pin XSS giữ **96/698**; banner `app.js 20260829-12` | `web/assets/js/app.js`, `web/index.html`, `server/src/modules/permissions/service.js`, `server/tests/unit/{tai-khoan-ui,pho-giam-doc-ui,tasks-nhiem-vu-ui}.test.js`, `server/tests/integration/permissions-api.test.js`, `docs/{KE-HOACH-PHAN-QUYEN-DONG,XSS-4.6}.md` | Phase 8 — deploy sync `web/` + `server/src/`, restart Node (không có migration mới) |
| 2026-09-03 | **Vòng 14续7: 3 lỗi người dùng — «Tải lên thất bại: máy chủ từ chối», GĐ/PGĐ không thấy mục «Hàng chờ phê duyệt», hàng chờ trống dù đã nạp data mới.** Cả ba đều là **dư âm của việc siết quyền ở 续6**, không phải lỗi mới của luồng file. (1) **Giám đốc/Phó Giám đốc không thấy mục**: lời gọi `capNhatNavChoDuyet()` bị đặt TRONG nhánh «đổi vai TP/PP/Phó GĐ» của `loadDepartmentContext` ⇒ admin không bao giờ chạy tới, mục nằm im với `class="hidden"` của HTML — vai quyền cao nhất lại là vai duy nhất không thấy cửa duyệt, và không lỗi nào hiện ra. Sửa: tách khối `try/catch` ra NGOÀI nhánh vai; badge chỉ cộng `/approvals/pending-count` cho vai thật sự duyệt cây (`laNguoiDuyetHeThong()`), TP/PP chỉ đếm file. (2)+(3) **«Tải lên thất bại: máy chủ từ chối» và hàng chờ TRỐNG**: 续6 siết `listChoDuyetKetQua` + `nop`/`verdict` theo `$1 = ANY(i.leader_ids)`, nhưng **bộ seed `dev-vong14.sql` để cột `leader_ids` rỗng** ⇒ TP/PP không phải «lãnh đạo phòng phụ trách» của nhiệm vụ nào: hàng chờ trống và nộp file bị 403. Đo bằng REST cho 4 vai trước khi sửa để chắc là seed thiếu chứ không phải quyền sai. Sửa seed: CV001-001 (cấp 2) nêu `ARRAY[tp.id, pp.id]` — bắt buộc vì `LEADER_NOT_IN_SOURCE` của `assignments/service.js` đòi lãnh đạo cấp 3 nằm trong danh sách của cấp 2; 5 nhiệm vụ cấp 3 chia đôi TP/PP bằng `CASE` vì cấp 3 chỉ được MỘT lãnh đạo (`task_leader_single`, migration 005). Test mới `nav-cho-duyet.test.js` TC-NAV-01..07 (chạy app.js thật trong jsdom, 5 vai; TC-NAV-07 canh bằng NGUỒN để lỗi «gộp lại vào `if`» không tái diễn mà test hành vi vẫn xanh) — **1549 test / 87 file** xanh; pin XSS giữ **101/830**; banner `app.js 20260903-1` | `web/assets/js/app.js`, `web/index.html`, `server/src/db/seeds/dev-vong14.sql`, `server/tests/unit/nav-cho-duyet.test.js`, `server/eslint.config.js` | seed lại `npm run seed:v14` (hoặc `chay-test.bat /v14`) — **bắt buộc**, data cũ không có `leader_ids`; Ctrl+Shift+R |
| 2026-09-03 | **Vòng 14续6: 8 lỗi/yêu cầu người dùng — phân công, cập nhật tại chỗ, siết `leader_ids`, bảng cây hàng chờ, nộp bản mới, thanh tải lên, giao diện công việc cha.** (1) **Trưởng phòng không chọn được cán bộ khi tạo nhiệm vụ**: `napUngVienPhanCong()` **thiếu hẳn nhánh TP/PP** — họ rơi vào nhánh "chỉ chính mình" rồi bị lọc `role === 'Nhân viên'` ⇒ danh sách RỖNG, không có lỗi nào hiện ra. Cùng lớp bẫy với Vòng 12e (A): `getUserAllowedProjects` cũng từng thiếu đúng nhánh này. Sửa: TP/PP lấy toàn bộ Cán bộ trong phòng từ `tenPhongTaiKhoan()`, KHÔNG dùng `visibleDepartments` (biến đó chỉ đúng cho PGĐ). (2) **Tạo công việc/CV con xong phải tắt-mở modal mới thấy**: `showProjectDetailsModal` chỉ vẽ từ `allProjects`/`allTasks` trong bộ nhớ, còn hàm tạo mới **không** nạp lại hai mảng đó khi modal đang mở ⇒ dựng lại HTML từ dữ liệu cũ. Sửa: sau khi tạo thành công, nếu `#project-details-modal` còn trong DOM thì `loadDataForUser()` rồi vẽ lại modal bằng `openedFromProjectDetails` — không thêm endpoint nào. (3) **Nhân viên sửa được ô «Lãnh đạo phòng phụ trách»**: form không khoá ô này theo vai ⇒ Cán bộ tự đổi người duyệt của mình, và vì mọi cửa duyệt file đọc `leader_ids` (mục 4) nên đây là lỗ **leo quyền thật**, không chỉ là lỗi hiển thị. Sửa `khoaPhanCongVoiNhanVien()`: Cán bộ thấy ô ở dạng `disabled` + câu «Do lãnh đạo phòng phân công»; máy chủ vẫn là rào cuối (`assignments/service.js` đã kiểm `LEADER_NOT_IN_SOURCE`). (4) **«Không phải lãnh đạo phòng phụ trách nhiệm vụ đấy vẫn sửa/phê duyệt được»**: `repo.listChoDuyetKetQua` bó theo `i.department_id = phòng mình` ⇒ MỌI TP/PP của phòng đều thấy và bấm được. Đổi sang `$1::bigint = ANY(i.leader_ids)` — khớp đúng `laLanhDaoPhuTrachNhiemVu()` mà `verdict`/`nop` đã dùng từ 续5, hết cảnh hàng chờ rộng hơn quyền thật. Hệ quả **có chủ ý**: nhiệm vụ chưa gán lãnh đạo thì KHÔNG TP/PP nào thấy (admin/PGĐ vẫn xử được nên file không treo vĩnh viễn). (5) **Hàng chờ phê duyệt thành BẢNG THEO CÂY**: `buildBangChoDuyetKetQua` + `buildHangCayChoDuyet` chèn hàng tiêu đề mỗi lần đổi nhóm, SQL thêm `ORDER BY w.code, cha.code NULLS FIRST, i.code` và `JOIN works` + `LEFT JOIN work_items cha` — client **không** sắp lại để bảng luôn khớp thứ tự máy chủ; 5 cột (Công việc/Nhiệm vụ/File · Trạng thái · Bản mới nhất · Ý kiến · Hành động), cột «Ý kiến» chỉ hiện chữ + số rồi bấm mới mở nhiệm vụ (độ rộng bảng có hạn — đúng yêu cầu người dùng). (6) **Nộp bản mới ngay trong hàng chờ**: `moChonFileChoDuyet()` đặt trước `fileKetQuaChoBan`/`taskKetQuaMa` rồi bật cờ `dangOTrangChoDuyet` để dùng LẠI `uploadKetQua` — phải có **ô `<input type=file>` riêng của trang** (`kq-cho-duyet-file-input`) vì lúc ở hàng chờ thì modal nhiệm vụ chưa dựng, ô kia không tồn tại trong DOM. (7) **Thanh trạng thái tải lên**: `veTrangThaiUpload()` ba pha (đang tải → xong → lỗi), dựng bằng `createElement`/`createTextNode` nên **không** thêm lỗ XSS nào. (8) **Giao diện công việc cha**: khối phân công gom thành 3 chip một dòng (`khoiPhanCongGon`, bấm «Chi tiết» mới bung), cây đổi sang khung `.cay-nhanh` có gờ màu — nhánh «Nhiệm vụ trực thuộc công việc» dùng màu khác (`.cay-nhanh-le`) để tách bạch với nhiệm vụ nằm trong công việc con. Test mới TCKQ-20..22, TC-PDPC-04..06, TC-HCPD-04 sửa lại theo `leader_ids` — **1542 test / 86 file** xanh; pin XSS **101/830** (chỗ ghi KHÔNG đổi, chỉ +19 giá trị — ca đầu tiên như vậy, xem `docs/XSS-4.6.md`); banner `app.js 20260902-2`, `app.css 20260902-2`, `project-details.js 20260902-1` | `web/assets/js/app.js`, `web/assets/js/project-details.js`, `web/assets/css/app.css`, `web/index.html`, `server/src/modules/taskFiles/{repo,service,routes}.js`, `server/tests/integration/task-files-api.test.js`, `server/tests/unit/{task-files-ui,project-details-phan-cong,xss-guard}.test.js` |


| 2026-09-02 | **Vòng 14续5: 5 lỗi/yêu cầu người dùng — «KHÔNG LƯU ĐƯỢC» (kèm ảnh hộp thoại DS), tên file mất dấu, nút Lưu, lãnh đạo phòng phụ trách, tách trang «Hàng chờ phê duyệt».** (1) **Hộp «Không thể lưu tài liệu. Vui lòng kiểm tra cài đặt kết nối…» KHÔNG phải lỗi kết nối**: route `/callback/:id` trả qua `ok()` của §5.3 ⇒ `{"ok":true,"data":{"error":0}}`, còn tài liệu «Callback handler» đòi **đúng `{"error":0}` ở CẤP CAO NHẤT** — DS không thấy khoá `error` nên coi là lưu thất bại rồi `storeForgotten`, trong khi bản mới **vẫn được lưu** (Lịch sử đúng) nên rất khó lần; log DS `sendServerRequest returned an error: data = {"ok":true,…}` là dấu vết duy nhất. Sửa: trả `res.json({error:0})` — ngoại lệ có chủ ý của §5.3, ghi lý do ngay trên route (TC-TF-18 canh cả ca status 1 / url rác / token sai). (2) **Tên file mất dấu tiếng Việt** (`BÀI 2.docx` → `BÃ€I 2.docx`): busboy giải `filename` của multipart bằng **latin1** trong khi trình duyệt gửi UTF-8; multer 2.x không có tuỳ chọn bảng mã ⇒ `tenGocUtf8()` gỡ ngược tại MỘT chỗ, ba lớp canh không phá tên vốn đúng (ASCII / có ký tự > 0xff / giải ra `U+FFFD`), dò bằng `codePointAt` vì regex `[^\u0000-\u00ff]` bị eslint `no-control-regex` (TC-TF-19 đi đường thật FormData→multer→CSDL). (3) **«Sửa xong lưu lại vào nhiệm vụ kiểu gì»**: Docs API KHÔNG có method JS nào bắt editor lưu ⇒ dùng **command service** `POST {dsUrl}/command` thân `{c:'forcesave', key, userdata}` + `token` JWT của chính thân đó; trang editor thêm **thanh trên** (tên nhiệm vụ · tên file · **«Lưu thành bản mới»** · «Đóng» · dòng trạng thái), route mới `POST /task-file-versions/:id/save` (`luuNgay`), 6 mã lỗi DS dịch sang tiếng Việt và **mã 4 = «chưa có thay đổi» không phải lỗi**; `document.key` tách thành `khoaDs(ban)` vì forcesave phải gửi ĐÚNG key editor đang mở. (4) **Lãnh đạo phòng phụ trách là người xem/sửa/duyệt + NHẬN THÔNG BÁO**: `repo.lanhDaoPhuTrach()` gộp `users` (TP/PP đứng tên phòng) **và** `department_managers` (`'head'`/`'vice'`) — trước chỉ đọc `users` nên người được gắn phụ trách mà vai khác không hề biết có file mới; `luuTuCallback` đọc `users[0]`/`actions[0].userid` của DS để ghi ĐÚNG người + ĐÚNG vai (trước ghi cứng người nộp + vai `'Nhân viên'`, nên TP sửa file cán bộ thì Lịch sử hiện tên cán bộ), hành động mới `'sua-truc-tuyen'` — migration **015** nới CHECK `task_file_flow`, down XOÁ dòng trước khi siết; sửa trực tuyến nay gửi thông báo cho lãnh đạo phòng + người nộp bản trước. (5) **Tách «Hàng chờ phê duyệt» thành trang riêng, 2 tab con**: mục mới `#nav-cho-duyet` (badge số) → `#cho-duyet-section`; tab «Công việc / Nhiệm vụ» **chuyển nguyên** `#approvals-panel` + hộp «Yêu cầu XOÁ» từ trang Công việc (giữ id nên hàm cũ chạy y nguyên, KHÔNG để lại bản sao — hai khối cùng id thì `getElementById` chỉ thấy một cái), tab «Phê duyệt kết quả» đọc `GET /task-files/cho-duyet` (mới): TP/PP thấy `cho-xem`+`can-sua` phòng mình, PGĐ thấy `cho-lanh-dao` các phòng phụ trách, admin thấy cả ba; máy chủ trả kèm `hanhDong[]` tính lại bằng `BANG_VERDICT` + `giaTriHieuLuc` (⏳ ở «Duyệt kết quả» ⇒ mất nút chốt ngay trong hàng chờ). **Kiểm chứng đầu-cuối bằng chính DS, không qua trình duyệt**: nộp `.docx` có dấu → CSDL giữ đúng tên → `/raw` **200/18278 byte** → `ConvertService.ashx` trả **`fileUrl`** → `POST /callback` `status=6` + `users=[id TP]` → **`{"error":0}`** và CSDL có **v1(Lê Thị Nhân), v2(Trần Thị Trưởng)**, dòng luồng `sua-truc-tuyen/Trưởng phòng`, thông báo tới người nộp trước + Phó phòng. Bẫy mới: `fetch` của Node 24 phân giải `localhost` sang `::1` ⇒ `ECONNRESET` dù DS sống (dùng `127.0.0.1`); máy chủ dev có thể nối `quanlycongviec_uat` **hoặc** `quanlycongviec` — hai bộ tài khoản khác nhau, 401 với mật khẩu đúng là dấu hiệu nối sai CSDL. Test mới TC-TF-18/19, **TC-HCPD-01..04**, TCKQ-16..19 — **1538 test / 86 file** xanh, lint + `format:check` sạch; pin XSS **99/792 → 101/811**; banner `app.js 20260902-1`, `app.css 20260902-1` | `server/src/db/migrations/015_file_sua_truc_tuyen.sql` (mới), `server/src/modules/taskFiles/{service,routes,dsRoutes,repo}.js`, `web/assets/js/app.js`, `web/index.html`, `web/assets/css/app.css`, `server/tests/integration/task-files-api.test.js`, `server/tests/unit/{task-files-ui,xss-guard}.test.js`, `docs/{KE-HOACH-KET-QUA-FILE,BAT-DAU-SESSION}.md` | Phase 8 — deploy **`npm run migrate:up` (015)** + sync `web/` + `server/src/`, restart Node, **kiểm 3 biến `ONLYOFFICE_*` trong `deploy/.env`** |


| 2026-09-02 | **Vòng 14续4: ONLYOFFICE NHÚNG THẬT — sửa lỗi «KO THẤY MÀN HÌNH SỬA» (người dùng gửi kèm `docs-api/get-started/basic-concepts`).** Đo trước khi sửa: route `/task-file-versions/:id/editor` trả **200**, HTML đúng cú pháp tài liệu (`<div id="placeholder">` + `api.js` + `new DocsAPI.DocEditor`), DS `busy_merkle` sống (`/healthcheck` 200, `api.js` 200) ⇒ lỗi KHÔNG ở HTML mà ở **hai lớp khác, cái nào cũng ra đúng triệu chứng trang trắng**. (1) **CSP của `helmet()` chặn `api.js` của Document Server**: `script-src 'self'` áp cho MỌI phản hồi, `api.js` ở origin khác ⇒ trình duyệt chặn thẻ script ⇒ `DocsAPI` không tồn tại ⇒ trang TRẮNG, lỗi chỉ hiện ở tab Console. Sửa: `cspEditor(dsUrl)` + `res.setHeader('Content-Security-Policy', …)` cho **đúng một** route — nới `script-src`/`frame-src` (DocEditor dựng iframe trỏ DS)/`connect-src` (+`ws:`/`wss:`)/`img-src`/`style-src`/`font-src`/`media-src`, bỏ `Cross-Origin-Embedder-Policy`, đặt `Cross-Origin-Resource-Policy: cross-origin`; phần còn lại của API giữ CSP chặt. (2) **`/raw/:id` làm SẬP CẢ MÁY CHỦ**: `createReadStream(duong).pipe(res)` với file không tồn tại phát `'error'` không ai bắt ⇒ Node ném «Unhandled error event» và tiến trình CHẾT — gặp ngay ở bộ seed Vòng 14 (chỉ có dòng CSDL, không có file trên đĩa), nên bấm ✎ là máy chủ sập và mọi thứ khác chết theo. Sửa: `access()` kiểm trước ⇒ **404** gọn + listener `'error'` cho trường hợp file mất giữa lúc truyền. Theo tài liệu `usage-api/config/document` sửa thêm: `documentType` **không ghi cứng `'word'`** (bảng đuôi → `pdf`/`word`; seed có `.pdf`), `document.key` phải ĐỔI khi nội dung đổi (ghép `tf-<id>-<size>-<mốc nộp>`, cắt 128 ký tự, chỉ ký tự cho phép — key trùng thì DS lấy lại bản trong **cache** của nó), thêm `events.onAppReady`/`onError`/`onRequestClose` + khối `#loi` để mọi đường thất bại hiện **một câu tiếng Việt + 3 bước cần kiểm** thay vì trang trắng. Kiểm chứng đầu-cuối KHÔNG qua trình duyệt: nộp `.docx` thật → `/editor` 200 với `script-src … http://localhost` → `docker exec busy_merkle wget <raw url>` **200, 7726 byte** → `POST /ConvertService.ashx` của DS với JWT ký bằng `ONLYOFFICE_JWT_SECRET` trả **`fileUrl`** (không `error`) ⇒ chứng minh cùng lúc secret hai bên TRÙNG, DS TẢI ĐƯỢC file qua `host.docker.internal:3000`, DS ĐỌC ĐƯỢC nội dung. Test mới TC-TF-16 (CSP có origin DS ở 3 mục, không COEP, CORP cross-origin, có `onerror=`/`onAppReady`, `documentType` theo đuôi cho cả docx+pdf) và TC-TF-17 (`/raw` thiếu file ⇒ 404 **và** `/healthz` vẫn 200 = máy chủ còn sống); `vitest.config.js` thêm 3 biến `ONLYOFFICE_*` giả để tính năng BẬT trong test — thiếu là route trả trang «chưa bật» và lỗi này lọt cổng test. **1528 test / 86 file** xanh; pin XSS giữ **99/792** (trang editor là HTML máy chủ, không vào phép đếm app.js); banner `app.js` không đổi (`20260901-6`) | `server/src/modules/taskFiles/{service,routes,dsRoutes}.js`, `server/vitest.config.js`, `server/tests/integration/task-files-api.test.js`, `docs/KE-HOACH-KET-QUA-FILE.md` (§11 mới), `docs/XSS-4.6.md` | Phase 8 — deploy sync `server/src/`, restart Node; **`deploy/.env` phải có 3 biến `ONLYOFFICE_*`** (URL cho trình duyệt ≠ CALLBACK_BASE cho DS) |

| 2026-09-01 | **Vòng 14续3: 3 lỗi người dùng báo + DATA TEST MỚI theo quy trình file.** (1) **Nút ✎ sửa trực tuyến không hiện**: `GET /work-items/:ref/files` không trả cờ nào về cấu hình ONLYOFFICE nên client luôn coi là tắt ⇒ route trả thêm `onlyOffice: service.onlyOfficeBat()`, client đọc vào `dsBat`. (2) **Phân quyền editor**: thêm `duocSuaTrucTiep()` + `moEditor` trả `mode: 'edit' | 'view'` — nhóm đã chốt (`da-duyet`) thì MỌI người chỉ xem, kể cả TP; người ngoài phòng không tới được vì `can(read,'task')` chặn 403 trước đó (TC-TF-15 canh cả ba nhánh). (3) **«Trưởng phòng không sửa được nhiệm vụ» — lỗi ở CLIENT, không phải máy chủ**: `PERMISSIONS['Trưởng phòng'].task` đã có `update` và `inScope` đã bó theo phòng, nhưng `canUserEditResource` chỉ mở `project`/`subwork` cho `laLanhDaoPhong()` ⇒ nhiệm vụ do Cán bộ khác phụ trách bị chặn ngay ở trình duyệt bằng toast «Bạn không có quyền chỉnh sửa mục này» (đối xứng: `canUserDeleteResource` đã có `'task'` từ Vòng 12e — lệch từ đó). Nay thêm `'task'`, TC-TP-CV-05 mở rộng canh luôn. (4) **Data test mới** (yêu cầu người dùng «xoá data cũ, tạo lại theo quy trình mới»): `server/src/db/seeds/dev-vong14.sql` + `npm run seed:v14` — TRUNCATE sạch dữ liệu nghiệp vụ rồi dựng 2 phòng · 7 người (`gd/pgd/tp/pp/nv1/nv2/nvb@test.local`, `Test@12345`, **không** bắt đổi mật khẩu) · 1 công việc → 1 CV con → **5 nhiệm vụ, mỗi cái một trạng thái luồng file**: NV-01 chưa có file (chỗ bấm «Tải file lên» để chạy luồng thật có file trên đĩa), NV-02 `cho-xem`, NV-03 `can-sua` (2 bản + ý kiến), NV-04 `cho-lanh-dao` (TP tự sửa bản 2 rồi trình), NV-05 `da-duyet` (khoá) + 8 thông báo mẫu (TP/PP nhận `approval_pending` cho **cả** nhiệm vụ mới và file chờ xem — đúng chỗ người dùng báo thiếu; PGĐ phụ trách nhận dòng «trình Phó GĐ phụ trách»). Bảng phân quyền để TRỐNG = dùng mặc định Vòng 14. `dev.sql` cũ **giữ nguyên** (bộ §8.3 cho Phase 0–13). Test mới `seed-vong14.test.js` TC-SEED-V14-01..06 (có ca chạy seed HAI LẦN) — **1526 test / 86 file** xanh; pin XSS giữ **99/792**; banner `app.js 20260901-6` | `server/src/modules/taskFiles/{routes,service}.js`, `web/assets/js/app.js`, `web/index.html`, `server/src/db/seeds/dev-vong14.sql` (mới), `server/package.json`, `server/tests/integration/{task-files-api,seed-vong14 (mới)}.test.js`, `server/tests/unit/pho-giam-doc-ui.test.js`, `docs/XSS-4.6.md` | Phase 8 — deploy sync `web/` + `server/src/`, restart Node (014 đã lên từ vòng trước) |

| 2026-09-01 | **VÒNG 14: KẾT QUẢ NHIỆM VỤ LÀ FILE — nộp → góp ý → duyệt, đọc Bảng phân quyền động.** Nhánh `vps/ket-qua-file`. Migration **014** — 4 bảng `task_files`/`task_file_versions`/`task_file_comments`/`task_file_flow` (chiều ĐỘC LẬP với `approval_status`, tiền lệ 3 cột `xoa_*` của 013) + nới 2 CHECK phân quyền (`po_entity_ok` thêm `'file'`, `po_cho_duyet` cho file:approve × TP/PP; down xoá dòng 'file' TRƯỚC khi siết); ma trận `PERMISSIONS` thêm thực thể `'file'` + helper **`giaTriHieuLuc`** (ghi đè thắng, mặc định theo vai: Cán bộ/TP/PP nộp là ⏳, Phó GĐ nộp là chốt luôn, admin không chịu ghi đè); `permissions/service.js` nới luật cho-duyet (file:create × Cán bộ/TP/PP, file:approve × TP/PP — Phó GĐ đặt ⏳ ở hàng file = 400); multer + module `taskFiles` (5 REST: nộp multipart / đọc / tải `?inline=1` cho PDF / góp ý theo bản / verdict với state machine 5 trạng thái `cho-xem`·`can-sua`·**`hoan-thanh`**·`da-duyet`·`cho-lanh-dao`) — TP chốt = `hoan-thanh` (người dùng chốt), `da-duyet` chỉ do PGD/GĐ «Duyệt» hoặc **TỰ ĐỘNG** khi giá trị hiệu lực cửa nộp = ✓ + dòng luồng «Tự động — phân quyền không yêu cầu duyệt», hiệu lực NGAY vì `attachSession` nạp lại; giao diện: tab «Kết quả & Luồng» trong modal nhiệm vụ (khuôn tab Nhật ký) + Bảng phân quyền 13 → **15 hàng**; PDF xem iframe, DOCX tải về (KHÔNG deploy Nextcloud/OnlyOffice — đánh giá trong `docs/KE-HOACH-KET-QUA-FILE.md` §7 + 4 câu hỏi §13.4 mục 21–24). Test: `task-files-api.test.js` TC-TF-01..14, `task-files-ui.test.js` TCKQ-01..19, TC-PQ-14 (giaTriHieuLuc), TC-TKPQ-06/14 cập nhật + TC-TKPQ-17, TC-DB-01 tự thích nghi — người dùng trả lời §13.4 mục 21–24 (21: TP/PP tự chọn khi trình — khớp cách đã làm; 22: 20 MB OK; 23: CÓ editor trực tuyến — session sau theo OnlyOffice, mọi lần sửa phải LƯU LẠI thành bản mới để xem; 24: .doc VÀ .docx đều sửa/xem trực tuyến) + thêm ô **«Ý kiến»** vào khối file (gửi vào bản mới nhất; các nút Yêu cầu sửa/Trình/Trả về đọc ô này trước khi hỏi lại) — **1519 test / 85 file xanh**; pin XSS **99/792**; banner `app.js 20260901-4` |

| 2026-08-31 | **Vòng 13 (ĐỢT 1/2): LUỒNG NHÁP → GỬI DUYỆT CẢ CÂY → DUYỆT CẢ CÂY.** Migration **012** thêm trạng thái duyệt thứ tư `Nháp` (CHECK ở cả `works` và `work_items`; **hai view `v_countable_*` nới sang `NOT IN ('Chờ duyệt','Nháp')`** — chỗ DUY NHẤT phải sửa để nháp không vào con số nào, đúng thiết kế 004; down migration **hạ dữ liệu trước khi siết CHECK**, đã thử down+up sạch). Nguồn sự thật mới `thayDuocNhap` (approvals/rules.js): nháp chỉ **người lập + admin** thấy, gọi ở **cả 5 đường đọc** (`/works`, `/works/tree`, `/work-items`, `/bootstrap`+RPC `getDataForUser`, và `getOne` trả **404** chứ không 403). Ba luồng lan cây dùng chung `ghiKhoaDuyetCaCay(target, patch, tuTrangThai)`: **gửi duyệt** cấp 1 ⇒ cả cây sang «Chờ duyệt» và hộp duyệt hiện **MỘT dòng gốc** (`listPending` loại dòng có cha đang chờ; badge vẫn đếm đủ — hai câu hỏi khác nhau); **duyệt** ⇒ **LAN XUỐNG CẢ CÂY** (luật cũ «không lan» của TC-APR-16 bị thay, người dùng chốt — cả cây gửi một lần và có «Xem chi tiết» đọc trước khi ký); **trả lại để sửa** (endpoint mới `POST /:entity/:id/return`) ⇒ cả cây về `Nháp` của người tạo, giữ ghi chú, KHÔNG mất dữ liệu; **từ chối = XOÁ HẲN cả cây** (người dùng xác nhận rõ là vĩnh viễn — thông báo gửi TRƯỚC khi xoá, `refType/refId` rỗng vì id đã mất). Client: nhãn **xám «Nháp»** + nút «Gửi duyệt» trên thẻ, nút **«Lưu nháp»** chỉ ở form TẠO MỚI (hai nút cùng `type=submit` nên bắt bằng listener `click` — jsdom không dựng `event.submitter`), màn duyệt thêm **Xem chi tiết** + **Trả lại để sửa** và **hỏi lại** trước khi Từ chối, modal chi tiết có **chế độ CHỈ-ĐỌC** (`cheDoDuyetChiDoc` — tắt icon ✎ + 2 nút thêm, dải nhắc, CV con chờ duyệt **tô khung vàng + chữ «đang chờ duyệt»**, tự tắt cờ khi đóng modal). Test mới `nhap-api.test.js` (TC-NHAP-01..09) + TC-APR-16 viết lại + TC-APR-17..19 + TC-DUYET-UI-01..05 + TC-CV-NHAP-01..04 + 4 ca view. Pin XSS **96/715** (+17 giá trị, 0 chỗ ghi mới); banner `app.js 20260831-1`, buster cả 3 file. **ĐỢT 2 còn nợ: duyệt nhiệm vụ cấp 3 + luồng yêu cầu xoá 3 cấp** — thiết kế + 7 quyết định ở `docs/KE-HOACH-DUYET-CAY.md` | `server/src/db/migrations/012_approval_draft.sql` (mới), `server/src/modules/approvals/{rules,service,routes,repo}.js`, `server/src/modules/{works,workItems}/{service,routes}.js`, `server/src/modules/works/tree.js`, `server/src/modules/bootstrap/service.js`, `server/src/utils/zodTypes.js`, `server/src/rpc/legacyFields.js`, `web/assets/js/{app.js,project-details.js}`, `web/assets/css/app.css`, `web/index.html`, `server/tests/integration/{nhap-api.test.js (mới),approvals-api,approval-pending-lock,countable-views}.test.js`, `server/tests/unit/{approvals-ui,project-details-phan-cong,bo-loc-cong-viec,xss-guard}.test.js`, `docs/{KE-HOACH-DUYET-CAY.md (mới),XSS-4.6.md}` | Phase 8 — deploy **`npm run migrate:up` (012)** + sync `web/` + `server/src/`, restart Node |
| 2026-09-01 | **Vòng 13 (ĐỢT 2/2): DUYỆT NHIỆM VỤ CẤP 3 + LUỒNG XIN XOÁ/DUYỆT XOÁ CẢ 3 CẤP.** Migration **013** — `works`/`work_items` thêm 3 cột `xoa_yeu_cau_boi`/`xoa_yeu_cau_luc`/`xoa_ly_do` («xin xoá» là chiều ĐỘC LẬP với `approval_status`: từ chối xoá trả về đúng trạng thái cũ), hai view `v_countable_*` dựng lại trong cùng migration; `assertCoBuocDuyet` mở cho nhiệm vụ đang Chờ duyệt/Nháp/Từ chối (admin bật ⏳ ô «Tạo Nhiệm vụ» thì nhiệm vụ mới rơi Chờ duyệt và **có người duyệt được** — hết kẹt vĩnh viễn); REST `request-delete`/`approve-delete`/`reject-delete` + `GET /pending-deletes`, badge cộng `deletes`; mục xin xoá **VẪN hiện + VẪN vào thống kê** kèm nhãn đỏ «Đang xin xoá»; hộp «Yêu cầu xoá» trong panel Chờ duyệt (Đồng ý xoá có confirm — mất cả cây; Từ chối không cần lý do); bấm 🗑 khi vai bị ⏳ ở Xoá ⇒ đổi thành «Xin xoá» nhập lý do ≥ 10 ký tự; bảng phân quyền +2 hàng («Duyệt Nhiệm vụ (cấp 3)» dropdown · «Duyệt yêu cầu XOÁ» chỉ hiển thị), option ⏳ ở Xoá mở cho Cán bộ; **1450 test / 83 file xanh** (`docs/KE-HOACH-DUYET-CAY.md` mục 8) |

| 2026-09-03 | **Mở thêm PowerPoint / Excel / ẢNH cho kết quả nhiệm vụ + nới 20 → 50 MB** (người dùng yêu cầu: «thêm cả up được cả file ppt và ảnh và excel nữa nhé»). Nhánh `vps/ket-qua-thiet-ke-lai`. Whitelist `DUOI_FILE_HOP_LE` từ **3 đuôi → 12** (`.doc .docx .pdf` · `.xls .xlsx` · `.ppt .pptx` · `.jpg .jpeg .png .gif .webp`) và đổi hình dạng: đuôi → **DANH SÁCH** mime (Office 2003 gửi mime khác nhau tuỳ trình duyệt: `application/vnd.ms-excel`, `application/excel`, `application/x-msexcel`…) nên so bằng `includes`, không `===`; regex đuôi + câu lỗi + `accept=` đều **SINH TỪ CHÍNH BẢNG** để không bao giờ lệch nhau. **`.svg` bị chặn CÓ Ý** (SVG là XML chạy được `<script>` ⇒ mở inline là XSS lưu trữ dưới tên miền của hệ thống) — chốt hai đầu bằng TC-TF-14 (máy chủ 400) + TCKQ-13 (client không gọi máy chủ). `?inline=1` đổi từ «đúng bằng `application/pdf`» sang whitelist `MIME_XEM_INLINE` = PDF + 4 ảnh raster; **Excel/PowerPoint dù xin `inline=1` vẫn ra `attachment`** (TC-TF-14c canh cả hai chiều). ONLYOFFICE: `DOCUMENT_TYPE_THEO_DUOI` thêm `xls/xlsx → 'cell'`, `ppt/pptx → 'slide'`; **bỏ `?? 'word'`** — trước đây mọi đuôi lạ hoá thành Word nên mở ảnh là ra một trang editor lỗi không ai hiểu, nay 400 với câu đọc được, và client ẩn nút ✎ ở ảnh bằng `suaTrucTuyenDuoc()`. Dung lượng: `DUNG_LUONG_TOI_DA` 20 → **50 MB**, thêm hằng `NHAN_DUNG_LUONG` vì con số «20 MB» từng viết cứng ở **6 câu lỗi** khác nhau (2 chỗ trong `routes.js`, 3 trong `service.js`, 1 ở client) — nay một chỗ. **KHÔNG nâng cao hơn 50 MB**: multer vẫn `memoryStorage()` nên mỗi file nằm trọn trong RAM lúc tải. Test mới TC-TF-14b (nộp được cả 6 định dạng mới, giữ đúng tên có dấu `ảnh chụp.jpg`), TC-TF-14c (inline/attachment), TC-TF-16b (editor `cell`/`slide`, ảnh 400), TCKQ-05b (👁 hiện ở ảnh, ẩn ở Office), TCKQ-13b (9 đuôi mới qua được cửa client), TCKQ-13c (50 MB + 1 byte bị chặn, đúng 50 MB đi qua), TCKQ-17b (✎ theo đuôi) — **1549 → 1556 test / 87 file** xanh; pin XSS **101/830 → 101/831** (+1 giá trị: `accept=` nay nội suy qua `escapeHtmlAttr`, số chỗ ghi KHÔNG đổi); banner `app.js 20260903-2` + buster khớp | `server/src/modules/taskFiles/{service,routes}.js`, `web/assets/js/app.js`, `web/index.html`, `server/tests/integration/task-files-api.test.js`, `server/tests/unit/{task-files-ui,xss-guard}.test.js`, `docs/{XSS-4.6,HUONG-DAN-TEST-GIAO-DIEN,BAT-DAU-SESSION}.md` | Phase 8 — deploy sync `web/` + `server/src/`, restart Node (**không** migration mới); Ctrl+Shift+R. **Nếu Nginx/reverse proxy có `client_max_body_size` thì phải nới lên ≥ 50 MB**, nếu không file lớn bị chặn ở tầng proxy trước khi tới Node (413, câu lỗi không phải tiếng Việt của app) |

| 2026-09-04 | **Vòng 14续11 — ĐỢT 2: khai kết quả TRƯỚC khi có file + «Báo cáo» là bản không có file + form TẠO hiện bảng 8 cột.** Migration **016**: `task_files.ten_ket_qua`/`dinh_dang` (CHECK Word/Excel/PPT/PDF/Ảnh/Báo cáo), `task_file_versions.noi_dung` + nới NOT NULL 3 cột file + CHECK `tfv_file_hoac_chu` (XOR file ↔ chữ ≥ 10 ký tự). «Chưa có» = nhóm **0 bản** (không dựng bản rỗng giả). Hai JSON mới: `POST /work-items/:ref/results` (`khaiKetQua` — luôn để `cho-xem`, **không** auto-duyệt kẻo khóa dòng trống → 409 lúc nộp file) và `POST /…/reports` (`nopBaoCao` — **có** `apTuDong`). Hàng chờ `AND v.id IS NOT NULL` nên nhóm 0 bản không vào queue. Form **TẠO NHIỆM VỤ**: nhúng `buildKhungDanhSachKetQua([], "")` vào chuỗi HTML (cấp 3, không cấp 2) nên bảng 8 cột + dòng khai tạm **1.** hiện NGAY — sửa lỗi người dùng «khi tạo mới không thấy bảng kết quả»; ＋ = `themDongKhaiTam` (không POST); ô **không** `name=`; `handleAdd` thu `thuThapDongKhaiTam()` **trước** `closeModal`, sau `taskId` mới `guiDongKhaiTam`. Test: TC-KQ2-01..09 + TCKQ-34..40, TCKQ-23 đổi «rỗng vẫn là bảng». **1567 → 1583 test / 87 file** xanh; pin XSS **101/873 → 102/893** (+1 sink `insertAdjacentHTML` của ＋, +20 giá trị). Banner + buster `20260904-3`. | `server/src/db/migrations/016_ket_qua_khai_truoc.sql`, `server/src/modules/taskFiles/{repo,service,routes}.js`, `web/assets/js/app.js`, `web/index.html`, `server/tests/{integration/task-files-api,unit/task-files-ui,unit/xss-guard}.test.js`, `KE-HOACH-VPS.md`, `docs/{BAT-DAU-SESSION,XSS-4.6,HUONG-DAN-TEST-GIAO-DIEN}.md` | Phase 8 — deploy sync `web/` + `server/src/` + **chạy migration 016**; Ctrl+Shift+R. **Không** seed lại |

| 2026-09-04 | **Vòng 14续10 — 3 việc người dùng báo sau khi XEM BẢNG THẬT (đợt 1 tiếp).** (1) **Cột «Tên kết quả làm được» ở hàng chờ**: người dùng chốt «ghi kèm icon định dạng file và bao nhiêu bản, dòng 2 sẽ ghi tên file» ⇒ `buildIconDinhDang` (Word/Excel/PPT/PDF/Ảnh, đuôi lạ → `fa-file` xám + nhãn «—», có `title`+`aria-label` nên mất font vẫn đọc được chữ), dòng 1 = tên kết quả + «N bản», dòng 2 = **tên file bản mới nhất** — cần trường mới nên repo trả thêm `v.ten_goc AS ban_cuoi_ten` (đúng cửa §6: «trừ khi ảnh đòi trường mà REST chưa trả»). (2) **Menu ⋯ «bị vấn trong hộp, phải kéo chuột xuống mới thấy»**: KHÔNG sửa được bằng CSS — `.glass-card` và `.modal-content` vừa `overflow` cắt vừa có `backdrop-filter`, mà thẻ có `backdrop-filter` là KHỐI CHỨA của cả `position: fixed` (đổi sang fixed tại chỗ vẫn bị cắt) ⇒ `batTatMenuKq` **dời thẻ menu ra `<body>`** lúc mở, định vị `fixed` theo rect của nút ⋯ (thiếu chỗ dưới thì mở ngược lên, sát mép thì kéo vào, `maxHeight`+`overflowY` để menu dài vẫn cuộn được), `dongMenuKq` **trả về đúng chỗ cũ** — và `napKetQua`/`renderChoDuyetKetQua` gập menu TRƯỚC khi ghi `innerHTML`, không thì thẻ mồ côi nằm mãi ở `<body>`. (3) **Badge số trên tab hàng chờ «màu trắng cùng màu với nền»**: `#nav-cho-duyet-badge` mang `bg-rose-600 text-white` nhưng **bản Tailwind cắt sẵn 39 KB không có `.bg-rose-600`** ⇒ số trắng trên nền trắng; soát cả trang thấy 8 lớp đang dùng mà thiếu (`bg-rose-600/100`, `text-rose-600`, `bg-yellow-100`, `text-yellow-700`, `bg-green-800`, `text-[11px]`, `text-gray-300`) — badge «Chờ TP/PP xem» và «Đã duyệt» của CẢ HAI bảng kết quả cũng mất nền vì cùng lý do ⇒ khai bù đúng 8 lớp ở cuối `app.css` (không nhồi cả bảng màu). Test: TCKQ-31/32/33 mới (icon + 2 dòng + thoát HTML; menu dời ra `<body>` rồi trả về, bấm ngoài cũng gập; vẽ lại không để thẻ mồ côi), TC-HCPD-01 thêm `ban_cuoi_ten` ⇒ **1567 test / 87 file** xanh, lint + format sạch; pin XSS **101 chỗ / 868 → 873 giá trị** (+3 `buildIconDinhDang`, +2 dòng tên file; phần dời thẻ dùng DOM API nên không thêm lỗ nào). Banner + buster `20260904-2`. | `web/assets/js/app.js`, `web/assets/css/app.css`, `web/index.html`, `server/src/modules/taskFiles/repo.js`, `server/tests/unit/{task-files-ui,xss-guard}.test.js`, `server/tests/integration/task-files-api.test.js` | Phase 8 — deploy chỉ cần sync `web/` + `server/src/`, **không** migration |

| 2026-09-04 | **Vòng 14续9 — THIẾT KẾ LẠI hai bảng kết quả theo đúng file thiết kế của người dùng (đợt 1: HÌNH DÁNG, chưa migration).** Nguồn thiết kế: hai ảnh `docs/kq-modal.png` + `docs/kq-hang-cho.png` **tôi không đọc được** (đúng luật §6: dừng, nói thẳng, không bịa bố cục) ⇒ người dùng gửi `docs/moi.xlsx` hai sheet, đọc bằng `exceljs` nên **toàn bộ 16 tên cột + mọi câu chú là của người dùng, không phải tôi đoán**. (A) **Khối «Kết quả» trong modal nhiệm vụ** từ các thẻ rời thành **BẢNG 8 cột**: Thời gian · Kết quả làm được · Định dạng · File đã tải lên · Người thực hiện · Ghi ý kiến · Tình trạng · Hành động (`buildBangKetQua` + `buildKhoiFile` viết lại thành `<tr>`). Dòng cha đánh **1., 2., 3.**; mỗi BẢN là một dòng con **1.1, 1.2 …** kèm chữ «Sửa lần N» (`buildDongBanKetQua`) và **THU GỌN mặc định**, bấm ▸ mới bung (`batTatBanKq`, chọn dòng bằng `data-nhom` chứ **không** đếm vị trí DOM — giữa dòng cha và dòng bản còn dòng panel nên đếm theo thứ tự là sai ngay khi thêm dòng). Nút **＋ «Thêm kết quả»** đặt ngay trong khối (người dùng chốt «Trong khối Kết quả», KHÔNG sửa form tạo nhiệm vụ — §6 cấm). (B) **«Hàng chờ phê duyệt → Phê duyệt kết quả»** thành **BẢNG PHẲNG 8 cột**: Tên kết quả làm được · Nhiệm vụ · Công việc con · Công việc chính · Trạng thái · Bản mới nhất · Ý kiến · Nút chức năng. Ba cấp cây từ **hàng tiêu đề gộp** đổi thành **ba CỘT của chính dòng file** ⇒ **gỡ hẳn `buildHangCayChoDuyet`** (mắt quét theo hàng, không phải nhớ đang ở dưới nhóm nào); client vẫn **KHÔNG sắp lại**, chỉ vẽ theo thứ tự máy chủ. (C) **Mọi hành động gộp vào MỘT menu ⋯** (`buildMenuHanhDongKq`/`buildMucMenuKq`/`batTatMenuKq`) đúng câu chú «ấn vào đây hiển thị các Hành động để chọn» — trước đây mỗi hành động một nút nên hàng nút tràn ngang bảng; mở menu này thì **đóng mọi menu khác**, listener «bấm ra ngoài» gắn **một lần** (`daGoiDongMenuKq`) khỏi rò mỗi lượt vẽ. Luật verdict tách ra `dsVerdictFile()` làm **MỘT nguồn sự thật** cho cả hàng nút cũ lẫn menu mới. (D) **Hai cột suy ra được, KHÔNG cần trường mới nào của máy chủ**: «Định dạng» = `dinhDangCuaTen()` tra từ đuôi (12 đuôi → Word/PDF/Excel/PPT/Ảnh, đuôi lạ trả `—` chứ không đoán bừa); «Tình trạng» = **CÂU KỂ** đọc được (`cauTinhTrangFile`) dựng từ `trang_thai` + **đếm số lần trả lại** trong bảng luồng ⇒ «Bị trả lại lần 2 — đang đợi Cán bộ sửa và nộp bản mới», và chỉ nói «TP/PP đã duyệt, đang gửi lên Phó Giám đốc» khi luồng **thật sự** có `trinh-lanh-dao`. Trang hàng chờ phải có bản riêng `cauTinhTrangHangCho` vì `listChoDuyetKetQua` **không trả bảng luồng** — không đếm được lần trả lại thì đừng bịa số. **Đợt 2 (chưa làm, chờ người dùng xem đợt 1)**: migration 016 cho dòng «Chưa có» (kết quả khai TRƯỚC khi có file ⇒ cần `ten_ket_qua`/`dinh_dang`) và «Báo cáo» = nhập text (`task_file_versions.noi_dung` + CHECK «hoặc file hoặc text» để tái dùng nguyên bộ máy bản/ý kiến/luồng/verdict). Test: TCKQ-16/20/21/22 sửa theo hình mới (**bẫy: menu ⋯ dựng cả chuỗi `onclick` rồi cho qua `escapeHtmlAttr` nên `'` thành `&#39;` — assert phải khớp chuỗi ĐÃ THOÁT**, khác ô «Nhiệm vụ» dựng onclick trực tiếp), thêm **TCKQ-23..30** (8 cột đúng thứ tự, số 1./1.1 + «Sửa lần N», thu gọn/bung, `dinhDangCuaTen` 12 đuôi + 6 ca đuôi lạ, câu kể tình trạng, menu ⋯ đóng lẫn nhau, «Chưa có», escape ở cả dòng cha lẫn dòng bản) — **1556 → 1564 test / 87 file** xanh; pin XSS **101/831 → 101/868** (+37 giá trị, **số chỗ ghi HTML KHÔNG đổi** vì hai bảng vẫn ghi qua đúng hai lỗ `innerHTML` cũ — lý do từng hàm ghi trong TC-SEC-17); banner `app.js 20260904-1` + `app.css 20260904-1` | `web/assets/js/app.js`, `web/assets/css/app.css`, `web/index.html`, `server/tests/unit/{task-files-ui,xss-guard}.test.js`, `KE-HOACH-VPS.md`, `docs/BAT-DAU-SESSION.md` | Người dùng **xem đợt 1 trên máy thật** (Ctrl+Shift+R; **không** migration, **không** seed lại) rồi chốt đợt 2: migration 016 + dòng «Chưa có» + «Báo cáo» nhập text |

| 2026-09-05 | **Vòng 14续12 — sửa form nhiệm vụ theo phản hồi người dùng.** Lỗi client khiến ô «Cán bộ trực tiếp» bị khóa sau timer 100ms vì chỉ mở cho admin hoặc người đứng tên công việc; sửa điều kiện hiển thị cho admin/Phó Giám đốc trong phạm vi/Trưởng phòng/Phó phòng/người quản lý công việc, vẫn giữ `isEdit22` khóa cán bộ tự sửa và để máy chủ kiểm quyền cuối. Form tạo và sửa bỏ textarea legacy `resultLinks` («Nhập mỗi link trên một dòng»); mỗi kết quả nhập một dòng trong bảng 8 cột đã có. Đổi lưới nội dung thành `md:grid-cols-3`, cột thông tin `md:col-span-1`, cột Mục tiêu/Kết quả/Kết quả đầu ra/Ghi chú `md:col-span-2`; không dùng lớp Tailwind không vendored. Bump banner/buster `app.js 20260905-1`. Test timer TP/PP/admin/cán bộ tự sửa + form tạo/sửa, **1588 test / 87 file** xanh; pin XSS **102/892** | `web/assets/js/app.js`, `web/index.html`, `server/tests/unit/{task-form-candidate,task-files-ui,xss-guard}.test.js`, `docs/{XSS-4.6,BAT-DAU-SESSION,HUONG-DAN-TEST-GIAO-DIEN}.md` | Phase 8 — sync `web/`, Ctrl+Shift+R; không migration, không seed lại |

| 2026-09-06 | **Vòng 14续13 — 5 việc người dùng báo qua ảnh + phần bổ sung.** (1) «Xem chi tiết» ở hộp chờ duyệt hiện **rỗng** (chỉ có tiêu đề, thống kê và cây trống): `moChiTietCheDoDuyet` vẽ ngay từ ảnh bootstrap đang có (thiếu thì `refreshData()` trước), rồi nạp lại nền và **chỉ** vẽ lại khi vẫn đúng modal đó còn mở và `cheDoDuyetChiDoc` còn bật; `project-details.js` thêm khối «Thông tin công việc» (mô tả, ngày bắt đầu/kết thúc, trạng thái, trạng thái duyệt) sau 4 thẻ số. (2) Tạo xong công việc cha thì **mở lại đúng modal chi tiết cha** để tạo tiếp công việc con/nhiệm vụ — `taoDauViecVaNapLai` tự gọi `showProjectDetailsModal`. (3) Chân form TẠO: thanh **dính đáy** (`.chan-form-tao sticky bottom-0`) có «Hủy» + **đúng hai** nút ý định «Lưu tạm» (`data-nhap`) / «Gửi đi duyệt» (`data-gui-duyet`); **bỏ nút submit trần** ở thanh tiêu đề của chế độ TẠO — bấm nó là tạo bằng mặc định máy chủ mà người dùng không biết mình đã gửi duyệt hay chưa (chế độ SỬA vẫn giữ «Cập nhật»). «Chờ duyệt» hiện **ngay** không tải lại trang: chờ tạo → ghi dòng kết quả → `refreshData()` → mới gửi cây, rồi đọc trạng thái **từ dữ liệu đã nạp lại**; **bỏ hẳn** dòng tự gán `T_APPROVAL = "Chờ duyệt"` chỉ vì cha đang chờ (sai: cấp 3 mới tạo không đi cùng cha), `capNhatTrangThaiChoDuyetLocal` chỉ nâng con đang **Nháp/Từ chối**; `closeModal` **không đóng** khi form còn `data-dang-luu="1"`; lỗi giữa đường nói rõ «đã tạo, đừng tạo lại». (4) Dòng khai kết quả thẳng hàng: hàng nội dung «Báo cáo» tách khỏi 8 cột chính thành `<tr class="kq-tam-noi-dung-hang">` với `colspan=8`, xoá dòng thì xoá **cả cặp** và đánh số lại, luôn còn 1 dòng. (5) Menu ⋯ **nằm trên** modal: dời ra `<body>`, `position:fixed`, z-index **200** > 70 của modal (`.glass-card` có `overflow` + `backdrop-filter` nên là khối chứa cả với `position:fixed`). Test: **1596 test / 87 file xanh** (trước vòng này 1588; 8 test mới — TCKQ-37 sửa theo cấu trúc mới, TCKQ-37b xoá cặp + đánh số, TCKQ-32b **bấm thật** trên modal, 3 test chân form ở `task-form-candidate`, 1 test «gửi công việc con không hạ nhiệm vụ đã duyệt», 2 test chi tiết authoritative); `npm run lint` + `format:check` sạch, `node --check` sạch. Hai bẫy nhỏ của chính vòng này: eslint `require-await` bắt `async () => false` trong test giả `refreshData` (đổi sang `() => Promise.resolve(false)`), và bọc `escapeHtml` quanh hằng chuỗi làm pin XSS phình 893 → 896. Pin XSS **giữ 102/893** — trừ một lỗ (`resultLinks`) thêm một lỗ (`buildLuuNhapNutHtml`), đối chiếu từng lỗ bằng `tools/dem-xss.mjs`; nhãn TĨNH của chân form **không** bọc `escapeHtml` (bọc hằng chuỗi chỉ làm phình con số chốt — bẫy mới ở §13.5). Buster `app.js 20260905-2`, `app.css`/`project-details.js` `20260905-1`. Còn lại: **chưa test tay** trên trình duyệt thật (mục **9b.11** của `docs/HUONG-DAN-TEST-GIAO-DIEN.md`) | `web/assets/js/app.js`, `web/assets/js/project-details.js`, `web/assets/css/app.css`, `web/index.html`, `server/tests/unit/{task-files-ui,approvals-ui,project-details-phan-cong,task-form-candidate,xss-guard}.test.js`, `KE-HOACH-VPS.md`, `docs/XSS-4.6.md` | Test tay mục **9b.11**; rồi làm **THÔNG BÁO** — chuông trong ứng dụng (§13.4 mục 16) + đẩy sang **Zalo** (mục 25 mới) |

| 2026-09-06 | **THÔNG BÁO việc A — chuông đọc được, §13.4 mục 16 hết treo sau 10 ngày.** Máy chủ đã ghi thông báo từ Phase 5 (gửi duyệt → Phó GĐ phụ trách, duyệt, từ chối, ủy quyền, kết quả file, cron quá hạn) nhưng **không có một đường REST nào để ĐỌC** ⇒ toàn bộ nằm im trong bảng, không ai thấy. Nay: **service** thêm `docCuaToi` / `demChuaDoc` / `danhDauDaDoc` với cổng quyền **riêng** `assertDangNhap` — tách khỏi `assertAdmin` của đường ghi vì hai câu hỏi khác nhau («được phát thông báo cho người khác không» ≠ «xem thông báo của chính mình»), gộp lại là hoặc khoá chuông của mọi người không phải admin, hoặc mở cho ai cũng phát tin hàng loạt. **KHÔNG nhận `userId` từ query, kể cả admin**: thông báo chứa tên đầu việc và lý do từ chối của người khác. **routes** thêm `GET /`, `GET /unread-count`, `PATCH /read`; `limit` rác không trả 400 mà để repo kẹp về `min(200, …)` — tham số hiển thị sai một chữ không đáng làm hộp chuông trắng; **không** ghi `activity_logs` cho lần đọc (60 giây/người sẽ nhấn chìm bảng, cùng lý lẽ `chat/routes.js`). **bootstrap** trả thêm `unreadCount` (cạnh `pendingCount`) và cầu RPC chuyển tiếp ⇒ badge đúng **ngay lần vẽ đầu**. **Giao diện**: nút 🔔 + hộp Alpine cạnh nút chat trong `index.html`, `buildMotThongBao`/`buildDanhSachThongBao` (tên `build*` là **bắt buộc** — bộ soát XSS xếp hàm khác tên thành `CAN-THOAT`), icon+màu theo cột `type`, mốc giờ «14:05 hôm nay / hôm qua / dd/mm/yyyy»; bấm một dòng thì đánh dấu đã đọc **rồi mở đúng mục** — `ref_id` là **id CSDL** còn giao diện tra theo **mã**, nên dò lại trong `allProjects`/`allTasks`, dò không ra thì báo «tải lại trang» chứ **không bịa mã**; vòng hỏi lại **60 giây** (chat 10 giây, nhưng thông báo phê duyệt là việc thưa và mỗi lượt là một `count(*)`) và **chỉ khi `document.visibilityState === "visible"`** — chat đang bỏ sót điều này nên một tab để quên vẫn nã request suốt ngày; `napLaiSauDuyet` gọi lại badge ngay sau khi ký, không đợi hết nhịp. Ba đường REST **không có tên RPC** (giao diện cũ chưa từng vẽ danh sách này) ⇒ gọi thẳng bằng `restGetIm`/`restGhi` như phần ủy quyền, cầu vẫn **37/37**; §5.2 đã ghi bảng «REST không có tên RPC» **trước** khi viết mã, đúng điều kiện mục 16 đặt ra. Test: `thong-bao-ui.test.js` mới **18 test** (TC-TBUI-01..06: `<img onerror>` trong nội dung thành chữ, `type` chứa nháy không sinh `onload`, `ref_id` chứa nháy không phá `data-*`, badge 0 ẩn hẳn / >99 thành «99+», mạng lỗi **không** xoá danh sách đang có, «đọc hết» gửi `ids: []` rồi nạp lại chứ không tự trừ số, bấm dòng mở đúng mã, tab ẩn **không** gọi mạng) + `notifications-api.test.js` **23 → 34** (Nhân viên không thấy của người khác; **admin cũng không đọc hộ được**; `onlyUnread` lọc danh sách nhưng `unread` vẫn là tổng thật; `ids` của người khác ⇒ `changed = 0`; `bootstrap.unreadCount`) ⇒ **1625 test / 88 file** xanh, `npm run lint` + `format:check` sạch. Pin XSS **102/893 → 104/900**: 2 sink (`innerHTML = buildDanhSachThongBao(…)` và một chuỗi hằng «Chưa tải được thông báo»), 7 lỗ mới **tất cả DA-THOAT** trong `buildMotThongBao` — `row.content` là dữ liệu người dùng đi qua **hai** tầng (máy chủ dựng câu từ tên đầu việc rồi lưu **nguyên văn**), đúng loại đường dễ bỏ escape nhất. Banner + buster `app.js 20260906-1`. **Việc B (Zalo) CHƯA làm**: đã tra tài liệu và chốt dùng **Zalo Bot API** (`bot-api.zaloplatforms.com/bot<token>/sendMessage`, token từ OA «Zalo Bot Manager» → Zalo Bot Creator, webhook xác thực bằng `X-Bot-Api-Secret-Token`), **loại bỏ** OA API/ZNS vì tin Tư vấn đòi người nhận tương tác với OA **trong 7 ngày** — thông báo phê duyệt thưa nên phần lớn sẽ không đến được; **loại bỏ** `node-zalo-bot@0.1.6` (phát hành 2025-07-22, không license/repo, kéo `@cypress/request`) — Node 24 có `fetch`, viết ~60 dòng là đủ. Bốn câu chặn ghi ở §13.4 **mục 25** | `server/src/modules/notifications/{routes,service}.js`, `server/src/modules/bootstrap/service.js`, `server/src/rpc/table.js`, `web/index.html`, `web/assets/js/app.js`, `server/tests/unit/thong-bao-ui.test.js` (mới), `server/tests/unit/xss-guard.test.js`, `server/tests/integration/notifications-api.test.js`, `server/eslint.config.js`, `docs/KE-HOACH-THONG-BAO.md` (mới), `docs/{XSS-4.6,HUONG-DAN-TEST-GIAO-DIEN}.md`, `KE-HOACH-VPS.md` §5.2 + §13 | Người dùng trả lời §13.4 **mục 25** (4 câu về Zalo) ⇒ làm **việc B**: migration **017** (`users.zalo_chat_id` + `zalo_link_codes` + 3 cột hàng đợi trên `notifications`), `services/zalo.js`, lịch `dayThongBaoZalo` 2 phút, webhook liên kết. Test tay mục **9b.11** và **9b.12**. **Không** migration nào ở vòng này |
| 2026-09-06 | **PHASE 8 việc 1–4 + 7 — hạ tầng VPS SỐNG: `https://ttdt.site` chạy thật.** **Việc 1** (Zalo): test cho mã việc B đã có ở dòng trên — **43 test TC-ZL-01..14 xanh, suite 1668/1668**, pin XSS **106/903** (2 sink mới của Zalo đều `build*` DA-THOAT); token cũ LỘ trong chat nên chạy thật bị chặn đến khi XOAY token. **Việc 2**: `Dockerfile` 2 tầng `node:24-alpine` (@node-rs/bcrypt native nên KHÔNG dùng image debian), `.dockerignore`, `deploy/docker-compose.yml` — db **không publish port**, app buộc `127.0.0.1:3000`, OnlyOffice buộc JWT bằng guard `:?`, volume `server/storage` (chown 1000:1000) ngoài ảnh; chuyển `node-pg-migrate` sang `dependencies` để migrate được TRONG container; smoke đủ trên máy dev (build, up, migrate, healthz/readyz, SIGTERM lịch sự). **Việc 3**: VPS thật — swap 4 GB, Docker 29.8, `/opt/qlcv`, `deploy/.env` sinh bí mật bằng lệnh máy chủ (SESSION 64 ký tự, PG 48, OO-JWT 64 — không qua chat), 17 migration, app healthy. **Việc 4**: nginx host `ttdt.site` — 80→443 + www→apex, cert có sẵn KHÔNG xin lại, HSTS **đúng một lần** (helmet cho đường proxy + snippet cho tĩnh — bẫy cộng dồn `add_header`), tài sản 30 ngày, `index.html` no-store, `client_max_body_size 60m`; ba lần sửa ngay trên VPS: cú pháp `http2` của nginx 1.24, đưa HSTS vào snippet, bỏ HSTS trùng trên đường proxy. **Việc 5** (phần cấu hình): `office.ttdt.site.conf` + `websocket-map.conf` commit sẵn, chờ DNS để bật. **Việc 7**: UFW giữ 22/80/443, fail2ban sshd (chặn ngay 5 IP đang dò mật khẩu), SSH chỉ khoá (`49-qlcv-hardening.conf` thắng `50-cloud-init` vì sshd ăn chỉ thị ĐẦU TIÊN), `.env` chmod 600; `backup.sh` pg_dump -Fc + tar storage giữ 14 bản, cron 02:00; **drill khôi phục THẬT: chèn dòng đánh dấu → sao lưu → xoá → `restore.sh` ~2 giây → dòng quay về, `readyz` 200**; `deploy/runbook.md`: build/deploy/rollback/restore + 5 sự cố + lệnh swap/cron/storage cho VPS mới. Lỗi hút trong phiên: SSH từ máy dev hay đứt giữa chừng ⇒ việc dài chạy `nohup` trên VPS rồi đọc log | `Dockerfile`, `.dockerignore`, `deploy/{docker-compose.yml,.env.example,backup.sh,restore.sh,runbook.md}`, `deploy/nginx/{ttdt.site.conf,security-headers.conf,office.ttdt.site.conf,websocket-map.conf}`, `server/package.json` (+lock), `server/tests/unit/zalo*.test.js` | Người dùng: (1) DNS A `office.ttdt.site` → 157.10.199.148 ⇒ certbot + bật office conf + e2e `.docx` (việc 5); (2) XOAY token Zalo, tự dán vào `deploy/.env` trên VPS ⇒ `zalo:kiem` + `zalo:webhook` + LIENKET thật + đẩy duyệt thật (việc 6). Sau đó chốt Phase 8, viết prompt Phase 9 (mục 3 của `docs/BAT-DAU-SESSION.md`) |
| 2026-09-06 | **PHASE 8 việc 5 + 6 — hạ tầng xong, phần «người thật» chuyển bước 0 Phase 9.** **Việc 5**: người dùng trỏ DNS A `office.ttdt.site` → certbot cert RIÊNG (không chung cert apex), bật `office.ttdt.site.conf` + `websocket-map.conf` vào `/etc/nginx/` (khối HTTP tạm cho certbot trước, thay bằng conf đầy đủ sau), kiểm thật: HTTP→HTTPS 301, HSTS, `/healthcheck` = `true`, `api.js` 200 qua HTTP/2. **Việc 6**: người dùng XOAY Bot Token, tự dán `.env` ⇒ `zalo:kiem` hợp lệ (bot «Trung tâm Đào tạo»); `zalo:webhook set` + `thu`: Zalo tự gọi 200, sai chữ ký → **403**, lịch đẩy `*/2 * * * *` bật. **Vá bảo mật giữa chừng**: `docker logs` in TRẦN header `x-bot-api-secret-token` của mọi request (pino-http log nguyên headers, `redact` cũ chỉ che cookie/authorization) ⇒ secret webhook lọt log ⇒ vá `logger.js` (thêm header đó + `currentPassword`/mật khẩu lồng nhau) + test chốt `logger-redact.test.js` (`2aeebd3`, **1670/1670**), và **XOAY secret lần hai** sinh bằng `openssl` NGAY trên VPS (không qua chat), đăng ký lại webhook, xác nhận log mới in `[đã che]`. **Chặn cuối phát hiện**: mọi e2e «người thật» (sửa `.docx`, `LIENKET`, đẩy duyệt) cần tài khoản — production **0 tài khoản** (đúng thiết kế, 9.1/9.1b mới tạo), mà UI không tạo được tài khoản ĐẦU (gà-con: tạo tài khoản cần admin đăng nhập) ⇒ ghi vào **bước 0 của prompt Phase 9**: admin đầu tạo thẳng bằng `psql`, hash bcrypt sinh TRONG container | `server/src/utils/logger.js`, `server/tests/unit/logger-redact.test.js`, `docs/BAT-DAU-SESSION.md` (mục 1 + mục 3 = prompt Phase 9) | Session kế: dán prompt Phase 9 (mục 3 `docs/BAT-DAU-SESSION.md`) — bước 0 tạo admin đầu rồi e2e `.docx` + `LIENKET` + đẩy duyệt, gộp `vps/phase-8-hatang` vào `main`, rồi 9.1–9.7 |
| 2026-09-06 | **Bước 0 Phase 9 — admin đầu XONG; người dùng XOAY sang Phase 8b sửa lỗi vặt.** Bước 0a: tạo admin đầu KHÔNG qua UI — mật khẩu gõ ẩn trên chính SSH của người dùng (`read -s < /dev/tty`), hash bcrypt sinh TRONG container `qlcv-app` (`node --input-type=module` + `hashSync` của @node-rs/bcrypt, cost theo `BCRYPT_COST`), INSERT qua `docker exec -i qlcv-db psql -v hash=…` với heredoc `<<'SQL'` dùng biến `:'hash'` (tránh shell diễn giải chuỗi `$2y$12$…`). Kết quả: tài khoản `TTDT001` — Trương Thanh Hà, role `admin`, `must_change_password = true` (đăng nhập bắt đổi mật khẩu); không mảnh bí mật nào qua chat. E2e OnlyOffice `.docx` + Zalo `LIENKET`/đẩy duyệt **tạm gác** — người dùng CHUYỂN HƯỚNG giữa chừng («cho tôi prompt để sửa các lỗi vặt đã») ⇒ mở **Phase 8b** (dòng 8b §13.2): sửa trên PC → test xanh → push `vps/sua-loi-vat` → deploy VPS theo runbook. Lỗi 1 đã chốt trong prompt: menu «+ Tạo mới» trang «Quản lý tài khoản» kéo dài khung đầu trang thay vì phủ xuống — đọc tĩnh chưa kết luận nguyên nhân (tailwind.min.css đủ `.absolute`/`.relative`; `.glass-card` có `backdrop-filter`, không position), khoanh vùng `web/index.html` ~386–417 + «Xuất Excel» ~361–384, vào session dùng DevTools tìm. Prompt đầy đủ ở **mục 3b** `docs/BAT-DAU-SESSION.md` | `docs/BAT-DAU-SESSION.md` (mục 3 dòng «Phase kế tiếp» + mục 3b mới), `KE-HOACH-VPS.md` §13.2 + §13.3 | Session kế: dán prompt **mục 3b** — nhánh `vps/sua-loi-vat` từ 81d9fb9, sửa lỗi 1 + lỗi người dùng bổ sung, test xanh rồi mới lên VPS |
| 2026-09-06→07 | **PHASE 8b — lỗi 1 + 2 XONG, đã deploy VPS.** **Lỗi 1** (`5f0bb21`): 4 menu đầu trang (Xuất Excel, + Tạo mới, chuông, Chat) kéo dài khung khi mở — `.glass-card` khai `position: relative` (cần cho đường viền `::before`) và app.css nạp SAU tailwind nên cùng độ ưu tiên (0,1,0) mà nó THẮNG `.absolute`; sửa `.glass-card.absolute { position: absolute }` (0,2,0), giữ nguyên `relative` của `.glass-card` thuần; test hồi quy `menu-tha-xuong.test.js`; buster `app.css?v=20260906-1`. **Lỗi 2** — người dùng chốt 4 quyết định qua câu hỏi: (1) mục KHÔNG có file kết quả = 0%; (2) thêm/xoá mục ⇒ tỷ lệ TỰ CO GIÃI giữ chỉnh tay (số nguyên đúng 100, largest-remainder); (3) phạm vi MỌI NƠI; (4) quyền sửa tỷ lệ thêm vào BẢNG PHÂN QUYỀN cho admin/Phó GĐ/TP/PP phụ trách (QLCV/Nhân viên không có). **Máy chủ** (`ca4c6fa`): migration **018** (cột `ty_le` + 2 CHECK, chia đều dữ liệu cũ, dựng lại 2 view `v_countable_*` để không đóng băng cột), `workItems/tyLe.js` + `tienDo.js` (luật thuần, không CSDL), đường `ty-le` + tái cân bằng trong create/update/remove/copy, RBAC `ACTION_TY_LE` riêng (KHÔNG vào mảng ACTIONS — TC-RBAC-01 vẫn ghim 4 hành động), `HANH_DONG_DUOC_SUA` nhận `'ty-le'` còn `coChoDuyet` thì không (bảng phân quyền không sinh ô ⏳ cho ty-le), E4+E3+Gantt+bootstrap gắn tiến độ tại nguồn chung, `stats/repo.js` select `ty_le` + ORDER BY đúng thứ tự bootstrap, `legacyFields.js`: cột «Tiến độ (%)» đọc từ `tien_do`, thêm cột «Tỷ lệ công việc (%)» (chuỗi phải KHỚP col-parity). **Web** (`17d74e5`): form bỏ ô «Tiến độ (%)» + listener «chọn Hoàn thành tự điền 100%»; thêm ô «Tỷ lệ công việc (%)» CHỈ cho đầu mục (cấp 2, hoặc cấp 3 không `T_PARENT`) — vai có quyền nhập được (tạo mới để trống ⇒ server chia đều), vai khác thấy ô KHOÁ và **KHÔNG name** ⇒ FormData không bao giờ gửi `tyLe` trái quyền — né **403 cả bản ghi** ở `service.js` (chỗ nguy hiểm nhất của lỗi này); optimistic update hết tự điền completion; `tienDoDauMucKhach()` gia quyền khớp `tienDoWork` cho fallback E4/E3 + thẻ dashboard + modal dự án (`project-details.js`); bảng phân quyền +2 hàng ty-le (subwork/task). **Test 1684 → 1696 / 96 file** xanh: +10 unit ty-le-tien-do, 5 integration theo luật mới, TC-TKPQ-06/14 ghim 15→17 hàng + fixture MAC_DINH + TC-TKPQ-18, và `ty-le-form.test.js` **+11 test** jsdom chạy app.js THẬT (thay bước «test tay trên PC» của workflow: completion biến khỏi form, ô tyLe theo vai/loại mục, toán gia quyền 60/40→70, rounding 33/67→33, danh sách rỗng→0). **Deploy VPS 2026-09-07**: backup 00:38 → checkout `vps/sua-loi-vat` @17d74e5 → `docker compose build app` → migrate 018 (pgmigrations xác nhận, cột `ty_le` có) → `readyz` `{"ok":true}`, 3 container healthy, trang sống phát buster `app.js?v=20260906-2`. **Bẫy mới phát hiện**: fetch refspec của clone VPS chỉ ghim `vps/phase-8-hatang` (từ lần thiết lập đầu) ⇒ nhánh mới TÀNG HÌNH với `git fetch` dù `ls-remote` thấy — đã đặt lại refspec mặc định `+refs/heads/*:refs/remotes/origin/*` để luồng runbook chạy đúng mọi nhánh. Còn phải **mắt kiểm trên trình duyệt thật**: form nhập tỷ lệ với từng vai, thanh Gantt và 2 biểu đồ E4/E3 với dữ liệu thật | `web/assets/css/app.css`, `web/assets/js/app.js`, `web/assets/js/project-details.js`, `web/index.html`, `server/src/db/migrations/018_ty_le_tien_do.sql`, `server/src/modules/workItems/{tyLe,tienDo,repo,routes,service}.js`, `server/src/middleware/rbac.js`, `server/src/modules/permissions/service.js`, `server/src/modules/{bootstrap,stats,gantt,works}` (gắn tien_do tại nguồn), `server/src/rpc/legacyFields.js`, `server/tests/unit/{menu-tha-xuong,ty-le-tien-do,ty-le-form,tai-khoan-ui}.test.js` + 5 file integration sửa theo luật mới | Người dùng: (1) báo lỗi vặt tiếp — vẫn quy trình PC → test xanh → push + deploy; (2) mắt kiểm form tỷ lệ/Gantt/biểu đồ trên trình duyệt thật; (3) khi DANH SÁCH LỖI ĐÓNG: gộp `vps/sua-loi-vat` vào `main`, rồi Phase 9 bước 0 đã xong (admin đầu `TTDT001`) → e2e `.docx` + Zalo `LIENKET` + đẩy duyệt người thật |

| 2026-09-07 | **Phase 8b — sửa local PC, CHỜ người dùng test và nói OK.** Quản lý công việc thấy công việc/cây chưa phân công cùng phòng theo `currentUser.department_id` ↔ `COL.P_DEPT_ID`, không phụ thuộc ngữ cảnh phòng bất đồng bộ, không sửa quyền ghi/server. Dữ liệu tháng 10 vẫn giữ nguyên ngày; danh sách rỗng do lọc hướng dẫn chọn «Tất cả tháng». Footer chỉ ở chi tiết cha Nháp, ngoài vùng cuộn và không ở chế độ duyệt: Lưu tạm chỉ đóng dữ liệu đã lưu, Gửi đi phê duyệt dùng REST submit cả cây, chặn nhấp đôi, lỗi bật lại nút, thành công mới cập nhật/nạp lại. Banner + 2 JS buster **20260907-1**, CSS giữ **20260906-1**. Launcher mặc định giữ UAT, không tự seed kể cả DB mới, không taskkill/remove container; chặn cổng 3000 bận trước migrate, ép development/cookie HTTP/tắt cron Zalo, kế thừa env không đưa mật khẩu vào command line, lỗi compose/migrate/health/ready/asset trả thất bại. Helper Node chỉ đọc so banner/buster và byte tài sản qua nginx. **Kiểm tra:** 44 test scoped 2 file + file tasks xanh; full lượt đầu exit 1 (1671 pass, 12 fail, 18 skip/1701, 94/96 file), lỗi thiếu relations sessions/v_countable_items trong lúc có Vitest khác dùng chung global setup. **Lượt full độc lập: 1701/1701 pass, 96/96 file, exit 0** (397 giây), không chạy Vitest khác song song. Lượt đầu đỏ do va chạm global setup với lượt focused; không phải bằng chứng backend cần sửa. Scoped eslint/prettier và node --check xanh; lint toàn repo exit 1 **20 lỗi** (ty-le-form globals và ty-le-tien-do unused), format exit 1 **8 file ngoài scope**, không sửa lan. **Stack chưa nghiệm thu**: cổng 3000 không listen, kiểm 8099 /healthz trả 502 exit 1; DB/nginx/socat Up không chứng minh tất cả healthy. Chưa chạy launcher toàn luồng vì sẽ migrate UAT/bật Node, để người dùng chạy giữ dữ liệu; không seed/reset/kill tiến trình. | `chay-test.bat`, `tools/local-assets-check.mjs`, `web/assets/js/{app,project-details}.js`, `web/index.html`, `server/tests/unit/{pho-giam-doc-ui,tasks-nhiem-vu-ui,project-details-phan-cong}.test.js`, `docs/BAT-DAU-SESSION.md`, `KE-HOACH-VPS.md` | Chạy `chay-test.bat /giu`, mở http://127.0.0.1:8099, Ctrl+Shift+R; kiểm vai Quản lý công việc/cùng phòng ngay lần đầu + tháng 10/Tất cả tháng + footer nháp/gửi duyệt + giữ draft khi lỗi. **CHỜ người dùng test PC và OK trước GitHub/VPS; chưa commit/push/deploy, không ghi VPS; giữ nguyên D chay.bat của người dùng.** |

| 2026-09-07 | **PC — sửa sáu biểu đồ 500, không triển khai**: xác nhận UAT có migration 018 và bảng có `ty_le` nhưng view thiếu. Thêm 019 `CREATE OR REPLACE VIEW`, giữ nguyên lọc Nháp/Chờ duyệt và dữ liệu, Down không tái tạo lỗi. Hồi quy dựng view cũ trong transaction, bắt SQLSTATE 42703, sửa hai lần vẫn đúng, kiểm tỷ lệ 73/đủ cột/lọc cả ba cấp và rollback không rò fixture. Focused **34/34**, full chạy sau focused **1703/1703 trong 96 file, exit 0**; scoped eslint/prettier sạch. Áp dụng 019 vào local UAT: số dòng và fingerprint giữ nguyên trên works=1, work_items=6, permission_overrides=0, task_files=4, task_file_versions=6. Hai QUERIES thống kê chạy được; đăng nhập test hợp lệ qua 8099, **6/6 loại biểu đồ HTTP 200**, Gantt 200 (lần thử đầu truyền bốn tên type không hợp lệ trả 400; đã kiểm lại đúng sáu tên hợp đồng). Launcher smoke chỉ đọc `LIMIT 0` sau migrate, mã chạy trực tiếp exit 0; `/giu /f` dừng an toàn exit 1 vì cổng 3000 bận, không giết server. Bọc smoke riêng bằng cmd.exe exit 1 nên chưa xác nhận toàn luồng launcher; không tuyên bố test tay giao diện đã xong. Không seed/reset UAT, không commit/push/deploy/VPS, giữ các sửa local và việc xoá chay.bat; buster giữ 20260907-1. | `server/src/db/migrations/019_refresh_countable_views.sql`, `server/tests/integration/countable-views.test.js`, `chay-test.bat`, `docs/BAT-DAU-SESSION.md`, `KE-HOACH-VPS.md` | Người dùng tải lại Tổng quan, kiểm sáu request 200 và Gantt; nếu cần tự đóng cửa sổ Node cũ rồi chạy /giu. CHỜ test PC và OK. |

| 2026-09-07 | **Kiểm bổ sung launcher 019**: chạy nguyên lệnh smoke qua stdin của `cmd.exe` thành công **exit 0**, có thông báo Schema thong ke OK. Lần `/c` ở dòng trên sai quoting của phép gọi kiểm, không phải lỗi câu lệnh .bat. Vẫn chưa chạy trọn launcher vì cổng 3000 của người dùng đang bận. | `docs/BAT-DAU-SESSION.md`, `KE-HOACH-VPS.md` | Giữ server người dùng; chờ nghiệm thu PC. |

| 2026-09-07 | **Phase 8b — người dùng NGHIỆM THU PC («Đã test PC và OK») và chọn «Push GitHub + deploy VPS»: ĐÃ PHÁT HÀNH.** Bốn commit trên `vps/sua-loi-vat`: `b531f38` (giao-dien: quyền đọc cùng phòng + footer bản nháp + buster `20260907-1` + 3 test), `71150c5` (may-chu: migration **019** + hồi quy view cũ thiếu `ty_le`), `2bebb8c` (may-chu: launcher giữ UAT/chặn cổng bận/smoke schema + `tools/local-assets-check.mjs`, ghi nhận việc người dùng xoá `chay.bat`), `c0858f4` (tai-lieu). Push `30e41b4..c0858f4`. VPS `/opt/qlcv` đúng §1 runbook: **backup TRƯỚC** (`deploy/backup.sh` exit 0, log ghi «xong 22:53», `qlcv-2026-09-07.dump` 112 KB) → `git fetch` → xem trước 5 commit sẽ kéo → `git pull --ff-only` về đúng `c0858f4` → `docker compose build app` exit 0 → `up -d` (`qlcv-app` Recreated/Started) → `run --rm app npm run migrate:up` exit 0, `pgmigrations` mới nhất `019_refresh_countable_views`. **Kiểm chứng sau deploy:** `/readyz` = `{"ok":true,"db":"up"}`; `qlcv-app` + `qlcv-db` **healthy**, `qlcv-onlyoffice` chỉ **Up** (không healthcheck — không suy ra healthy); smoke chạy đúng hai `QUERIES` của `stats/repo.js` với `LIMIT 0` trong container → «Schema thong ke OK»; **dữ liệu giữ nguyên** works/work_items/permission_overrides/task_files/task_file_versions = **4/2/0/2/1** trước và sau, `sum(ty_le)=100` cả trên bảng lẫn đọc qua hai view; trang sống phát `app.js?v=20260907-1`, `project-details.js?v=20260907-1`, `app.css?v=20260906-1`; sáu `/api/v1/stats/charts` trả **401** khi không có phiên (không còn 500 ở tầng route — **chưa** kiểm 200 kèm phiên, vì không đăng nhập bằng tài khoản thật trên production); log app 3 phút đầu không có error/fatal. Trước deploy VPS **không** dính lỗi view cũ (018 đã `DROP`+`CREATE` nên `ty_le` có sẵn), do đó 019 chỉ thay định nghĩa y hệt. **Parity PC ↔ GitHub ↔ VPS:** cùng `c0858f4`; `app.js` nginx phát **trùng sha256** file PC; `project-details.js` file PC lớn hơn 470 byte **chỉ do CRLF** của `core.autocrlf=true` — blob đã commit trùng khớp từng byte bản nginx phát, `git diff HEAD` rỗng. Trên VPS còn untracked `deploy/.env.save`: giữ nguyên, không đọc, không commit. Lint toàn repo **20 lỗi** + format **8 file ngoài scope** vẫn chưa sửa, không tuyên bố sạch; launcher vẫn chưa chạy trọn luồng vì cổng 3000 của người dùng bận. | `server/src/db/migrations/019_refresh_countable_views.sql`, `server/tests/integration/countable-views.test.js`, `server/tests/unit/{pho-giam-doc-ui,project-details-phan-cong,tasks-nhiem-vu-ui}.test.js`, `web/assets/js/{app,project-details}.js`, `web/index.html`, `chay-test.bat`, `tools/local-assets-check.mjs`, `KE-HOACH-VPS.md`, `docs/BAT-DAU-SESSION.md` | Người dùng Ctrl+Shift+R `https://ttdt.site`: `vanhiepttdt@gmail.com` thấy công việc/nhiệm vụ cùng phòng ngay lần vẽ đầu; Tổng quan sáu biểu đồ + Gantt vẽ được (không còn 500); tạo công việc cha → Lưu tạm → footer ngoài có «Lưu tạm»/«Gửi đi phê duyệt». Có sự cố thì ROLLBACK §2 runbook: bản chạy tốt trước đó là `17d74e5`, CSDL đã có backup 22:53. |

| 2026-09-08 | **Phase 8b — lệnh sửa + ý kiến/gửi OnlyOffice + tab vàng + chuông: mã xong PC, CHỜ người dùng test PC và OK mới.** Nhánh đang có `vps/sua-loi-vat`, HEAD `54c2759`; không cắt lại từ `81d9fb9`. Migration **020** thêm ba cột lệnh, backfill `can-sua` từ flow, CHECK `huy-lenh-sua` và down an toàn có test. Bốn REST chỉ chủ lệnh, phạm vi/quyền giữ nguyên; khóa nhóm khi ghi, gửi dùng bản mới nhất + `apTuDong`, hủy giữ file/bản và báo người ra lệnh, lưu tạm không chuyển trạng thái. PGD `tra-ve-tp` → `can-sua`/`lanh-dao`, TP có thể trả tiếp Cán bộ. OnlyOffice ý kiến/confirm gửi riêng; dirty phải đợi callback lưu COMMIT qua mã tương quan, thất bại không gửi bản cũ; Ctrl+S/poll báo «chưa gửi đi», callback giữ `{"error":0}` và không gửi duyệt. Tab vàng dùng template/textContent/value, NV thấy nav và chỉ tab vàng; TP/PP có ba tab, PGD/admin không lệnh không mở tab vàng; chuông `task_file` mở đúng hàng chờ. Không sửa Zalo allowlist/template/lịch, không xử lý vai NV003. **Kiểm chứng cuối:** `npm test` từ server, chạy độc lập sau sửa cuối → **1735/1735 test, 97/97 file, exit0**, 162.35 giây; scoped ESLint/Prettier máy chủ/tests sạch, `node --check` app.js và `git diff --check` sạch, XSS **106/903**. Không tuyên bố lint/format toàn repo sạch. Buster app.js **20260907-2**, project-details **20260907-1**, CSS **20260906-1**. **PC:** UAT019→020, fingerprint nghiệp vụ cũ giữ nguyên; dev017→018+019+020 có backfill tỷ lệ018 nên fingerprint dev thay đổi; cả hai giữ số dòng **1/6/0/4/6**, không seed/reset/kill server. 8099 health200, OnlyOffice local healthcheck true; browser NV thấy badge1/tab vàng/4 nút, Lưu tạm giữ sau tải lại, đã phục hồi ghi chú rỗng ban đầu. Chưa e2e DOCX thật/gửi-hủy thật/Zalo, chưa OK người dùng. | `server/src/db/migrations/020_task_file_lenh_sua.sql`, `server/src/modules/taskFiles/{repo,service,routes,dsRoutes}.js`, `server/tests/integration/task-files-api.test.js`, `server/tests/unit/{task-files-editor,task-files-ui,nav-cho-duyet,thong-bao-ui}.test.js`, `web/assets/js/app.js`, `web/index.html`, `docs/{KE-HOACH-KET-QUA-FILE,HUONG-DAN-TEST-GIAO-DIEN,BAT-DAU-SESSION}.md`, `KE-HOACH-VPS.md` | Người dùng mở PC8099, Ctrl+Shift+R, test mục **9b.13**, đặc biệt DOCX thật/Ctrl+S/Lưu/Gửi/Không/Hủy/TP-PGD/chuông. **Chưa commit/push/deploy, không đụng VPS. OK cũ không áp dụng.** Sau OK mới commit tách với explicit paths, push và deploy backup/build/migrate020 theo runbook. |

| 2026-09-08 | **Phase8b — ĐÃ NGHIỆM THU PC VÀ PHÁT HÀNH; reset database VPS theo yêu cầu rõ của người dùng.** Người dùng: «đã test trên pc, thấy ok; xóa data trên database, giữ tài khoản admin, liên kết zalo, restart lại docker». Commit explicit paths + Co-Authored-By đúng yêu cầu: `63f05b0` máy chủ/API020/OnlyOffice/test, `4cc8bba` giao diện/tab/chuông/test; push `54c2759..4cc8bba`, VPS ff-only đúng mã. PC full gần nhất **1735/1735, 97 file**, không đổi code sau lượt xanh, scoped checks sạch, XSS106/903. Sao lưu theo runbook và giữ riêng **`/var/backups/qlcv/release-20260908-140701/`** (700, dump600): `pre-deploy.dump`, `pre-reset.dump` **112569 bytes**, storage **73197 bytes**, cấu hình chỉ lưu dấu kiểm tra nội bộ. Trước xóa đã dừng app/OnlyOffice, dump lại. **Bản cũ có 4 works.created_by mồ côi**: restore chuẩn gặp FK lỗi; thử khôi phục vào DB tạm bằng `restore-original.list`, tạo riêng FK đó NOT VALID trên DB tạm → đối chiếu fingerprint **đủ 21 bảng trùng bản nguồn**, không sửa source. DB tạm đã xóa. Build/migrate lần đầu gặp EACCES vì umask077 của bước backup ảnh hưởng file Git mới; khôi phục quyền0644 chỉ các file tracked vừa kéo, build lại, migrate020 exit0. Transaction reset giữ nguyên toàn dòng admin/mật khẩu/liên kết + toàn bộ zalo_link_codes được giữ và pgmigrations bằng EXCEPT hai chiều, xóa 17 bảng nghiệp vụ bằng TRUNCATE RESTRICT, 2 tài khoản không-admin, 1 phòng; không reset sequence, không xóa storage/volume/.env. **Sau restart:** users1/admin hoạt động1/Zalo1/mã liên kết1; works/work_items/task_files/versions/departments/sessions=0; không còn FK mồ côi. Tạo lại ba container `--force-recreate`, app+db healthy, OO Up và HTTPS healthcheck=true. `/readyz` ok/db up, website200, route hàng chờ401 không phiên; hai truy vấn thống kê thật + đọc lệnh sửa trong container chạy được/rỗng; ba asset HTTPS khớp sha256 blob Git, buster app20260907-2/project-details20260907-1/CSS20260906-1. Zalo getMe hợp lệ, getWebhookInfo thành công, webhook+cron bật, lịch */2; không gửi tin thử, không thay token. Config checksum không đổi, `.env`600, app log mới error/fatal0. `post-reset.dump` đã restore đầy đủ chuẩn thành công trên DB tạm rồi xóa DB tạm. | `server/src/db/migrations/020_task_file_lenh_sua.sql`, `server/src/modules/taskFiles/{repo,service,routes,dsRoutes}.js`, `server/tests/integration/task-files-api.test.js`, `server/tests/unit/{task-files-editor,task-files-ui,nav-cho-duyet,thong-bao-ui}.test.js`, `web/assets/js/app.js`, `web/index.html`, `docs/{KE-HOACH-KET-QUA-FILE,HUONG-DAN-TEST-GIAO-DIEN,BAT-DAU-SESSION}.md`, `KE-HOACH-VPS.md`, `deploy/runbook.md` | Người dùng đăng nhập lại admin bằng mật khẩu cũ, Ctrl+Shift+R, khai lại phòng/tài khoản/công việc. Không tự seed/reset lần nữa. Backup trước reset có dữ liệu mồ côi nên xem runbook §3.1 trước restore; backup sau reset restore chuẩn xanh. Chưa e2e gửi DOCX/Zalo người thật trên production sau reset; không suy ra chỉ từ healthcheck. |

| 2026-09-08 | **Phase8b — bỏ vai phân quyền Quản lý công việc: mã xong PC, CHỜ OK mới.** Đối chiếu VPS đúng b03c74a, source giống PC: trước chỉ ẩn bảng quyền, form vẫn gửi Quản lý→vai cũ; dept_role chưa đồng bộ role. Đọc-only VPS xác nhận NV004=QLCV/Phó phòng, NV005=QLCV/Trưởng phòng; không đổi NV007 PGD chưa phòng. Migration021 chuyển vai cũ theo dept_role, đồng bộ NV đang giữ chức vụ TP/PP, xóa override vai cũ, siết CHECK; không thay mật khẩu/Zalo/phòng/phân công. App chỉ có vai cũ chuyển allowed_roles thành admin (admin vốn luôn thấy), không thành mảng rỗng công khai, kể cả vai lặp; app có vai khác chỉ bỏ vai cũ. Form/API/RBAC còn năm vai, không tạo lại vai cũ; sửa admin không hạ quyền. Script restart.sh Linux: preflight/lock/backup riêng → build → dừng app/OO → tạo lại db/OO → migrate trước app → health; không pull/seed/reset/down-v/đổi secrets, mock tests canh lỗi backup/build/migrate và check chỉ đọc. Full lần đầu1750 xanh/1 đỏ do thông điệp403 cũ của test nhắc việc; sửa kỳ vọng đúng phạm vi, không nới quyền. **Full cuối sau sửa migration app:1751/1751,99 file,exit0,173.23giây**;12 JS scoped lint/format sạch, bash-n/node-check/diff-check sạch, XSS106/903. Prettier không có parser SQL, SQL kiểm qua migration/test thực tế. **PC dev/UAT020→021**, backup riêng trước, dump kiểm danh mục; fingerprint toàn dòng20 bảng/DB trùng trước-sau, role cũ0, không seed/reset/dừng máy chủ; backup tại C:/Users/HANAPH~1/AppData/Local/Temp/qlcv-role021-pc-j2bVz2. 8099 readyz OK; đã mở trình duyệt PC nhưng chưa nghiệm thu form bằng admin/người dùng. Buster app20260908-1, project-details20260907-1/CSS20260906-1. | server/src/db/migrations/021_remove_legacy_manager_role.sql; server/src/modules/users/service.js; server/src/middleware/rbac.js; server/src/db/seeds/dev.sql; tests phân quyền/tài khoản/migration/restart; web/assets/js/app.js; web/index.html; deploy/restart.sh; deploy/runbook.md; KE-HOACH-VPS.md; docs/BAT-DAU-SESSION.md; docs/KE-HOACH-PHAN-QUYEN-DONG.md; docs/HUONG-DAN-TEST-GIAO-DIEN.md | Người dùng kiểm PC mục9b.14 và OK RIÊNG bản mới. Chưa commit/push/migrate021/restart VPS; restart mã cũ không sửa role trong CSDL. Sau OK: explicit commits, backup/build/migrate021/restart theo runbook, kiểm NV004=PP/NV005=TP và dữ liệu/Zalo giữ nguyên. Không reset database lần nữa. |

| 2026-09-08 | **Phase8b — người dùng OK bản phân quyền mới, ĐÃ PHÁT HÀNH VPS.** Commit explicit có trailer: `fcc8aed` (may-chu: migration021, RBAC/users/seed/tests và `deploy/restart.sh` mode755) + `f23fd34` (giao diện: form năm vai, đồng bộ vai/chức vụ, buster app20260908-1). Push `b03c74a..f23fd34`; VPS backup trước ở `/var/backups/qlcv/role021-20260908-164329/` (dump110998 bytes + storage + checksum), fetch/pull ff-only rồi chạy **chính** `bash deploy/restart.sh` nền; exit0, backup riêng script `/var/backups/qlcv/restart-20260908-164333-NO5q7k`, build/migrate/tạo lại db/OnlyOffice/app. `pgmigrations=021_remove_legacy_manager_role`; NV004 Phó phòng, NV005 Trưởng phòng, role cũ=0, legacy override=0, app role cũ=0. Fingerprint tất cả dữ liệu nghiệp vụ trùng trước-sau; users trùng khi bỏ role/updated_at; Zalo liên kết/mã=1/1; sessions vẫn4 dòng, hash đổi do phiên có thời hạn biến động. `.env` checksum trùng, không reset/seed/đổi volume/storage/token; untracked `deploy/.env.save` giữ nguyên. app/db healthy, readyz dbup, OO=true, public `/healthz`200; static public app20260908-1, project-details20260907-1, CSS20260906-1. | `fcc8aed`, `f23fd34`, `server/src/db/migrations/021_remove_legacy_manager_role.sql`, `deploy/restart.sh`, `deploy/runbook.md`, `KE-HOACH-VPS.md`, `docs/BAT-DAU-SESSION.md` | Danh sách lỗi Phase8b tiếp tục mở; dùng `bash deploy/restart.sh --check` để xem trạng thái, hoặc chạy không tham số sau khi đã pull mã mới và người dùng đã đóng OnlyOffice. Không reset dữ liệu lần nữa. |
...
| 21 | **Trình lên TỰ ĐỘNG cho Phó GĐ phụ trách (fallback GĐ) hay TP/PP tự chọn thời điểm?** Hiện làm: TP/PP bấm nút «Trình Phó giám đốc», người nhận = Phó GĐ phụ trách phòng của công việc qua `department_managers`; phòng không có PGD phụ trách thì không ai được báo (admin vẫn duyệt được) | Không chặn | ✅ **chốt 2026-09-01: TP/PP TỰ CHỌN** — đúng như đang làm: nút «Trình Phó giám đốc» do TP/PP bấm khi thấy đủ, nội dung trình ≥ 10 ký tự; thông báo tới Phó GĐ phụ trách phòng của công việc |
| 22 | **Giới hạn file kết quả nhiệm vụ?** (loại + dung lượng) | Không chặn | ✅ **chốt 2026-09-01: OK 20 MB** — `.doc/.docx/.pdf` ≤ **20 MB**, mỗi lần nộp là một bản mới; đổi giới hạn là 2 hằng số (`DUNG_LUONG_TOI_DA`, `DUOI_FILE_HOP_LE` của `taskFiles/service.js`) |
| 23 | **RAM/HTTPS của VPS cho editor trực tuyến?** Nextcloud AIO cần domain + HTTPS (Let's Encrypt tự động) và ~2–4 GB RAM; ONLYOFFICE Docs standalone nhẹ hơn, KHÔNG cần Nextcloud — bảng so sánh ở §7 `docs/KE-HOACH-KET-QUA-FILE.md` | Phase editor trực tuyến (chưa làm — app chạy được ngay không cần editor: PDF iframe, DOCX tải về) | ✅ **chốt 2026-09-01: CÓ editor trực tuyến** — làm ở session editor theo khuyến nghị §7 (ONLYOFFICE Docs standalone Docker, iframe + JWT + callback). **Yêu cầu người dùng kèm theo: mọi lần sửa file đều phải LƯU LẠI thành bản mới trong cùng nhóm để xem được** (khớp thiết kế callback-save §7d) |
| 24 | **`.doc` cũ có cần xem trực tuyến không?** | Không chặn | ✅ **chốt 2026-09-01: các dạng Word (.doc VÀ .docx) đều cho sửa + xem trực tuyến** khi bật editor — ONLYOFFICE mở `.docx` nguyên bản, `.doc` đi qua chuyển đổi của Document Server; ghi yêu cầu vào `docs/KE-HOACH-KET-QUA-FILE.md` §7 |
| 25 | **THÔNG BÁO SANG ZALO — bốn câu cần chốt trước khi chạy thật.** Người dùng yêu cầu 2026-09-06: «khi có thông báo phê duyệt gì thì nổi lên thông báo. Thêm thông báo đến zalo bot creator để thông báo đến zalo người nhận». Đã tra tài liệu (`docs/KE-HOACH-THONG-BAO.md` §2) và chọn **Zalo Bot API** (`bot-api.zaloplatforms.com/bot<token>/sendMessage`, token lấy từ OA «Zalo Bot Manager» → Zalo Bot Creator), **không** dùng OA API/ZNS vì tin Tư vấn đòi người nhận có tương tác với OA **trong 7 ngày** — thông báo phê duyệt là việc thưa nên phần lớn sẽ **không đến được**. Bốn câu: **(a) ai tạo bot** — cần một tài khoản Zalo đứng tên bot, token gửi qua tin nhắn cho tài khoản đó, ai giữ token? **(b)** mỗi cán bộ phải **tự nhắn `LIENKET <mã 6 số>`** cho bot một lần để hệ thống biết `chat_id` (Zalo **không** cho tra từ số điện thoại/email) — 13–50 người làm việc này có chấp nhận được không? **(c)** đẩy Zalo cho những loại nào — đề xuất chỉ `approval_pending` + `approval_rejected` + `overdue`, **không** đẩy «đã duyệt» và tin chung? **(d)** webhook đòi **domain công khai + HTTPS** (tài liệu từ chối `localhost` và IP nội bộ) ⇒ tự liên kết chỉ chạy được từ **Phase 8**; trước đó gán tay bằng `npm run zalo:link`. Chấp nhận không? | Việc 6 (chạy thật) | ✅ **trả lời 2026-09-06 bằng prompt Phase 8**: người dùng cho chạy việc Zalo thật = chấp nhận cả bốn câu — (a) người dùng tự giữ token; (b) mỗi cán bộ tự nhắn `LIENKET <mã>` chấp nhận được (~20 người sau go-live); (c) đẩy theo thiết kế `docs/KE-HOACH-THONG-BAO.md` §4; (d) webhook tại `https://ttdt.site/api/zalo-bot/webhook`. Mã + 43 test xong (Phase 8 việc 1). **Còn chặn**: token cũ LỘ trong chat — người dùng XOAY token mới, tự dán vào `deploy/.env` trên VPS (KHÔNG qua chat) thì việc 6 mới chạy thật |
...
**Phase8b 2026-09-08 — bỏ vai cũ:** ẩn cột bảng quyền không xóa vai trong form/API/CSDL;
restart đúng mã vẫn sinh lại vai sai nếu FORM_ROLE_MAP còn «Quản lý». `role` và `dept_role`
phải đồng bộ cho TP/PP/Cán bộ, nhưng không được hạ admin/PGD theo chức vụ phòng.
Migration021 + `legacy-manager-role.test.js`, `users-departments-crud.test.js`,
`tai-khoan-ui.test.js` canh cả dữ liệu cũ lẫn tạo/sửa mới. App chỉ có vai đã bỏ không được
thành `allowed_roles=[]` (mở tất cả); chuyển sang chỉ admin, kể cả danh sách lặp vai cũ,
để sửa form app sau đó cũng không vô tình công khai. Script restart phải migrate trước bật
app; `restart-script.test.js` kiểm dừng đúng khi backup/build/migrate lỗi, không reset dữ liệu.
...
| Route GET đặt `res.locals.audit` (`bootstrap.get`, `stats.*`, `gantt.tree`) | Code chết với REST trực tiếp (audit chỉ ghi với GHI) nhưng qua **cầu RPC subrequest `locals` dùng chung** ⇒ giẫm đè mốc `rpc.*` của cầu — mỗi lần mở trang một dòng rác trong «Hoạt động gần đây» | bỏ hết `res.locals.audit` ở route GET; cầu RPC: mốc `rpc.<tên>` còn nguyên sau handler ⇒ `skipAudit` (TC-RPC-24, 2026-08-29) |
| Anchor ĐA DÒNG trong script `tools/_tam.mjs` (file đích CRLF) | Mốc ghép `\n` không bao giờ khớp ⇒ script ném «gặp 0 lần» giữa chừng (an toàn vì chưa ghi, nhưng mất một lượt chạy) | anchor chỉ dùng MỘT dòng (hoặc `\r?\n`); script luôn đếm ĐÚNG số lần trước khi ghi |
| **MỘT file có HAI kiểu escape lẫn nhau** (app.js: vùng nối dài `\n` dùng `\"` cho thuộc tính, vùng thường dùng nháy trần) | Anchor đoán theo mẫu khác → «gặp 0 lần» hoặc tệ hơn là khớp nhầm chỗ khác | in nguyên văn bằng `JSON.stringify(slice)` + `charCodeAt` trước khi vá; cắt theo chỉ số (indexOf đầu + indexOf kết thúc) thay vì ghép chuỗi dài |
...
| Vẽ giao diện theo dữ liệu mà một lời gọi **bất đồng bộ** mới nạp xong (`getDepartmentContext` → `visibleDepartments`) mà không vẽ lại khi nó về | `handleSuccessfulLogin` gọi `renderProjects()`/`renderTasks()` **ngay** (đồng bộ), lúc đó `visibleDepartments` vẫn là `[]` mặc định ⇒ **Phó Giám đốc thấy trắng cả 2 tab**. Tệ hơn là nó *chập chờn*: đổi tab hay bấm lại là thấy, nên trông như "PGĐ này lỗi, PGĐ kia không" và dễ đi tìm ở phân quyền máy chủ (đã mất một lượt điều tra ngày 2026-08-29 mới biết cả hai tài khoản đều nhận đúng dữ liệu). Hễ nạp xong phạm vi xem thì **vẽ lại** đúng những khung phụ thuộc nó | `loadDepartmentContext` trong `web/assets/js/app.js` — vẽ lại 6 khung + Gantt khi `isDeputyDirector` bật **hoặc vừa tắt**, bọc `try/catch` để trang thiếu khung không chặn `callback` |
| Đọc `activity_logs` ngay sau khi supertest trả về | `middleware/audit.js` **cố ý** ghi trong `res.on('finish')` (nguyên tắc 2: nhật ký không làm chậm người dùng), nên lượt ghi sống lâu hơn request: test đọc liền thì **chưa thấy dòng**, còn lượt ghi rơi muộn thì lọt qua `TRUNCATE` của test SAU và hiện ra như **dòng lạ**. Sinh ra một chùm đỏ giả ĐỔI CHỖ mỗi lượt chạy (TC-UQ-13b/13c/17, `proposal.update`, `chat.send`, TC-RPC-21) — mất công đi tìm ở chỗ không có lỗi, và tệ nhất là nó dạy người ta bỏ qua màu đỏ | `flushAudit()` xuất từ `src/middleware/audit.js` (tập promise đang bay); `resetTables()` chờ nó **trước** khi `TRUNCATE`, mọi helper đọc nhật ký gọi nó trước khi `SELECT` |
| Vẽ bảng quyền/phân quyền phía client bằng **cột trạng thái tĩnh** (`g/tp/pp/nv`) tự đoán | Lệch vĩnh viễn với ma trận server: thiếu vai, sai khi admin vừa lưu ghi đè — user khác F5 vẫn thấy bảng cũ («không cập nhật»). Trạng thái gốc phải lấy **từ server** (`GET /api/v1/permissions` trả `macDinh` = `PERMISSIONS`), ghi đè chồng lên | Vòng 12c — `oPhanQuyenHieuLuc(row, vai, ghiDe, macDinh)` |
| Bổ sung **quyền mới cho vai** ở server (ma trận/inScope) mà **quên rà các điểm khoá client viết TRƯỚC tính năng bảng quyền** (`coQuyenSuaCongViecCon`, `dsNhiemVuToiDuocThay`, `canUserEditResource`…) | TP/PP bị khoá nhầm ở UI dù server cho phép: mất icon sửa, tab nhiệm vụ trống — người dùng báo «đã cài cho phép mà không thấy». Hễ đổi luật vai thì **grep lại mọi hàm khoá client theo vai** | Vòng 12c — 3 lỗi thật 2026-08-29 |
| Tra ghi đè / ma trận phân quyền bằng **NHÃN HIỂN THỊ** của cột thay vì **VAI CSDL** | `users.role` là `'Nhân viên'` còn bảng in ra `'Cán bộ'`; tra bằng nhãn thì mọi ô của cột đó rơi về «✕ Tắt» **và** option đầu mang `value=""` ⇒ bấm Lưu gửi `giaTri: 'mac-dinh'` = **XOÁ SẠCH ghi đè** mà không có thông báo lỗi nào (`'mac-dinh'` là lệnh hợp lệ). Người dùng thấy đúng như «không sửa được chức năng nào của Cán bộ». `VAU_BANG[].ten` chỉ dùng in `<th>`; mọi tra cứu + `data-vai` phải là `VAU_BANG[].vai` | Vòng 12e 2026-08-31 — `buildBangPhanQuyenHtml` / `oPhanQuyenHieuLuc`, TC-TKPQ-11..15 |
| Bộ lọc tháng của một tab **không có mục «Tất cả tháng»** trong khi tab kia có | Mở tab ra đúng tháng không có dữ liệu là người dùng kết luận «không thấy tháng nào» và báo lỗi phân quyền — mất một lượt điều tra tìm lỗi không tồn tại. Hai bộ lọc song song thì phải cùng bộ option và cùng luật giao khoảng (đối chiếu ở §10.2 `docs/KE-HOACH-PHAN-QUYEN-DONG.md`) | Vòng 12e — ô Tháng tab «Quản lý nhiệm vụ» thiếu value 0 |
| Đếm số thẻ thống kê bằng **bộ lọc riêng** thay vì dùng lại hàm lọc phạm vi của danh sách | `renderTaskStats` tự lọc «việc của mình + công việc mình quản lý» ⇒ PGĐ/TP/PP thấy danh sách có nhiệm vụ mà 4 thẻ đếm trên đầu hiện **0** — mâu thuẫn ngay trong một màn hình. Thẻ đếm và danh sách phải uống chung một hàm (`dsNhiemVuToiDuocThay`) | Vòng 12e — phát hiện khi soát, chưa ai báo |
| Thêm **giá trị mới cho `approval_status`** mà chỉ nới CHECK, không nới **hai view `v_countable_*`** | Trạng thái mới lọt ngay vào mọi thẻ số và biểu đồ (chúng đọc qua view). Tệ hơn: nhiệm vụ cấp 3 LUÔN «Đã duyệt» nên một nhiệm vụ trong cây nháp vẫn cộng vào «Tổng nhiệm vụ» dù cả công việc chưa ai gửi duyệt — đúng kiểu sót mà 004 sinh ra để chặn. Nới CHECK và nới view là **một** việc, không phải hai | 012 (Vòng 13) — `Nháp`; `countable-views.test.js` có 4 ca canh |
| Down migration siết lại CHECK mà **không hạ dữ liệu trước** | Còn một dòng mang giá trị mới là câu `ALTER … ADD CONSTRAINT` nổ và lượt `migrate:down` đứt giữa, để lược đồ nửa vời. Luôn `UPDATE` dữ liệu về giá trị cũ TRƯỚC khi thêm ràng buộc cũ — và thử `down` rồi `up` lại ngay khi viết, đừng để lần cần lùi bản mới biết | 012 |
| Hai nút `type="submit"` trong CÙNG một form, phân biệt bằng `event.submitter` | jsdom **không dựng** `event.submitter` nên test luôn đi vào nhánh mặc định và bản sửa trông như chạy đúng. Đặt cờ bằng listener `click` trên từng nút rồi đọc cờ lúc submit (và xoá cờ ngay, nếu không lần submit sau kế thừa) | 012 — nút «Lưu nháp» cạnh nút chính |
| Thêm một trạng thái mà chỉ lọc ở **cấp 1** rồi tưởng cả cây đã kín | Dòng cấp 2/3 để nháp RIÊNG trong một công việc đã duyệt vẫn lọt: bộ lọc ở cấp 1 cho công việc đó qua. Phải gọi vị từ ở **mọi** đường đọc — kể cả `bootstrap`/RPC `getDataForUser`, và `getOne` phải trả **404** (403 đã là tiết lộ «có bản nháp tên này») | 012 — `thayDuocNhap`, 5 chỗ gọi |

**Bẫy phát hiện thêm 2026-09-01 (Vòng 13 đợt 2 — duyệt cấp 3 + yêu cầu xoá):**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| Vòng retry kết nối CSDL test tái dùng MỘT client pg — pg chỉ cho `connect()` MỘT lần trên mỗi client, KỂ CẢ khi lần trước thất bại | Lần thử 2 ném «Client has already been connected. You cannot reuse a client.» và **CHE lỗi thật** (ECONNREFUSED khi Docker chưa lên) — tưởng test hỏng trong khi Docker Desktop đang tắt; unit jsdom cũng đỏ vì `globalSetup` chạy cho MỌI run vitest | `tests/global-setup.js`: mỗi lần thử tạo client MỚI (đã vá); thấy lỗi reuse ⇒ kiểm Docker trước khi soi code. Docker tắt thì: `docker compose -f deploy/docker-compose.dev.yml up -d` |
| Thêm cột vào bảng mà view dựng bằng `SELECT *` — Postgres ĐÓNG BĂNG danh sách cột lúc tạo view | View trả thiếu cột mới, đọc ra `undefined` âm thầm; test chốt «view đủ cột như bảng» đỏ | `CREATE OR REPLACE VIEW` trong CÙNG migration thêm cột (013 đã làm cho `v_countable_*`) |
| Client nhận biết lỗi máy chủ bằng CHUỖI câu thông báo (`phaiXinXoa` khớp chữ «Xin xoá») vì đường xoá đi qua cầu RPC chỉ mang `error` chữ, không có `details.canXinXoa` | Đổi câu ở server ⇒ luồng «Xin xoá» vỡ IM LẶNG (bấm 🗑 chỉ thấy toast lỗi, không vào hộp nhập lý do) | Câu chữ là hằng có test canh (TC-XOA-01 khớp «Xin xoá») — đổi câu ở `xoaPhaiQuaDuyet` phải đổi `phaiXinXoa` CÙNG commit |
| Helper trả HTML không mang tiền tố build*/tao*/render* (`xinXoaBadge`) | Bộ soát XSS coi MỌI chỗ gọi là lỗ CAN-THOAT ⇒ TC-SEC-10 đỏ 2 chỗ; dễ sa vào «thêm CO_Y_KHONG_BOC cho hết đỏ» | Đổi tên builder (`buildXinXoaBadge`) là hết — đừng bạch hoá lỗi đặt tên bằng pin; ngược chiều với bẫy «tao*Html» 2026-08-26 |

**Bẫy phát hiện thêm 2026-09-03 (mở PowerPoint / Excel / ảnh cho kết quả — Vòng 14续8):**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| Vá `web/assets/js/app.js` bằng script node mà chuỗi cần tìm viết trong **template literal nhiều dòng** | File đó lưu **CRLF**, template literal trong `.mjs` sinh **LF** ⇒ mọi đoạn từ 2 dòng trở lên báo `Cần 1 lần, tìm thấy 0` dù copy đúng từng ký tự. Mất thời gian đi soi khoảng trắng. Trong hàm `thay()` phải `.replace(/\n/g, '\r\n')` **cả needle và replacement** trước khi đếm/thay — và giữ nguyên cách đếm đúng số lần (đừng hạ xuống `replaceAll` cho qua) | `tools/_tam-duoi-file.mjs` (đã xoá sau khi vá) — mọi script `tools/_tam*.mjs` sau này |
| Bảng whitelist «đuôi → **MỘT** mime» rồi so bằng `===` | Cùng một file `.xls`/`.ppt`, mỗi trình duyệt/Windows gửi một mime khác (`application/vnd.ms-excel`, `application/excel`, `application/x-msexcel`…) ⇒ người dùng bị 400 «Chỉ nhận file…» với đúng file Excel thật, không cách nào tự hiểu. Đuôi phải trỏ tới **danh sách** mime + so bằng `includes`, và vẫn nhận `application/octet-stream` (trình duyệt không đoán được thì gửi cái này) | `DUOI_FILE_HOP_LE` trong `taskFiles/service.js`; TC-TF-14b canh cả 6 định dạng mới |
| `DOCUMENT_TYPE_THEO_DUOI[duoi] ?? 'word'` — fallback im lặng khi mở sửa trực tuyến | Đuôi lạ (ảnh, zip…) biến thành tài liệu **Word** ⇒ ONLYOFFICE mở ra một trang editor lỗi trắng, không câu nào giải thích, người dùng báo «bấm ✎ không được». Không có mặc định thì **400 kèm câu đọc được** (`Không sửa trực tuyến được file .png — hãy tải về để xem`), client ẩn luôn nút ✎ theo `suaTrucTuyenDuoc()` | `moEditor()`; TC-TF-16b (ảnh ⇒ 400) + TCKQ-17b (✎ theo đuôi) |
| Con số dung lượng viết cứng trong từng câu lỗi | «20 MB» nằm ở **6 câu** (2 `routes.js`, 3 `service.js`, 1 client): nới hạn mức mà sót một chỗ là người dùng đọc được hai con số khác nhau trong cùng một luồng nộp file. Một hằng `NHAN_DUNG_LUONG` + nội suy; regex đuôi và `accept=` cũng **sinh từ chính bảng** whitelist | `service.js` xuất `NHAN_DUNG_LUONG`; TC-TF-14 khớp chuỗi `'50 MB'` |
| Nới whitelist đuôi rồi cho `?inline=1` mở «mọi thứ trông như ảnh» | `.svg` là **XML chạy được `<script>`**: mở inline dưới tên miền của hệ thống là XSS lưu trữ, đọc được cookie phiên. `helmet()` chỉ cho `nosniff`, nó **không** cứu trường hợp mime đúng là `image/svg+xml`. Nên `.svg` bị chặn ngay ở whitelist, và `inline` đi theo danh sách **cho phép** `MIME_XEM_INLINE` (PDF + 4 ảnh raster), không theo phép thử «có phải ảnh không» | TC-TF-14 (`.svg` ⇒ 400) + TCKQ-13 (client không gọi máy chủ) + TC-TF-14c (xlsx xin inline vẫn ra `attachment`); ghi ở `docs/XSS-4.6.md` mục 2026-09-03 |
| Nới hạn mức file mà quên tầng **proxy** | Node nhận 50 MB nhưng Nginx mặc định `client_max_body_size 1m` ⇒ 413 với trang lỗi của Nginx, không phải câu tiếng Việt của app, và log app **trống trơn** nên tìm mãi trong code. Nới hạn mức là việc của cả `deploy/nginx*` | Phase 8 — ghi ở dòng §13.3 ngày 2026-09-03 |
| `multer.memoryStorage()` + hạn mức tăng dần | Mỗi file nằm **trọn trong RAM** lúc tải: 50 MB × vài người nộp cùng lúc là ăn hết bộ nhớ VPS. Muốn vượt 50 MB thì phải đổi sang `diskStorage`/stream TRƯỚC, đừng chỉ sửa con số | `taskFiles/routes.js`; giới hạn 50 MB là **có chủ đích** |

**Bẫy phát hiện thêm 2026-09-04 (thiết kế lại hai bảng kết quả theo file của người dùng — Vòng 14续9):**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **Ảnh thiết kế không đọc được thì ĐỪNG suy ra bố cục** | Vòng trước tôi đã một lần dựng câu hỏi trên chi tiết ảnh **bịa ra** rồi phải rút lại. Ảnh `.png` mô hình không mở được ⇒ nói thẳng «không đọc được ảnh» và hỏi dạng khác. Người dùng gửi `docs/moi.xlsx`, đọc bằng `exceljs` (đã có trong `devDependencies`, không thêm thư viện) là ra **đủ 16 tên cột + mọi câu chú**, không còn chỗ nào phải đoán | Luật §6; `docs/moi.xlsx` |
| Gộp cả chuỗi `onclick` rồi cho qua `escapeHtmlAttr` (menu ⋯) mà test vẫn assert chuỗi thô | `escapeHtmlAttr` đổi `'` thành `&#39;` nên `expect(html).toContain("f('7')")` **luôn đỏ** dù mã đúng. Trong CÙNG một bảng có hai kiểu: menu ⋯ (escape cả chuỗi ⇒ assert `&#39;`) và ô «Nhiệm vụ» (dấu `'` là chữ tĩnh của mã ⇒ assert thô). Đọc HTML thật rồi mới viết assert | TCKQ-16/17/21 |
| Ẩn/hiện nhóm `<tr>` con bằng **thứ tự DOM** (`nextElementSibling`, `rows[i+1]`) | Giữa dòng cha và các dòng bản còn **dòng panel** «Ý kiến/Lịch sử», và một bảng có nhiều kết quả ⇒ đếm theo vị trí bung sai dòng ngay khi thêm kết quả thứ hai. Chọn bằng `data-nhom="<id nhóm>"` | `batTatBanKq`; TCKQ-25 |
| Menu mở ra mà không đóng menu khác + gắn listener «bấm ra ngoài» trong hàm dựng | Hai menu chồng nhau ⇒ bấm nhầm dòng khác; listener gắn mỗi lượt vẽ ⇒ **rò listener** (bảng vẽ lại sau mỗi hành động). Đóng hết menu khác trước, listener gắn **một lần** qua cờ `daGoiDongMenuKq` | `batTatMenuKq`/`goiDongMenuKq`; TCKQ-28 |
| Dựng câu «Tình trạng» cho trang hàng chờ bằng cùng hàm với modal | `listChoDuyetKetQua` **không trả bảng luồng** ⇒ mọi dòng hàng chờ hiện «Bị trả lại lần 0» hoặc mất hẳn phần đó. Phải có `cauTinhTrangHangCho` truyền luồng RỖNG — thiếu dữ liệu thì nói ít đi, không bịa số | `cauTinhTrangHangCho`; TCKQ-27 |
| Suy «Định dạng» từ đuôi rồi `?? 'Word'` cho đuôi lạ | Cùng lớp bẫy `DOCUMENT_TYPE_THEO_DUOI ?? 'word'` của 续8: file `.zip` hiện là «Word» thì người duyệt tin sai. Đuôi ngoài bảng trả `—` | `dinhDangCuaTen`; TCKQ-26 |
| Gỡ một hàm dựng HTML mà quên `EXPORTS` của test jsdom | `Object.assign(window, { …, buildHangCayChoDuyet, … })` nêu tên đã xoá ⇒ **`ReferenceError` lúc nạp app.js**, cả 30 test của file đỏ cùng lúc với thông báo không liên quan gì tới thứ vừa sửa | `tests/unit/task-files-ui.test.js` |
...
**Bẫy phát hiện thêm khi làm Vòng 14 (2026-09-01) — kết quả nhiệm vụ là file:**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| `ENTITY_LABEL`/`ACTION_LABEL` thiếu thực thể mới | Vai ngoài phạm vi bị từ chối thì `can()` nội suy `${ENTITY_LABEL[entityType]}` = `undefined` ⇒ **500 INTERNAL** thay vì 403 với câu tiếng Việt | thêm `file: 'file kết quả'` vào `ENTITY_LABEL` · TC-TF-13 canh đúng 403 |
| `resetTables()` không thêm bảng mới | Dữ liệu test trước rò sang test sau; `schema.test.js` TC-DB-01 đếm bảng bằng `BUSINESS_TABLES.length + 1` nên **thêm bảng mà không thêm vào list này là đỏ ngay** | 4 bảng file vào `tests/helpers/db.js` trước `permission_overrides` |
| Lỗi **multer** nổ TRƯỚC handler | `try/catch` trong handler không bao giờ thấy `MulterError` ⇒ vượt 20 MB trả **500** thay vì 400 | middleware riêng đứng ngay sau `upload.single('file')` đổi `MulterError` → 400 tiếng Việt · `taskFiles/routes.js` |
| `app.js` **trộn LF/CRLF** (dòng template khổng lồ là CRLF, phần còn lại LF) | Script vá khớp một biến thể EOL là «thấy 0 lần» dù chuỗi đúng nguyên văn | hàm `thay()` thử cả 3 biến thể (gốc/LF/CRLF) — khuôn `tools/_tam*.mjs` |
| node-pg-migrate tách up/down bằng marker `-- Up Migration` / `-- Down Migration` | Đọc nhầm luật CHECK của migration trước (`po_cho_duyet` của **011** cho cho-duyet ở create/update/delete) là nới "thu hẹp" lại — `ADD CONSTRAINT` nổ trên UAT có dữ liệu cũ | đọc ĐỦ migration 009→013 trước khi viết 014; down phải trả về đúng luật mới nhất (011), xoá dòng 'file' TRƯỚC khi siết |
| PowerShell session **giữ `$env:DATABASE_URL` giữa các lệnh** | Set URL cho UAT ở lệnh trước, lệnh sau tưởng đang chạy vào dev mà thực ra vẫn vào UAT (hoặc ngược) — migrate nhầm CSDL | set `$env:DATABASE_URL` TƯỜNG MINH ngay trước mỗi nhóm lệnh migrate, không tin session cũ |
| `can()` mới trả `version_no` từ bảng không có cột đó | Test đọc bảng luồng truy vấn `task_file_flow.version_no` trực tiếp ⇒ lỗi SQL; cột thật nằm ở bảng versions | JOIN `task_file_versions` (khuôn `listLuongByFile`) · helper test luongCuaNhom |
| supertest không parse body nhị phân | `.text`/`.body` undefined khi tải `.pdf` về ⇒ tưởng máy chủ trả rỗng | `.buffer(true).parse(...)` ghép Buffer · TC-TF-01 |
| Cờ cấu hình máy chủ mà **không trả về client** | Nút ✎ «sửa trực tuyến» không bao giờ hiện dù ONLYOFFICE đã chạy: client không có cách nào biết máy chủ có `ONLYOFFICE_URL`/`ONLYOFFICE_JWT_SECRET`, mặc định coi là tắt — dễ đi tìm lỗi ở container Docker thay vì ở payload | `GET /work-items/:ref/files` trả thêm `onlyOffice: service.onlyOfficeBat()`; hễ thêm tính năng phụ thuộc env thì **cờ phải đi cùng dữ liệu** |
| Assertion trên chuỗi JSON có **khoảng trắng tưởng tượng** | `expect(text).toContain('"mode": "edit"')` đỏ dù trang đúng: `JSON.stringify` **không** chèn space sau dấu hai chấm | so `'"mode":"edit"'` · TC-TF-15 |
| Ma trận máy chủ và hàm mở nút phía client **lệch âm thầm** | TP/PP có `task:update` ở `PERMISSIONS` nhưng `canUserEditResource` không mở `'task'` ⇒ người dùng báo «không sửa được nhiệm vụ» mà log máy chủ **trắng** (request không bao giờ được gửi) — mất một vòng điều tra ở `rbac.js`/`inScope` | mỗi lần nới ma trận: soát **cặp** `canUserEditResource`/`canUserDeleteResource` cho đủ 3 loại `project`/`subwork`/`task`; TC-TP-CV-05 canh cả hai |
| Seed mới TRUNCATE sạch mà test chạy chung CSDL | File test nạp seed rồi để lại 7 người/5 nhiệm vụ ⇒ file test SAU thấy dữ liệu lạ, đỏ đổi chỗ mỗi lượt | `seed-vong14.test.js` gọi `resetTables()` ở **cả** `beforeAll` và `afterAll` |
| **`helmet()` chặn script của dịch vụ ngoài — trang nhúng bên thứ ba TRẮNG, không lỗi** | `script-src 'self'` áp cho MỌI phản hồi ⇒ `<script src="http://<DS>/…/api.js">` bị chặn, `DocsAPI` không tồn tại, màn hình trắng và lỗi CHỈ hiện ở tab Console. Đi tìm ở container/JWT/URL mất cả vòng mà không thấy gì sai | `cspEditor(dsUrl)` + `res.setHeader('Content-Security-Policy', …)` cho **đúng một** route; nới cả `frame-src` (iframe của DocEditor) và `connect-src` (+`ws:`/`wss:`); bỏ COEP, đặt CORP `cross-origin` · TC-TF-16 |
| **`createReadStream(...).pipe(res)` không có listener `'error'` = SẬP CẢ TIẾN TRÌNH** | File thiếu trên đĩa ⇒ `'error'` không ai bắt ⇒ «Unhandled error event», Node chết. Triệu chứng ở người dùng là «tính năng X không mở được», thực ra máy chủ vừa sập nên mọi thứ khác cũng chết — chẩn đoán sai hướng rất dễ | `access()` kiểm TRƯỚC ⇒ 404 gọn, **và** vẫn gắn `.on('error')` cho trường hợp file mất giữa lúc truyền · TC-TF-17 (canh `/healthz` còn 200) |
| Bộ seed có dòng CSDL nhưng KHÔNG có file vật lý | Mọi đường đọc file của dữ liệu seed (tải về, xem PDF, mở editor) đều thất bại — và trước khi có chốt trên thì làm sập máy chủ | Ghi rõ ở đầu `dev-vong14.sql`: muốn thử luồng file thật thì nộp file mới ở **NV-01** |
| `vitest.config.js` không đặt biến env của tính năng mới | Tính năng «tắt» trong test nên route trả trang «chưa bật» ⇒ **cả lớp lỗi của tính năng đó lọt cổng test** dù 1500 ca xanh | 3 biến `ONLYOFFICE_*` giả trong `env` của vitest; hễ thêm tính năng phụ thuộc env thì thêm biến giả vào đây |
| `documentType` của ONLYOFFICE ghi cứng `'word'` | Mở `.pdf` là DS báo lỗi định dạng, đúng ngay dữ liệu seed | bảng `DOCUMENT_TYPE_THEO_DUOI` (doc/docx → `word`, pdf → `pdf`) · TC-TF-16 |
| `document.key` của ONLYOFFICE không đổi khi nội dung đổi | DS lấy lại bản trong **cache của nó** ⇒ sửa xong mở lại vẫn thấy bản cũ, tưởng callback không lưu | key = `tf-<idBản>-<kíchThước>-<mốc nộp>`, cắt 128 ký tự, chỉ `0-9 a-z A-Z -._=` (đúng giới hạn tài liệu Docs API) |
**Bẫy phát hiện thêm khi làm Vòng 14 cuối 5 (2026-09-02) — «không lưu được», tên file, nút Lưu:**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **Đường máy-đối-máy trả thân §5.3 thay vì hợp đồng của bên kia** | ONLYOFFICE đòi **đúng `{"error":0}` ở cấp cao nhất**; trả `ok()` ⇒ `{"ok":true,"data":{"error":0}}` nên DS coi là **lưu thất bại**, hiện hộp «Không thể lưu tài liệu. Vui lòng kiểm tra cài đặt kết nối…» — câu đó dẫn người ta đi tìm lỗi mạng/JWT, trong khi bản mới **vẫn được lưu** nên «Lịch sử» đúng, càng khó lần. Log của DS (`sendServerRequest returned an error: data = {"ok":true,…}`) là dấu vết duy nhất | `dsRoutes.js` callback trả `res.json({error:0})` — **ngoại lệ có chủ ý của §5.3**, ghi lý do ngay trên route · TC-TF-18 canh cả hình dạng thân và ca token sai |
| **busboy (nhân của multer) giải `filename` bằng latin1** | Tên file UTF-8 mất dấu: `BÀI 2.docx` → `BÃ€I 2.docx`, hiện ở khối «Kết quả», tiêu đề trang sửa **và** trong thông báo. multer 2.x không có tuỳ chọn bảng mã nên không ai nghĩ tới tầng này | `tenGocUtf8()` một chỗ duy nhất trong service, ba lớp canh không phá tên đã đúng (ASCII / có ký tự > 0xff / giải ra `U+FFFD`) · TC-TF-19 đi qua đường thật FormData → multer → CSDL |
| Regex `[^\u0000-\u00ff]` để dò «ngoài latin1» | eslint `no-control-regex` đỏ vì lớp phủ định chứa `\x00` — đúng luật, không phải lint khó tính | dò bằng `codePointAt(0) > 0xff` trong vòng lặp `for…of` |
| Nghĩ Docs API có phương thức JS để **bắt editor lưu** | Không có (methods chỉ có `downloadAs`/`requestClose`/…), nên câu «sửa xong lưu lại kiểu gì» không có đường trả lời bằng client | **command service**: `POST {dsUrl}/command` thân `{c:'forcesave', key, userdata}` + `token` JWT của chính thân đó ⇒ DS gọi `callbackUrl` với `status=6`. `document.key` tách thành `khoaDs(ban)` vì forcesave **phải gửi đúng key** editor đang mở · 6 mã lỗi dịch sang tiếng Việt, **mã 4 = «chưa có thay đổi» KHÔNG phải lỗi** |
| Danh sách người nhận thông báo đọc **một** nguồn | Đọc chỉ `users` (vai TP/PP + `department_id`) thì người được **gắn phụ trách phòng** ở `department_managers` (`'head'`/`'vice'`) mà vai khác **không hề biết** có file mới — đúng chỗ người dùng chốt «Lãnh đạo phòng phụ trách … đồng thời nhận được thông báo» | `repo.lanhDaoPhuTrach(phongId)` gộp hai nguồn, `DISTINCT` theo id, **không** lấy `'deputy_director'` (đã có `phoGiamDocPhuTrach` riêng) · TC-HCPD-04 |
| Bản lưu từ editor ghi cứng người nộp + vai `'Nhân viên'` | Trưởng phòng sửa file của cán bộ thì «Lịch sử» hiện **tên cán bộ với vai Nhân viên** — sai người, sai vai, và không phân biệt được «nộp bản mới từ máy mình» với «sửa trực tuyến rồi lưu» | đọc `users[0]`/`actions[0].userid` của DS, kiểm người có thật + còn hoạt động rồi mới ghi; hành động mới `'sua-truc-tuyen'` (migration **015** nới CHECK, down **xoá dòng trước** khi siết) |
| Hai khối **cùng `id`** khi chuyển panel sang trang mới | `getElementById` chỉ thấy một cái ⇒ người dùng bấm cái kia tưởng nút chết | chuyển `#approvals-panel` sang `#cho-duyet-section`, **không** để lại bản sao ở trang Công việc · TCKQ-19 |
| `fetch` của Node 24 phân giải `localhost` sang `::1` trước | Container DS chỉ bind IPv4 ⇒ script Node nổ `ECONNRESET` **dù** `/healthcheck` trả `true` — nhìn như DS chết. Trình duyệt không bị (thử cả hai họ địa chỉ) | gọi DS bằng `127.0.0.1` trong mọi script Node |
| Máy chủ dev nối **CSDL nào** không hiển nhiên | `chay.bat` đặt `quanlycongviec_uat`, `deploy/.env` đặt `quanlycongviec` — **hai bộ tài khoản khác nhau**: seed Vòng 14 (`tp@`, `nv1@`…) ở `quanlycongviec`, bộ cũ (`tp01@`, `nv01@`…) ở `_uat`. Đăng nhập trượt 401 với mật khẩu đúng là dấu hiệu nối sai CSDL, không phải sai mật khẩu | kiểm CSDL **trước** khi kiểm mật khẩu; `npm run migrate:up` cho **cả hai** khi thêm migration |

**Bẫy phát hiện thêm khi làm Vòng 14续6 (2026-09-03) — phân công, cập nhật tại chỗ, `leader_ids`, bảng cây:**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **Hàm dựng ứng viên thiếu nhánh một vai** | `napUngVienPhanCong()` không có nhánh TP/PP ⇒ họ rơi vào nhánh "chỉ chính mình", rồi bị lọc `role === 'Nhân viên'` ⇒ ô «Cán bộ trực tiếp» **rỗng, không lỗi nào hiện ra**, trông như máy chủ không trả người. Đây là **lần thứ hai** cùng lớp bẫy: Vòng 12e (A) `getUserAllowedProjects` cũng thiếu đúng nhánh TP/PP. Cách tự kiểm: chạy app.js thật trong jsdom với `currentUser.role = 'Trưởng phòng'` rồi đếm `<option>` | nhánh TP/PP lấy Cán bộ trong phòng từ `tenPhongTaiKhoan()`, **không** dùng `visibleDepartments` (biến đó chỉ đúng cho PGĐ — dùng sai là TP thấy cả phòng khác) |
| Modal đang mở vẽ từ **mảng trong bộ nhớ** | `showProjectDetailsModal` dựng HTML từ `allProjects`/`allTasks`; hàm tạo mới không nạp lại hai mảng đó ⇒ tạo công việc con xong **phải tắt-mở modal mới thấy**, người dùng tưởng lưu thất bại | sau khi tạo thành công, còn `#project-details-modal` trong DOM thì `loadDataForUser()` rồi vẽ lại modal bằng `openedFromProjectDetails` — không thêm endpoint |
| Ô phân công **không khoá theo vai** | Cán bộ tự đổi «Lãnh đạo phòng phụ trách» của nhiệm vụ mình. Vì mọi cửa duyệt file đọc `leader_ids`, đây là **leo quyền thật** (tự chọn người duyệt mình), không chỉ lỗi hiển thị | `khoaPhanCongVoiNhanVien()` đặt `disabled` + câu «Do lãnh đạo phòng phân công»; máy chủ vẫn là rào cuối (`LEADER_NOT_IN_SOURCE` của `assignments/service.js`) |
| **Hàng chờ rộng hơn quyền thật** | `listChoDuyetKetQua` bó `i.department_id = phòng mình` trong khi `verdict`/`nop` đã siết theo `laLanhDaoPhuTrachNhiemVu` (续5) ⇒ MỌI TP/PP của phòng vẫn **thấy và bấm được** dòng không phải của mình, chỉ bị chặn ở request cuối — người dùng báo đúng chỗ này | SQL đổi sang `$1::bigint = ANY(i.leader_ids)`; **hệ quả có chủ ý**: nhiệm vụ chưa gán lãnh đạo thì không TP/PP nào thấy — admin/PGĐ vẫn xử được nên file không treo vĩnh viễn |
| Sắp lại cây **ở client** | Bảng theo cây mà client tự `sort` thì thứ tự lệch với `LIMIT 200` của máy chủ ⇒ nhóm bị vỡ ngẫu nhiên khi dữ liệu vượt hạn | `ORDER BY w.code, cha.code NULLS FIRST, i.code` ở SQL, client chỉ **chèn hàng tiêu đề khi đổi nhóm** · TCKQ-20 |
| Dùng chung một `<input type=file>` cho hai trang | Ở trang «Hàng chờ phê duyệt» thì modal nhiệm vụ **chưa dựng**, ô chọn file của modal không tồn tại trong DOM ⇒ nút «Nộp bản mới» bấm không phản ứng, không lỗi | ô riêng `kq-cho-duyet-file-input` trong `web/index.html` + cờ `dangOTrangChoDuyet` quyết định vẽ lại bảng hay khối trong modal · TCKQ-21 |
| Pin XSS: **chỗ ghi đứng yên, giá trị tăng** | Thêm 19 giá trị mà `sink` vẫn 101 (đều nằm trong hai lỗ đã pin của tab «Phê duyệt kết quả») — dễ tưởng bộ đếm hỏng rồi đi sửa `dem-xss.mjs` | ca đầu tiên như vậy trong dự án; ghi rõ ở `docs/XSS-4.6.md` mục Vòng 14续6, TC-SEC-17 chỉ đổi số `gia_tri` |

**Bẫy phát hiện thêm khi làm Vòng 14续7 (2026-09-03) — «Tải lên thất bại: máy chủ từ chối», GĐ/PGĐ không thấy mục, hàng chờ trống:**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **Lời gọi đặt trong nhánh «đổi vai»** | `capNhatNavChoDuyet()` nằm TRONG `if (doiVai)` của `loadDepartmentContext` — nhánh đó chỉ chạy cho TP/PP/Phó GĐ khi bối cảnh phòng đổi, nên **Giám đốc (admin) không bao giờ chạy tới** và mục «Hàng chờ phê duyệt» nằm im với `class="hidden"` của HTML. Không lỗi nào hiện ra: vai có quyền cao nhất lại là vai duy nhất không thấy cửa duyệt | tách ra khối `try/catch` **ngoài** nhánh vai; TC-NAV-07 canh bằng NGUỒN (đếm ≥ 2 lần gọi trong hàm) vì nếu ai gộp lại vào `if` thì mọi test hành vi vẫn xanh |
| Badge cộng hàng chờ **không thuộc vai đó** | TP/PP không duyệt cây công việc nhưng badge vẫn gọi `/approvals/pending-count` ⇒ số phồng lên, bấm vào tab kia thì trống | `laNguoiDuyetHeThong()` quyết định có gọi endpoint thứ hai hay không · TC-NAV-03 |
| Seed cấp 2 **không có `leader_ids`** | Sau khi siết hàng chờ theo `leader_ids` (续6), bộ seed cũ để cột đó rỗng ⇒ hàng chờ của TP/PP **trống dù data mới đã nạp**, và nộp file bị 403 «không phải lãnh đạo phòng phụ trách». Người dùng báo đúng hai triệu chứng này. Đây là hệ quả trực tiếp của việc siết quyền mà **quên seed đi cùng** | `dev-vong14.sql` gán `leader_ids` cho CV001-001 (cả TP+PP) và từng nhiệm vụ cấp 3 (chia đôi TP/PP để test được cả hai vai) |
| Cấp 2 và cấp 3 khác nhau về **số lãnh đạo** | Cấp 3 chỉ được MỘT lãnh đạo (`task_leader_single`, migration 005) còn cấp 2 nhiều người; seed gán mảng 2 phần tử cho cấp 3 là CHECK nổ giữa lúc seed, thông báo lỗi không nói rõ cột nào | seed dùng `CASE` chia từng nhiệm vụ cho MỘT người, `ARRAY[...]::bigint[]` ép kiểu rõ |
| Lãnh đạo cấp 3 phải nằm trong danh sách cấp 2 | `LEADER_NOT_IN_SOURCE` của `assignments/service.js` chặn ⇒ seed chạy được nhưng SỬA qua giao diện thì 400, lỗi chỉ hiện lúc test tay | CV001-001 nêu đủ `ARRAY[tp.id, pp.id]` trước khi cấp 3 chia nhau |
| Test jsdom mới không khai vào eslint | File `.test.js` chạy môi trường jsdom mà thiếu trong `files:` của `server/eslint.config.js` ⇒ `document`/`window` bị coi là biến lạ, lint đỏ dù test xanh | thêm `tests/unit/nav-cho-duyet.test.js` vào khối jsdom (lần thứ ba cùng lỗi trong Vòng 14 — nhớ khai NGAY khi tạo file) |
...
**Bẫy phát hiện thêm khi làm Vòng 14续10 (2026-09-04) — menu bị hộp cắt, lớp Tailwind thiếu:**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **`backdrop-filter` biến thẻ thành khối chứa của `position: fixed`** | Menu ⋯ bị `.glass-card` / `.modal-content` cắt; đổi `absolute` → `fixed` **tại chỗ** vẫn bị cắt y như cũ vì cả hai thẻ đều có `backdrop-filter: blur(20px)`. Sửa CSS mãi không xong mà không hiểu vì sao | `batTatMenuKq` **dời thẻ menu ra `<body>`** rồi định vị theo rect nút, `dongMenuKq` trả về chỗ cũ · TCKQ-32 |
| Thẻ đã dời ra `<body>` mà **vẽ lại bảng** | Chỗ cũ rụng khỏi DOM ⇒ menu nằm mãi ở `<body>`, chồng lên giao diện, bấm đâu cũng thấy nó | `napKetQua` / `renderChoDuyetKetQua` gọi `dongMenuKq()` **trước** khi ghi `innerHTML`; `dongMenuKq` thấy chỗ cũ mất thì `el.remove()` · TCKQ-33 |
| **Bản Tailwind cắt sẵn thiếu lớp trang đang dùng** | `assets/vendor/tailwind/tailwind.min.css` chỉ 39 KB (§7 việc 4.3 — không CDN) nên **không có** `.bg-rose-600`, `.bg-yellow-100`, `.bg-green-800`, `.text-[11px]`, `.text-gray-300`… Thiếu lớp `text-*` thì chỉ sai cỡ chữ, nhưng thiếu `bg-*` của badge chữ TRẮNG là badge **biến mất** (số trắng trên nền trắng — đúng lỗi người dùng báo). Không có cảnh báo nào, tự nhìn không ra | Khai bù đúng những lớp đang dùng ở cuối `app.css` (không nhồi cả bảng màu); soát lại bằng cách so mọi `class=` của `index.html` + `app.js` với `.<lớp>{` trong bản đóng gói |
| Tailwind cắt sẵn **không có giá trị tuỳ ý** (`text-[11px]`, `bg-white/80`) | Cú pháp JIT chỉ chạy khi build tại chỗ; bản tĩnh không sinh lớp nào ⇒ mọi `[...]`, `/50` trong mã là **lớp chết** | Dùng lớp có sẵn, hoặc khai tay trong `app.css` (phải escape: `.text-\[11px\]`) |

**Bẫy phát hiện thêm khi làm Vòng 14续11 / Đợt 2 (2026-09-04) — khai trước, Báo cáo chữ, form tạo:**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **CHECK xor file/text** (`tfv_file_hoac_chu`) | Bản «lửng» (có `noi_dung` lẫn `ten_luu`) hoặc chữ < 10 ký tự nổ constraint thay vì 400 tiếng Việt | Service kiểm trước (`DO_DAI_NOI_DUNG_TOI_THIEU` = 10); Down **DELETE** bản chữ trước khi siết lại NOT NULL (bẫy 012/014/015) |
| **Nhóm 0 bản lọt hàng chờ** | LATERAL là LEFT JOIN ⇒ nhóm chưa nộp file hiện `cho-xem` với `ban_cuoi_*` null | `listChoDuyetKetQua` thêm `AND v.id IS NOT NULL` · TC-KQ2-02 |
| **`khaiKetQua` auto-duyệt** | `file:create = cho-phep` chốt `da-duyet` lúc chưa có file ⇒ nộp file sau **409** | `khaiKetQua` luôn để `cho-xem`; chỉ `nopBaoCao`/`nop` gọi `apTuDong` |
| **Tải / editor bản Báo cáo** | `ten_luu` NULL → `path.join(..., null)` / ONLYOFFICE không có file | `chanBanBaoCao` ở `docBan`/`docBanSystem` · TC-KQ2-05 |
| **LATERAL thiếu cột** | `SELECT v.noi_dung` mà subquery không `SELECT noi_dung` ⇒ Postgres 500 «column does not exist» (cùng bẫy `ban_cuoi_ten` 续10) | Mọi `v.col` ở ngoài LATERAL phải có trong SELECT trong LATERAL |
| **COMMENT ON TABLE ở Down dán nhầm** | Down 016 từng dán comment bảng luồng vào bảng bản | Down phải trả **đúng** comment gốc 014 (tên vật lý `v{n}-{uuid}.{ext}`) |
| **Form tạo bỏ qua `napKetQua`** | `#task-ket-qua-danh-sach` trống; `buildBangKetQua([])` chỉ ra một dòng xám không có cột | Nhúng `buildKhungDanhSachKetQua([], "")` vào chuỗi HTML `createTaskModal` khi `!isEdit && createLevel !== 2` · TCKQ-34 |
| **`handleAdd` đóng modal trước success** | Đọc DOM sau `closeModal` = mất dòng khai tạm | `thuThapDongKhaiTam()` **trước** `closeModal`; POST sau `response.taskId` · không rollback nhiệm vụ nếu POST lỗi |
| **Ô khai tạm có `name=`** | FormData `#task-form` nuốt vào `taskFromLegacy` | Chỉ `id` / class `kq-tam-*` · TCKQ-34/40 |
| **Cấp 2 POST `/results`** | `mustFindNhiemVu` 400 «chỉ nhiệm vụ cấp 3» | Không nhúng bảng, không thu thập, không POST khi `data.level === 2` · TCKQ-35 |
| **Tái dùng `buildKhoiFile` cho dòng chưa lưu** | Id giả sinh menu tải/xoá/verdict gọi REST | `buildDongKhaiTam` riêng, chỉ ô tên/định dạng/ý kiến |
| **Tên hàm HTML không `build*`** | Mỗi chỗ gọi bị đếm `CAN-THOAT` (bẫy `xinXoaBadge`) | `buildKhungDanhSachKetQua` / `buildDongKhaiTam` — `themDongKhaiTam` là sink `insertAdjacentHTML` phải giải trình (pin 102) |

**Bẫy phát hiện thêm khi sửa form nhiệm vụ (2026-09-05):**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **Timer 100ms ghi đè quyền chọn Cán bộ trực tiếp** | HTML ban đầu có `select` nhưng sau khi modal mở, TP/PP bị `disabled` và value bị ép về tên của họ; người dùng tưởng máy chủ không cho sửa | `updateAssigneePermission` mở cho admin/Phó Giám đốc trong phạm vi/Trưởng phòng/Phó phòng/người quản lý công việc; giữ `isEdit22` cho Cán bộ tự sửa; test timer |
| **Dùng lớp Tailwind tỷ lệ không có trong bản vendored** | `w-1/3`/`w-2/3` không sinh CSS trong bản cắt sẵn, cột không đổi kích thước | Dùng `md:grid-cols-3` + `md:col-span-1`/`md:col-span-2`, đã đo vendor |
| **Xoá field legacy làm thay đổi dữ liệu cũ ngoài ý muốn** | PATCH có thể gửi mảng rỗng và xóa `result_links` cũ dù người dùng chỉ sửa form | Chỉ bỏ textarea `resultLinks`; giữ `COL.T_RESULT_LINKS`, cột CSDL và nhánh legacy `taskFromLegacy`; test tạo + sửa không có field |

**Bẫy phát hiện thêm khi vá giao diện Vòng 14续13 (2026-09-06):**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **Nút submit KHÔNG mang cờ ý định** | Form TẠO từng có 2 chỗ submit: nút «Tạo nhiệm vụ» ở thanh tiêu đề (không cờ) và 2 nút ở chân form. Bấm nút tiêu đề ⇒ `event.submitter` không có `data-nhap`/`data-gui-duyet` ⇒ tạo bằng mặc định máy chủ, người dùng **không biết** mình vừa lưu nháp hay đã gửi duyệt | Chế độ TẠO chỉ để lại đúng 2 nút submit, mỗi nút một cờ; nút tiêu đề chỉ còn ở chế độ SỬA · 3 test ở `task-form-candidate.test.js` |
| **Suy trạng thái duyệt của con từ cha** | `handleAdd` từng gán `T_APPROVAL = "Chờ duyệt"` cho nhiệm vụ mới chỉ vì công việc cha đang «Chờ duyệt». SAI: cấp 3 **không** gửi độc lập (`assertCoBuocDuyet` trả 409) và `approvals.submit` chỉ nâng con đang Nháp/Từ chối ⇒ giao diện nói «Chờ duyệt» còn máy chủ vẫn để Nháp | Bỏ hẳn dòng suy diễn; đọc trạng thái **sau** `refreshData()`; `capNhatTrangThaiChoDuyetLocal` chỉ nâng con trong `["Nháp","Từ chối"]` · `approvals-ui.test.js` |
| **Đóng modal trước khi ghi xong** | `closeModal` gỡ DOM ⇒ mất các dòng khai kết quả đang chờ POST, và hẹn 300ms vẽ lại cây chồng lên lượt nạp đang chạy | `taoDauViecVaNapLai` tự đóng form sau khi xong; `closeModal` **từ chối** đóng khi form còn `data-dang-luu="1"` |
| **Bọc `escapeHtml` quanh hằng chuỗi** | Nhãn tĩnh («Hủy», «Gửi đi duyệt») bọc `escapeHtml` làm **phình** con số chốt của bộ soát XSS (893 → 896), tưởng là lỗ mới; con số phình che mất lỗ thật về sau | Hằng chuỗi viết thẳng vào HTML; chỉ bọc giá trị đến từ dữ liệu · `docs/XSS-4.6.md` mục 2026-09-05 |
| **Sửa `KE-HOACH-VPS.md` bằng chuỗi `"\n"` nguyên văn** | Một lượt vá trước đã dán `\n\n### 13.4 ...` (hai ký tự, không phải xuống dòng) vào cuối hàng bảng ⇒ tiêu đề §13.4 biến thành ô bảng, `grep "^### 13.4"` không thấy | Script `_tam*.mjs` phải ghi xuống dòng thật (`\r\n` trong chuỗi JS); kiểm lại bằng `grep -n "^### 13.4"` sau khi vá |

**Bẫy phát hiện thêm khi làm CHUÔNG THÔNG BÁO (2026-09-06):**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **Gộp cổng quyền ĐỌC với cổng quyền GHI** | `notifications/service.js` chỉ có `assertAdmin`. Dùng lại nó cho đường đọc là khoá chuông của mọi người không phải admin; nới nó ra là cho ai cũng phát thông báo hàng loạt cho toàn đơn vị | Hai hàm riêng: `assertAdmin` (ghi) và `assertDangNhap` (đọc) · `notifications-api.test.js` |
| **Nhận `userId` từ query cho tiện** | Thông báo chứa tên đầu việc, ai gửi duyệt và **lý do từ chối** của người khác. Một tham số `?userId=` là đọc hộ cả hộp thư của họ | `docCuaToi` lấy id **chỉ** từ `req.user`; test có một ca riêng cho admin để chắc không có ngoại lệ ẩn |
| **`ref_id` là id CSDL, giao diện tra theo MÃ** | Truyền thẳng `ref_id` vào `showProjectDetailsModal` là mở sai việc (hoặc mở modal trắng) — id 12 không phải `CV012` | `moThongBao` dò `allProjects`/`allTasks` theo `id` để lấy mã; dò không ra thì **báo tải lại trang**, không bịa mã · TC-TBUI-04 |
| **Vòng hỏi lại không kiểm `visibilityState`** | Chat (10 giây) đang bỏ sót: một tab để quên trong nền nã request suốt ngày; một người mở 5 tab là 5 lần `count(*)` mỗi nhịp mà không ai đọc | `batDauHoiLaiThongBao` bỏ qua lượt khi `document.visibilityState === "hidden"` và khi chưa đăng nhập · TC-TBUI-05 |
| **Tự trừ số badge ở trình duyệt** | Đánh dấu đã đọc rồi tự `badge -= 1` là hai nguồn sự thật; một tab khác vừa đọc là số lệch ngay và không bao giờ tự sửa | `PATCH /read` trả `unread` mới, giao diện chỉ vẽ lại theo con số máy chủ trả |

**Bẫy phát hiện thêm khi làm HẠ TẦNG PHASE 8 (2026-09-06):**

| Bẫy | Hậu quả nếu bỏ qua | Xử lý ở |
|---|---|---|
| **nginx: location có `add_header` riêng bỏ rơi MỌI `add_header` cấp server** | Trang tĩnh (/, /assets/) mất HSTS + CSP trong khi đường API vẫn có — mỗi nơi một kiểu | HSTS đặt trong snippet `deploy/nginx/security-headers.conf` include vào TỪNG location tĩnh; đường proxy KHÔNG đặt thêm vì helmet đã đặt — đặt thêm là header nhân đôi (commit `b06673f`, `459d16e`) |
| **nginx 1.24 trên VPS không có chỉ thị `http2 on;`** | `nginx -t` chết «unknown directive», không reload được | Viết `listen 443 ssl http2;` (cú pháp trước 1.25.1) — `deploy/nginx/ttdt.site.conf` (commit `fc5f1d8`) |
| **sshd ăn chỉ thị ĐẦU TIÊN gặp, và `sshd_config.d` nạp theo bảng chữ cái** | `50-cloud-init.conf` đặt `PasswordAuthentication yes` đè cấu hình của mình — cửa dò mật khẩu vẫn mở | Đặt tên `49-qlcv-hardening.conf` để được đọc TRƯỚC; kiểm bằng `sshd -T \| grep passwordauthentication` |
| **`chmod +x` tệp của repo ngay trên VPS** | git thấy đổi mode ⇒ `git pull --ff-only` từ chối đúng lúc cần kéo bản sửa | Đánh dấu executable từ máy dev: `git update-index --chmod=+x <tệp>` rồi commit (commit `16328b0`) |
| **`source deploy/.env` trong shell** | Trong `.env` có dòng lịch cron (`*/2 * * * *`) không phải shell hợp lệ — kịch bản chết giữa chừng | Chỉ đọc biến cần dùng: `eval "$(grep -E '^(POSTGRES_USER\|POSTGRES_DB)=' .env)"` — xem đầu `deploy/backup.sh` |
| **Ảnh `node:24-alpine` không có curl** | HEALTHCHECK viết bằng curl làm container «unhealthy» vĩnh viễn dù app sống | busybox có sẵn wget: `wget -qO- http://127.0.0.1:3000/healthz` (Dockerfile) |
| **`npm ci --omit=dev` trong ảnh không cài devDependencies** | node-pg-migrate vắng trong container ⇒ không chạy được migration trên VPS | Chuyển `node-pg-migrate` sang `dependencies` (`server/package.json`); chạy bằng `docker compose run --rm app npm run migrate:up` |
| **File upload nằm TRONG ảnh** | Dựng lại ảnh (`compose build`) xoá sạch kết quả nhiệm vụ của người dùng | volume `../server/storage:/app/storage`; thư mục host phải `chown 1000:1000` (user `node` trong ảnh, uid 1000) |
| **`host.docker.internal` không tồn tại trên Linux** | OnlyOffice gọi ngược app lưu tài liệu thất bại | Trên VPS: `ONLYOFFICE_CALLBACK_BASE=http://app:3000` — TÊN dịch vụ trong mạng compose (chú thích trong `deploy/docker-compose.yml`) |
| **Phiên SSH từ máy dev đứt giữa chừng (không báo lỗi)** | Kịch bản dài chết theo phiên, để lại trạng thái nửa vời không biết đâu mà lần | Việc dài: `nohup ... > /root/<ten>.log 2>&1 &` trên VPS rồi đọc log bằng lệnh ngắn |
| **Log request in TRẦN mọi header — bí mật webhook lọt `docker logs`** | Ai đọc được log giả mạo được chữ ký webhook Zalo (đã xảy ra 2026-09-06: secret nằm nguyên trong log ngay lần Zalo tự kiểm đầu tiên) | Header bí mật phải có trong `redact` của `logger.js` (`2aeebd3` + `logger-redact.test.js` chốt); bí mật đã xuất hiện trong log hay chat là phải XOAY ngay, không «để mai» |
| **Production 0 tài khoản mà UI không tạo được tài khoản ĐẦU** (tạo tài khoản cần admin đăng nhập trước) | Mọi luồng «người thật» — e2e `.docx`, `LIENKET` Zalo, đẩy duyệt — đứng chờ, dễ tưởng hệ thống hỏng | Bước 0 Phase 9: admin đầu tạo thẳng bằng `psql` (hash bcrypt sinh TRONG container app, `must_change_password=true`, mật khẩu giao trực tiếp); mọi tài khoản SAU đó qua trang «Quản lý tài khoản» |
