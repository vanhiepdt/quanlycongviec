...
| 8b | **Sửa lỗi vặt giao diện** (người dùng yêu cầu làm TRƯỚC Phase 9) | 🟡 | **2026-09-07: người dùng NGHIỆM THU PC và cho phép phát hành — ĐÃ push GitHub + deploy VPS, HEAD `c0858f4`** (4 commit `b531f38`/`71150c5`/`2bebb8c`/`c0858f4`; backup exit 0 → pull ff-only → build → `up -d` → `migrate:up` ghi `019_refresh_countable_views`; dữ liệu giữ nguyên **4/2/0/2/1**, `sum(ty_le)=100` cả trên bảng lẫn qua view; `qlcv-app`+`qlcv-db` healthy, `qlcv-onlyoffice` chỉ Up; trang sống phát `app.js`/`project-details.js?v=20260907-1`, CSS `20260906-1`; sáu charts trả **401** không phiên, chưa kiểm 200 kèm phiên trên production; parity PC↔Git↔VPS cùng `c0858f4`, blob JS trùng byte — chi tiết dòng mới §13.3). Bổ sung sửa sáu biểu đồ 500: migration **019** làm mới view thiếu `ty_le`, đã lên local UAT, fingerprint/số dòng nghiệp vụ giữ nguyên; sáu API biểu đồ và Gantt HTTP 200 qua tài khoản test hợp lệ; focused 34/34, full tuần tự **1703/1703, 96 file**, scoped lint/format sạch. Launcher thêm smoke chỉ đọc sau migrate; `/giu /f` đã kiểm dừng an toàn cổng bận, chưa kiểm trọn luồng khởi động — launcher giữ UAT và chặn cổng bận/health lỗi; Quản lý công việc đọc cùng phòng theo ID ngay lần đầu; footer nháp Lưu tạm/Gửi đi phê duyệt; buster app.js + project-details.js **20260907-1**, CSS giữ **20260906-1**. Lint/format toàn repo còn lỗi ngoài scope, không tuyên bố sạch; kết quả test và giới hạn launcher ở dòng mới §13.3. Lịch sử trước đó: Mở 2026-09-06 — sửa trên PC → test xanh → push `vps/sua-loi-vat` (cắt từ 81d9fb9) → deploy VPS theo runbook. **Lỗi 1 XONG** (`5f0bb21`): 4 menu đầu trang KÉO DÀI khung ⇒ `.glass-card.absolute { position: absolute }` (app.css) + test hồi quy `menu-tha-xuong.test.js`, buster `app.css?v=20260906-1`. **Lỗi 2 XONG** (máy chủ `ca4c6fa` + web `17d74e5`, **đã deploy VPS 2026-09-07**): tiến độ KHÔNG còn nhập tay — tính từ mức hoàn thành các **nhóm file kết quả** (mục không có file = **0%**); thêm «**Tỷ lệ công việc (%)**» cho mỗi ĐẦU MỤC (việc con cấp 2, hoặc nhiệm vụ không nằm trong việc con): mặc định **chia đều** (số nguyên đúng 100, largest-remainder, tự co giãn khi thêm/xoá mục và GIỮ chỉnh tay), quyền sửa = admin/Phó GĐ/TP/PP **phụ trách** — thêm hàng `ty-le` vào bảng phân quyền động; **MỌI NƠI** (form, Gantt, cột «Tiến độ», modal chi tiết, E4/E3, thẻ dashboard, modal dự án, Excel) chuyển bình quân **GIA QUYỀN** (`tienDoDauMucKhach` phía client khớp `tienDoWork` server). Migration **018** (cột `ty_le` + 2 CHECK, dựng lại 2 view `v_countable_*`). **1696 test / 96 file** xanh. Buster: `app.css?v=20260906-1`, `app.js` + `project-details.js?v=20260906-2`. Chi tiết §13.3 dòng 2026-09-07. **Danh sách MỞ — người dùng bổ sung lỗi tiếp** |
