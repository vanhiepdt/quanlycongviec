...
| 8b | **Sửa lỗi vặt** (danh sách còn mở, trước Phase 9) | 🟡 | **2026-09-08: người dùng đã test PC, xác nhận OK và yêu cầu deploy + xóa dữ liệu database, giữ admin/Zalo. ĐÃ PHÁT HÀNH.** Mã `63f05b0` (máy chủ) + `4cc8bba` (giao diện), nhánh `vps/sua-loi-vat`; full PC **1735/1735, 97 file**, XSS106/903. VPS backup trước → build → migrate020 → transaction xóa dữ liệu có kiểm chứng giữ admin/Zalo → tạo lại db/OnlyOffice/app, GIỮ volume/.env/storage. Sau reset: **1 admin, 1 liên kết Zalo, 1 dòng mã liên kết**; công việc/nhiệm vụ/file/bản/phòng ban/phiên cũ =0. readyz OK, app/db healthy, OnlyOffice healthcheck true, website200; ba asset khớp blob Git, app buster20260907-2, project-details20260907-1, CSS20260906-1. Zalo token hợp lệ/webhook bật/lịch */2 giữ nguyên. Backup riêng và giới hạn restore dữ liệu cũ xem dòng mới §13.3 + runbook §3.1. Không seed production; không tự reset lại. Danh sách lỗi tiếp tục mở. |
