...
| Test đang xanh | **1258** trong 71 file, lint + `format:check` sạch. Pin XSS **86 chỗ / 606 giá trị** (`docs/XSS-4.6.md`). Banner + buster hiện tại: `app.js 20260828-83`, `app.css 20260828-5`, `project-details.js -4`. Thư viện mới của Phase 7: `exceljs@4.4.0` (đã ghi §3.3). Test jsdom chạy app.js thật đã có: `dept-select` (thêm `buildStaffEmailDatalist`), `project-form-phan-cong`, `task-form-candidate`, `project-details-phan-cong`, `gantt-ui`, `tasks-nhiem-vu-ui`, `pho-giam-doc-ui`, `uy-quyen-ui` (TC-UQ-15/16/18/18b/**19/19b**), `bo-loc-cong-viec`, `tai-khoan-ui` — thêm file jsdom mới thì **phải** khai vào danh sách `files` ở `server/eslint.config.js` |
...
TRẠNG THÁI: Phase 0–7 đã xong. 1258 test xanh trong 71 file, lint + format:check sạch.
...
XONG KHI: 1258 test cũ vẫn xanh · có test cho phần kiểm được bằng máy (Dockerfile/compose
