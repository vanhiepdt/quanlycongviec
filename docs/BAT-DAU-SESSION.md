...
| Phase kế tiếp | **ĐANG Ở Phase 8b — SỬA LỖI VẶT** (prompt mục 3b): lỗi 1 + lỗi 2 **ĐÃ XONG, đã deploy VPS** (2026-09-07, nhánh `vps/sua-loi-vat` @17d74e5). **Danh sách lỗi còn MỞ** — người dùng báo lỗi tiếp thì làm đúng quy trình mục 3b: sửa trên PC → `npm test` xanh → commit riêng từng lỗi → push + deploy VPS theo runbook (đổi `server/` ⇒ backup + build + migrate; chỉ đổi `web/` ⇒ bump buster, không build). Mắt kiểm còn nợ trên trình duyệt thật: form tỷ lệ theo vai, Gantt, biểu đồ E4/E3. Khi danh sách lỗi ĐÓNG: gộp `vps/sua-loi-vat` vào `main`, rồi **Phase 9**: bước 0 tạo admin đầu **ĐÃ XONG** 2026-09-06 (tài khoản `TTDT001`, psql + hash trong container), còn e2e OnlyOffice `.docx` + e2e Zalo `LIENKET`/đẩy duyệt (tạm gác để sửa lỗi vặt) rồi 9.1–9.7 trên `main` (§7.11) |
