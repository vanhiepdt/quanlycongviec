...
- Phase 8 việc 1–4 + 7 ĐÃ XONG trên nhánh vps/phase-8-hatang: https://ttdt.site SỐNG (HTTPS + HSTS), Docker chạy ở /opt/qlcv, 17 migration, sao lưu cron 02:00 + drill khôi phục THẬT ~2 giây, deploy/runbook.md, hardening (UFW/fail2ban/SSH chỉ khoá/.env 600). Suite test 1668/1668, pin XSS 106/903.
- Phase 8 việc 5 và 6 CHỜ NGƯỜI DÙNG: (a) việc 5 — DNS A office.ttdt.site → 157.10.199.148 chưa trỏ nên chưa certbot, chưa bật conf OnlyOffice, chưa e2e .docx (conf đã commit sẵn: deploy/nginx/office.ttdt.site.conf + websocket-map.conf); (b) việc 6 — Bot Token Zalo cũ đã LỘ trong chat, người dùng phải XOAY token mới và TỰ dán vào /opt/qlcv/deploy/.env (KHÔNG dán vào chat) rồi mới chạy zalo:kiem + zalo:webhook + luồng liên kết thật.

VIỆC:

0. Chốt Phase 8 TRƯỚC (nếu chưa xong):
   - Người dùng đã trỏ DNS office.ttdt.site → làm việc 5: certbot --nginx -d office.ttdt.site; đưa office.ttdt.site.conf + websocket-map.conf vào /etc/nginx/; nginx -t + reload; e2e: một người đăng nhập thật, mở sửa một file .docx qua OnlyOffice từ đầu đến cuối.
   - Người dùng đã dán token mới vào deploy/.env → làm việc 6: docker compose up -d app; npm run zalo:kiem (trong container app); npm run zalo:webhook (đăng ký https://ttdt.site/api/zalo-bot/webhook); một người thật nhắn LIENKET <mã 6 số>; thử đẩy duyệt thật. Webhook không thông thì tạm dùng ZALO_BOT_NHAN=polling.
   - Phase 8 đóng hẳn → gộp vps/phase-8-hatang vào main (§7.11: Phase 9 làm trên main).
