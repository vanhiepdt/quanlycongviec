...
| Test đang xanh | **1095** trong 64 file, lint + `format:check` sạch. Pin XSS **79 chỗ / 566 giá trị** (`docs/XSS-4.6.md`). Thư viện mới của Phase 7: `exceljs@4.4.0` (đã ghi §3.3). Vòng 2–4 Gantt (2026-08-27): **bump cache-buster app.css** (`?v=20260827-3`) + hàng 48px/thanh 38px + mọi cấp thẳng lề + **thanh đặt bằng grid-column trên lưới chung với header** (fix múi giờ ngày cuối tháng). Test jsdom chạy app.js thật đã có: `dept-select`, `project-form-phan-cong`, `task-form-candidate`, `project-details-phan-cong`, `gantt-ui` |
