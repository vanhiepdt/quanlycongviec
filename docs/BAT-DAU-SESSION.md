...
| Test đang xanh | **1325** trong 77 file, lint + `format:check` sạch — **hai lượt `npm test` liền không còn ca nào đỏ** (2026-08-29). Trước đó mỗi lượt đỏ 1–4 ca *đổi chỗ*: gốc là `middleware/audit.js` ghi nhật ký ở `res.on('finish')` nên lượt ghi sống lâu hơn request; nay `flushAudit()` chờ đúng những lượt ghi đang bay, `resetTables()` gọi nó **trước** `TRUNCATE` — thấy đỏ ở test nhật ký thì **đừng** cho là đỏ giả nữa. Pin XSS **90 chỗ / 662 giá trị** (`docs/XSS-4.6.md`). Banner + buster hiện tại: `app.js 20260828-87`, `app.css 20260828-5`, `project-details.js -4`. Thư viện mới của Phase 7: `exceljs@4.4.0` (đã ghi §3.3). Test jsdom chạy app.js thật đã có: `dept-select` (thêm `buildStaffEmailDatalist`), `project-form-phan-cong`, `task-form-candidate`, `project-details-phan-cong`, `gantt-ui`, `tasks-nhiem-vu-ui` (thêm TC-TASKUI-14..18 **phạm vi xem của PGĐ**), `pho-giam-doc-ui`, `uy-quyen-ui` (TC-UQ-15/16/18/18b/**19/19b**), `bo-loc-cong-viec`, `tai-khoan-ui`, `nhat-ky-ui` (TC-NKUI-01..10), `ten-thang-ui` (TC-TENTHANG-25..38) — thêm file jsdom mới thì **phải** khai vào danh sách `files` ở `server/eslint.config.js` |
