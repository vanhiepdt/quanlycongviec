...
| | |
|---|---|
| Nhánh đang làm | `vps/sua-loi-vat`; mã Phase8b đã phát hành **`63f05b0` + `4cc8bba`**, tài liệu phát hành commit riêng sau kiểm chứng. Không cắt lại nhánh từ `81d9fb9`; không quay về `54c2759` trừ rollback có chủ đích. Nhật ký mới §13.3 ghi nghiệm thu PC, backup, reset giữ admin/Zalo và restart cả ba container. |
...
| Test đang xanh | **2026-09-08: 1735/1735 test, 97/97 file, exit0**, full `npm test` từ `server/` sau thay đổi code cuối, 162.35 giây; không chạy focused song song. Scoped ESLint/Prettier 9 file máy chủ/tests sạch, cú pháp app.js và diff-check sạch; XSS **106/903** giữ nguyên. Baseline đợt này 1703/96, thêm 32 test. **Không chạy lại lint/format toàn repo**, nợ ngoài scope từ đợt trước không được coi là đã sửa. Browser NV/tab vàng/Lưu tạm đã kiểm; callback/editor tự động không thay thế nghiệm thu DOCX thật/gửi-hủy/Zalo. |
...
| Phase kế tiếp | **Vẫn Phase8b, danh sách lỗi mở.** Đợt lệnh sửa/OnlyOffice đã có OK PC và đã lên VPS ngày2026-09-08; reset dữ liệu theo yêu cầu đã xong, **không lặp lại reset/seed**. Người dùng đăng nhập lại admin, tạo lại phòng/tài khoản/công việc; e2e DOCX/Zalo production cần dữ liệu mới. Lỗi bổ sung phải quay lại PC → full test xanh → người dùng OK mới → commit/push/deploy theo runbook. Chỉ chuyển Phase9/gộp nhánh khi người dùng đóng danh sách. |
