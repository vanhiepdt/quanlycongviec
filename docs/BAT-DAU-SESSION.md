...
| | |
|---|---|
| Nhánh đang làm | `vps/quan-ly-nhiem-vu-pgd` (tách từ `vps/phase-7-misc`) — HEAD `af1b92e`. Việc ngoài kế hoạch đã xong: tab «Quản lý Nhiệm vụ», Phó Giám đốc thấy tab «Quản lý công việc», **ủy quyền có thời hạn**, bộ lọc một dòng + Tháng/Năm cho «Quản lý công việc», trang «Quản lý tài khoản», **siết ủy quyền theo §13.4 mục 17/18/20** (thứ bậc + cùng phòng + phê duyệt của người được ủy quyền), **ô «Người nhận» của form ủy quyền là Ô CHỌN NGƯỜI** (danh sách đúng luật máy chủ), và **Phó Giám đốc thấy hết nhiệm vụ của CÁC phòng mình phụ trách** trong tab «Quản lý Nhiệm vụ». Nhánh trước: `vps/phase-7-misc` dừng ở `82e6958`, `vps/tinh-nang-phan-cong` ở `5cb6360`, `vps/phase-6-stats` ở `b2e65f1`, `vps/phase-5-approval` ở `5e89293` |
...
| Test đang xanh | **1263** trong 71 file, lint + `format:check` sạch. Pin XSS **86 chỗ / 606 giá trị** (`docs/XSS-4.6.md`). Banner + buster hiện tại: `app.js 20260828-84`, `app.css 20260828-5`, `project-details.js -4`. Thư viện mới của Phase 7: `exceljs@4.4.0` (đã ghi §3.3). Test jsdom chạy app.js thật đã có: `dept-select` (thêm `buildStaffEmailDatalist`), `project-form-phan-cong`, `task-form-candidate`, `project-details-phan-cong`, `gantt-ui`, `tasks-nhiem-vu-ui` (thêm TC-TASKUI-14..18 **phạm vi xem của PGĐ**), `pho-giam-doc-ui`, `uy-quyen-ui` (TC-UQ-15/16/18/18b/**19/19b**), `bo-loc-cong-viec`, `tai-khoan-ui` — thêm file jsdom mới thì **phải** khai vào danh sách `files` ở `server/eslint.config.js` |
