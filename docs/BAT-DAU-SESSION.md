...
| Test đang xanh | **1091** trong 64 file, lint + `format:check` sạch. Pin XSS **79 chỗ / 566 giá trị** (`docs/XSS-4.6.md`). Thư viện mới của Phase 7: `exceljs@4.4.0` (đã ghi §3.3). Vòng 2 Gantt (2026-08-27): **bump cache-buster app.css lần đầu** (`?v=20260827-1` — trước đó chưa từng bump, Nginx cache 30 ngày kẹt CSS cũ) + hàng 48px/thanh 38px + test bắn sự kiện tooltip. Vòng 3 Gantt (2026-08-27): **mọi cấp thẳng lề trái** — bỏ toàn bộ thụt dòng work/CV con/nhiệm vụ + `pl-4` nhóm (`.gantt-item` & thân nhóm `padding-left:0 !important`). Test jsdom chạy app.js thật đã có: `dept-select`, `project-form-phan-cong`, `task-form-candidate`, `project-details-phan-cong`, `gantt-ui` |
