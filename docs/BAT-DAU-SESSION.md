...
| Test đang xanh | **1368** trong 81 file, lint + `format:check` sạch — **hai lượt `npm test` liền không còn ca nào đỏ** (2026-08-29). Trước đó mỗi lượt đỏ 1–4 ca *đổi chỗ*: gốc là `middleware/audit.js` ghi nhật ký ở `res.on('finish')` nên lượt ghi sống lâu hơn request; nay `flushAudit()` chờ đúng những lượt ghi đang bay, `resetTables()` gọi nó **trước** `TRUNCATE` — thấy đỏ ở test nhật ký thì **đừng** cho là đỏ giả nữa. Pin XSS **98 chỗ / 686 giá trị** (`docs/XSS-4.6.md`). Banner + buster hiện tại: `app.js 20260829-4`, `app.css 20260828-7`, `project-details.js 20260829-1`. Thư viện mới của Phase 7: `exceljs@4.4.0` (đã ghi §3.3). Test jsdom chạy app.js thật đã có: `dept-select` (thêm `buildStaffEmailDatalist`), `project-form-phan-cong`, `task-form-candidate`, `project-details-phan-cong`, `gantt-ui`, `approvals-ui` (màn hình «Chờ duyệt» — 2026-08-28), `tasks-nhiem-vu-ui` (thêm TC-TASKUI-14..18 **phạm vi xem của PGĐ**, TC-TASKUI-19 **hết mã**), `pho-giam-doc-ui` (thêm TC-TP-UI **TP/PP thêm được công việc**), `uy-quyen-ui` (TC-UQ-15/16/18/18b/**19/19b**), `bo-loc-cong-viec` (thêm TC-CV-BL-2 **thẻ hết mã**), `tai-khoan-ui` (thêm TC-TKPQ-01..08 **bảng Phân quyền hệ thống + trình sửa dropdown**), `phan-quyen-ghi-de` (TC-PQ-10..13 **`can()` đọc ghi đè**), `nhat-ky-ui` (TC-NKUI-01..10), `ten-thang-ui` (TC-TENTHANG-25..38), `hoat-dong-ui` (TC-HD-01..06 «Hoạt động gần đây» — 2026-08-29) — thêm file jsdom mới thì **phải** khai vào danh sách `files` ở `server/eslint.config.js` |
