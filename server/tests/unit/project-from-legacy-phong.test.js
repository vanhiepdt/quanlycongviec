...
//   • Đợt A (028): ô Ban lãnh đạo kiểm soát thành MẢNG (`supervisorIds`), nhưng client CŨ vẫn gửi
//     khoá đơn `supervisorId` — cầu phải nhận cả hai, và `""` vẫn nghĩa là «bỏ chọn hết».
...
describe('projectFromLegacy — ô Phòng và Ban lãnh đạo kiểm soát', () => {
  it('chuỗi id số từ form ⇒ số nguyên trong payload', () => {
    ...
    expect(out.supervisorIds).toEqual([2]);
  });

  it('khoá MỚI `supervisorIds` (chuỗi id ngăn bởi dấu phẩy) ⇒ mảng số', () => {
    const out = projectFromLegacy({ name: 'A', supervisorIds: '2, 5' });
    expect(out.supervisorIds).toEqual([2, 5]);
  });

  it('`supervisorIds` thắng `supervisorId` khi form mới gửi cả hai', () => {
    const out = projectFromLegacy({ name: 'A', supervisorIds: '5', supervisorId: '2' });
    expect(out.supervisorIds).toEqual([5]);
  });
  ...
  it('"" = chọn «Công việc chung» ⇒ null (PATCH xoá liên kết, không phải bỏ qua)', () => {
    ...
    // Bỏ chọn = MẢNG RỖNG chứ không phải `null`: cột giờ là `bigint[] NOT NULL DEFAULT '{}'`,
    // và rỗng PHẢI đi qua được `dropUndefined` để PATCH thật sự xoá người đã chọn.
    expect(out.supervisorIds).toEqual([]);
    expect(projectFromLegacy({ name: 'A', supervisorIds: '' }).supervisorIds).toEqual([]);
  });
  ...
  it('thiếu khoá ⇒ không có khoá trong payload (PATCH không đổi trường không gửi)', () => {
    ...
    expect(Object.hasOwn(out, 'supervisorIds')).toBe(false);
